...
- **Compression alone terminates on a fixed dataset**, and the a2a ratchet's recurring villain — the absorbing state — is named in [`dimensionality_expansion.md`](../beliefs/dimensionality_expansion.md) as *"the predicted consequence of running a compressor with no expansion drive."* The behavioural fact (naive wake-sleep stops compounding; the gated ratchet hits activation-norm inflation by 16 cycles) is not in doubt.
  > **Caveat added 2026-07-26.** This bullet used to state the mechanism as a partition — `R_act ≈ R_comp + R_res`, with compression draining `R_res → R_comp` at fixed `R_act` — and to cite rank numbers (wake-sleep 23.9 → 13.5; the internal route 27.8 → 30.2) as the measurement of it. **The partition does not hold**: `R_comp` is not independent of `R_act`, and `R_res` is blind to residual magnitude, so those numbers do not license the reading "the frontier drained." The *terminates* claim is retained on the behavioural evidence above; the rank-flow mechanism is **withdrawn outright**. *(Resolved 2026-07-27: the re-measurement landed at [`rhm/residual_decomposition/`](../experiments/rhm/residual_decomposition/README.md). A wake-sleep 2x2 on a fixed held-out probe finds compression has no measurable effect on the triple — wake-sleep ~ open-loop, ΔR_act +52.7 vs +51.9 — and `R_res` does not drain. The replacement instrument is β and `R_res_participation`.)* Nothing in §4's dense/evaluative argument depends on it — that is a claim about *signals* and never referenced the partition.
...
> **Measured, 2026-07-27** — [`mjc/expansion/`](../experiments/mjc/expansion/README.md). Eight rounds of continuous, non-stationary, genuinely-difficult prediction pressure on an n=5 arm produced **no expansion**: the frontier moved ±0.08 directions against a +0.72 ± 0.42 calibration from a real support increase. This is the existence proof the argument above wanted — dense predictive pressure, however hard and however non-stationary, does not open dimensions. **Caveat on scope**: the plant is fixed-DOF and non-hierarchical, so this shows dense pressure is *insufficient*, not that an evaluative grader is *sufficient*. Whether an evaluative grader (LP) actually expands anything is still untested.
>
> ✅ **Answered 2026-07-29 — and the answer separates the two clauses.** [`full_loop`](../experiments/rhm/directed_sculpting/full_loop/README.md) §5 runs the missing cell on a hierarchical domain: a **grounded evaluative** grader expands (belief PR 7.0 → 11.6, transferable fresh-FM plannability 0.17 → 0.55, ballistic control ~15× the no-loop floor), at 3 seeds, in 3 grading worlds. So *"an evaluative grader is sufficient"* now has an anchor — **but not for LP**. The grader that did it is a grounded DP-best-move CE (external, precomputed); LP-alone is separately measured as the *weak* allocation tap (18% vs relevance's 84%, §10's own parenthetical cashed out). And the same 2×2 finds **drift orthogonal** to all of it. See [meta_learning_under_metered_data.md](meta_learning_under_metered_data.md).
...
> **Note on our own history.** *Dimensionality expansion under drifting dynamics* was [`physical_control_substrate.md`](physical_control_substrate.md) cut #5, billed in its own text as **"the single biggest open question of the whole program."** It was never run — the drift machinery built to make it free was consumed by the value-loop program for a different purpose, and `mjc/HISTORY.md`[^private] records the abandonment under "Planned and never run" without noting it was the flagship. Two independent routes have now arrived back at it, which is usually the tell that it is the real question.
>
> **Verified unrun (2026-07-25), and the search found the hazard.** No rank or spectral measurement exists anywhere in `mjc/` except cut #1's, and git history shows nothing started-and-abandoned. But cut #1 *did* run a participation ratio (`contact_residual/contact_residual.py:104-119`) on the 8-dim state residual of one FM on a stationary pusher, got a **negative** (contact eff-rank 3.60 > free 2.52), and retired *"residual rank ∝ DGP complexity"* — and a2a independently found `rank ⊥ noise` (language's residual is full-rank 200/256 yet learnable to cosine 0.97). **This repo has found rank to be a weak instrument twice**, so any cut #5 must open by saying why it is not the third time.

> **Update 2026-07-27: run, and the flagship's premise was wrong.** [`mjc/expansion/`](../experiments/mjc/expansion/README.md) — drift does not open the frontier (±0.08 directions vs a +0.72 ± 0.42 calibration), so *"drifting dynamics is a novelty generator"* is retired; drift stays essential for **realism** (§2 condition 4), not for representational growth. It was **not** the fourth rank failure, and the reason is structural rather than lucky: the cut was built **calibrate → measure → calibrate**, certifying the instrument against a known-wrong model *and* against a known-real dimensional increase before reading the null. Two findings for this doc: **gap width on a control substrate IS §2 condition 1's commitment-under-delay axis** — a wrong model's signature grows from 1.4× at one step to 2.7–4.1× at the ballistic horizon, i.e. the ballistic asymmetry showing up in a *representation* readout; and **β is unusable below ~100 directions**, so §4b's representation-side framing needs a high-dimensional domain to test at all. The §9 cut is untouched and still unbuilt.
>
> **Update 2026-07-26: it was going to be the third time.** The defense written here — *the retired claim was absolute and static, this one is differential with a matched control* — was correct as far as it went and **insufficient**. In-flight re-measurement on the a2a/RHM substrate shows the `(R_act, R_comp, R_res)` triple does not decompose: `R_comp` is not independent of `R_act`, so differencing it against a matched control yields a redundant difference. The lesson to carry forward is that a matched control does not rescue a readout whose components are not independent — **check independence and magnitude-sensitivity before designing around any rank-shaped instrument.** The design constraints in this note (support-growing drift, chain ≥ 5 so capacity binds) are unaffected; the *readout* is open, and cut #5 is now additionally gated behind a gap-width calibration, both at [`mjc/README.md`](../experiments/mjc/README.md) §Next steps #3.
...
`mjc/HISTORY.md`[^private]'s deepest cross-cutting claim is not about value at all: **most of the arc's nulls were instrument failures of escalating subtlety.** Every one was caught by playing two graders of different type against each other.
...
Consequences: most current RL post-training has one grader and a reflection; **verifiable-reward domains (does the code run, does the proof check) are the real second grader**, which is suggestive about why exactly those are the domains where RL visibly works.

> **Extended 2026-07-29 — the criterion applied to corpus breadth.** [breadth_as_grader_heterogeneity.md](breadth_as_grader_heterogeneity.md) reads a *semantically distant corpus region* as heterogeneous grading material (physics is sighted where fantasy football is blind), so scaling-era breadth was a cheap, fossilized way to buy what §4 says one signal cannot be. Two consequences it draws from §8 specifically: a learner that mints its own curriculum and evaluates it from within the same band is **one grader in a mirror** (model autophagy is the concrete failure), and **verification is available inside narrow bands where breadth is not** — which is why the criterion predicts a narrow-band expansion drive is viable *iff* its acceptance signal is non-mirror. Argument, not measurement; the cut is unbuilt. Note also where Ilya locates the same intuition (§01:29:23): he reaches for diversity *between* agents (self-play, debate, prover-verifier). The brain does it *within* one agent, by having organs with genuinely different objectives — enormously cheaper, and operating on the timescale of a single action rather than of a population.
...
- Does the §9 grader-disagreement cut have a **representation-side readout** at all? This was originally phrased as *"does detecting a blind grader show up as `R_res` refilling?"* — withdrawn, since `R_res` is magnitude-blind and does not count a frontier (§4b caveat). The question survives the instrument: is there *any* representational signature of a grader having gone blind in a region, or is blindness only ever visible as disagreement between two graders' outputs? A negative answer would be interesting on its own — it would mean the frame's compression/expansion half has no independent readout and rests entirely on the signal-side argument.
