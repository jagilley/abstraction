...
**Constructive counterpart (2026-07-12).** Rather than test whether cancellation *emerges* from additive training, [efference_copy_cancellation.md](efference_copy_cancellation.md) proposes **wiring it in**: subtract the forecast and propagate the residual (`f(h − gate·p) + p`, predictive coding) instead of adding it as a side-channel (`h + gate·p`). That doc argues the additive sign/topology is what produces the dependency, runaway-gate, and rollout-drift pathologies, and that cancellation fixes all three while giving the separable self-model a home feedforward otherwise lacks (Payoff 2 there = a direct partial answer to the "no home for a self-model feedforward" claim above).
...
- Does the model treat injected (efference-copy) activations as a **categorically distinct input class** — cancellation/attribution rather than summation (the corollary-discharge signature)? If so, the injection is a genuine new modality, not noisy extra data. (Feedforward-runnable on existing CL checkpoints; not degenerate.) **Constructive version now in [efference_copy_cancellation.md](efference_copy_cancellation.md)** — build the cancellation in (subtract-then-recombine) rather than test whether it emerges from additive training.
