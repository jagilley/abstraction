...
4. The gate-closing prediction requires the bilevel outer loop to evaluate on a distribution that differs from the inner loop. On a fixed dataset, inner = outer, so more compression is always beneficial. The human genome builds in the prior that "the outer loop WILL contain novelty" even when the current experience stream doesn't show it yet — this is the missing prior in our current setup.
   ...
   [**Update (2026-06-18):** OOD gate experiments tested this directly. Distribution difference is necessary but not sufficient for *selective* closing. The gate's selectivity is per-dimension of FM error — it can only discriminate distinctions the FM's error structure encodes. On a within-manifold shift (digits 0-6 → 0-9), the FM error decomposition remained valid and the gate was input-selective (opening more for novel digits). On a cross-manifold shift (MNIST → Fashion-MNIST), the FM error dimensions didn't align with domain boundaries, so the gate closed globally rather than selectively. The gate does meta-learning over the model's computational structure, not over data domains. Self-knowledge built on a narrow training distribution (MNIST) doesn't transfer to a different visual domain (Fashion-MNIST) because the FM's error vocabulary is calibrated to the original domain's computational patterns. This suggests the gated ratchet should be tested on language, where the training distribution is broad enough that the FM develops computational abstractions spanning the full input space.]]
