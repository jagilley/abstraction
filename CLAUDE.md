...
- For coding-type tasks where not burning context spuriously is important, using subagents up front to ground yourself in the state of our work is a good idea. If you're going to use an Explore subagent, please use Sonnet or Opus as the executor model (and don't background the agent; wait for it to come back so it can point you to the most relevant files you should read). For analysis-type tasks where the goal is more to provide an ideological synthesis to the user, consuming the relevant context directly is often load-bearing for interpreting results properly. Subagents, especially those that run smaller models, risk misinterpreting results in subtle domains, so you can either do the exploring yourself or read the files it surfaces for yourself to align on the interpretation.
