...
| `directed_loop_audit.py` | **The S2 audit** (local, read-only; no Modal, no re-run). Re-reads `figures/directed_loop_<tag>/results.json` and asks whether the harness could have measured its own headline null. Four checks: **(1)** seed decomposition of `excess damage`; **(2)** early/late split — what fraction of the "adaptation speed" score accrues *after* recovery completes (loopC 62%, loopB 35%); **(3)** mechanism-vs-control — re-ranks every policy by the **value-relevant FM error** (`drift_value_loop` Cut 3's teacher) and prints both orderings; **(4)** the measurement subsidy, `probe_n×K + monitor_n×K` free teleported transitions vs the allocated `budget` (22×). **Result**: the mechanism ranking reproduces the predicted ladder exactly (oracle 2nd) while the control ranking puts the privileged oracle 4th; `value` beats `lprog-only` on the sighted grader in 6/6 seeds. This is the provenance of the README's retraction. `--tags`. |
...
`directed_loop_audit_{loopB,loopC}.log` — output of `directed_loop_audit.py`, the provenance of the README's retraction of S2's inner null. Regenerable at any time from the stored `results.json`.
