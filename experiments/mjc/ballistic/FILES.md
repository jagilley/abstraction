...
| [`arm/`](arm/) | **Cut 4c-arm — the second task family** (`arm_readapt.py`, `train.sh`). Ports Cut 4c's endogenous re-adaptation loop onto [`../arm_substrate/`](../arm_substrate/README.md) with a Shadmehr curl-field drift (`b0=0` naive → `b1=6` field), retiring this node's **single-family** caveat. Substrate config held byte-identical to arm_substrate P5 (the exogenous-axis half of the same claim) so the only difference is where FM quality comes from. Adds the **aftereffect** readout — grade the re-adapted FM back in the field-free world and check for a *mirror-signed* lateral deviation — which the pusher structurally could not produce. Scores **excess** FM error against a fresh-`b1` ceiling, P5's own prescribed fix for its non-monotonic task-probe axis. **Status: runs in flight, no README yet.** |
