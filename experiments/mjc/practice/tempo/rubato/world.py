"""The rubato world: `accelerando/world.py`, FORKED VERBATIM, plus the KINEMATIC unit and the
executors that convert one into commands.

WHAT RUBATO ADDS (all additive and all at the bottom of the file; every accelerando number is
reproduced bit-for-bit, which gate P-T1 asserts at max|delta| = 0)
    * `RolloutPool.run_path` / `_pool_rollout_path` — an open-loop grounding that records the
      FULL STATE (x, v) at every control step, not just the checkpoint errors and the final
      state. This is how a tape's REALISED PATH is measured. It is the plant, executed; nothing
      is predicted.
    * `KinUnit` / `kin_path` / `resample_path` — THE KINEMATIC UNIT. A committed unit's stored
      content becomes the path its own tape produced, expressed against PHASE phi in [0, 1] over
      the unit's span, with the clock OUTSIDE the program: at a target tempo the same phase
      curve is sampled on the destination's step grid and its velocities scale by the speed
      ratio s = H_src / H_dst. There is no tempo inside a path.
    * `inv_zoh` / `inv_ct` — THE EXECUTOR, feedforward. Two inverse-dynamics schemes on the
      body's true constants, both experimenter-side oracles:
        `zoh` — the exact inverse of the ZERO-ORDER-HOLD discretisation of the free-flight
                plant, `v_{n+1} = alpha v_n + (1 - alpha) v_term u_n` with
                `alpha = exp(-dt_ctrl / tau)`. On this body `dt_ctrl / tau = 0.8`, so the
                control step is NOT small against the body and the discretisation is the whole
                question. THE SCHEME OF RECORD.
        `ct`  — the continuous-time formula `u = (m a + c v) / gear` evaluated at the step's own
                start, i.e. the first-order scheme the brief names. It differs from `zoh` ONLY
                in the coefficient on acceleration, by `(dt/tau) / (1 - exp(-dt/tau))`, and both
                are free, so which one the body wants is measured rather than argued.
      NEITHER reads a state at run time: a feedforward executor turns a path into a command tape
      before the unit is launched, exactly as a recording is a command tape. The body model sits
      on the LEARNER's side of the meter and is never handed to the incumbent as a planner.
    * `kin_library` — a `LevelLibrary` whose members'  commands are a source library's paths
      converted at a destination tempo. The slot structure, the parents, the poison twins and
      the keys are carried over UNCHANGED, exactly as `resample_library` does, so a kinematic
      arm is the same vocabulary asked for at a different tempo and `LadderDecider` plays it
      with no changes at all.
    * `fwd_zoh` — the exact forward step of the same free-flight model, and `World.obs_predict`,
      the EFFERENCE-COPY read (`accompanist` d3b's operator, ported): the Delta-stale read
      rolled forward along the commands already issued. It exists to support the `reflex_ec`
      REFERENCE ROW — the incumbent given the same body model the learner holds — which is
      reported and never headlined. Off by default, so every accelerando number is untouched.

The original accelerando docstring follows.

The accelerando world: `prestissimo/world.py`, FORKED VERBATIM, plus the listener's clock and
the tempo resampler.

WHAT ACCELERANDO ADDS (all additive; every donor number is reproduced bit-for-bit)
    * `listener_err` / `_boxcar` and the `e_listen` fields on `traverse`'s output — THE SECOND
      GRADE, pre-fixed with the first. Both the executed trajectory and the schedule's own
      reference are low-passed with the SAME fixed WALL-CLOCK boxcar (one slow note, 384 ms) and
      compared densely. At the slowest tempo the window is one note and the grade is a mildly
      smoothed tracking error; at the fastest it spans eight notes, so within-phrase wiggle and
      timing slop are forgiven while the phrase's shape is not. Nothing about it is arm-specific
      and it is computed from the TRUE trajectory, like every other instrument.
    * `resample_cmds` / `resample_library` — the `rec_scaled` arm's content: a recording cut at
      one tempo, time-resampled note by note to another. Arithmetic on the command tape alone;
      it reads no state, so it is not an inverse model of the body (which on this plant would be
      `u = (m a + c v)/gear` — exactly the thing this arc forbids). Because the resampling is
      NOTE-WISE, the weld identity survives it: resampling a level-l member equals welding the
      resampled halves, so gate P-N holds on the scaled library too.
    * `traj_all` is always recorded (it was already allocated whenever `collect_traj` was set);
      the listener grade needs the realised path at every cell, and the cost is one float32
      (Bn, K*H, 2) array.
...
def _pool_rollout_path(payload):
    """(s0 (n,4), cmds (n,L,2)) -> states (n, L+1, 4): the FULL realised path.

    RUBATO. The same grounding `_pool_rollout` performs, recording the state after every control
    step instead of only the checkpoints and the final state. `states[:, 0]` is the launch state
    itself, so the array is the path INCLUDING its own start and has `L + 1` samples for `L`
    steps. No RNG inside, so chunking across processes is bit-identical, exactly as for `run`.
    """
    s0, cmds = payload
    n, L = cmds.shape[0], cmds.shape[1]
    st = np.empty((n, L + 1, 4), np.float64)
    for i in range(n):
        _ENV.set_state(s0[i, :2].astype(np.float64), s0[i, 2:].astype(np.float64))
        st[i, 0] = np.asarray(s0[i], np.float64)
        for h in range(L):
            s2, _ = _ENV.step(cmds[i, h], _FS)
            st[i, h + 1] = np.asarray(s2, np.float64)
    return st
...
class RolloutPool:
    ...
    def run_path(self, s0, cmds):
        """RUBATO: `run`, recording the full realised path (n, L+1, 4) instead of checkpoints.

        Charged by the caller exactly like `run` when it is a grounding; called with
        `charge=False` only at instrument sites, each labelled there."""
        s0 = np.ascontiguousarray(s0, np.float32)
        cmds = np.ascontiguousarray(cmds, np.float32)
        n = len(s0)
        if n == 0:
            return np.zeros((0, cmds.shape[1] + 1, 4))
        if self._ex is None:
            return _pool_rollout_path((s0, cmds))
        nb = min(self.n_proc, n)
        bnds = np.linspace(0, n, nb + 1).astype(int)
        parts = [(s0[a:b], cmds[a:b]) for a, b in zip(bnds[:-1], bnds[1:]) if b > a]
        return np.concatenate(list(self._ex.map(_pool_rollout_path, parts)), 0)
...
class World:
    ...
    def __init__(self, cfg, piece=None, n_proc=1, rot=True):
        ...
        # ACCELERANDO: the listener's clock, in CONTROL STEPS. One slow note (16 steps, 384 ms)
        # at every tempo, so the grading window is a constant of the room and not of the score.
        self.w_listen = int(cfg.get("w_listen", P.W_LISTEN))
        # RUBATO, additive and INERT unless switched on: the efference-copy read
        # (`accompanist` d3b's operator). With `obs_predict` set AND an `fm_step` installed,
        # every agent-side read is the Delta-stale true state rolled forward along the raw
        # commands already issued since it, instead of the stale state itself. It exists for the
        # `reflex_ec` REFERENCE ROW — the incumbent handed the same body model the learner holds
        # — and is reported, never headlined. At `obs_predict = False` (the default, and what
        # every accelerando arm runs under) `obs()` is accelerando's `obs()` verbatim.
        self.obs_predict = bool(cfg.get("obs_predict", False))
        self.fm_step = None            # (states (n,4), u (n,2)) -> next states (n,4)
    ...
    def rollout_path(self, s0, cmds, led=None, charge=True):
        """RUBATO: a grounding that returns the FULL realised path (n, L+1, 4).

        This is how the kinematic unit's content is MEASURED: a stored tape is executed on the
        plant from the launch it was cut at, and what the body did is the path. `legato` F4's
        discipline — measured content carries no composition error because the body produced it.
        Charged exactly like `rollout`; `charge=False` is for instruments only.
        """
        st = self.pool.run_path(s0, cmds)
        if led is not None and charge and len(s0):
            led.add(ground=len(s0), ground_steps=len(s0) * cmds.shape[1])
        return st
    ...
    def traverse(self, decide, starts, rng, led, explore=0.0, collect=False, obs_delay=0,
        ...
        issued = [] if maxD > 0 else None     # RUBATO: efference copy. issued[j]: hist[j]->hist[j+1]
        cur = [D]                     # the delay in force at the segment being played

        def obs():
            """The agent's read. `hist[-1-D]`, clamped at the traversal start.

            RUBATO, additive and inert unless `obs_predict` is set AND an `fm_step` is installed:
            the stale read is carried forward to now by rolling the body model along the RAW
            commands already issued since it (`accompanist` d3b / `offbook` round 7b, verbatim in
            operation). The raw command is what the agent knows it asked for; the motor noise on
            top of it is exactly what the copy cannot contain. At `obs_predict = False` this
            function is accelerando's, line for line.
            """
            if cur[0] <= 0:
                return states
            i = max(0, len(hist) - 1 - cur[0])
            if not self.obs_predict or self.fm_step is None:
                return hist[i]
            st = np.asarray(hist[i], np.float32).copy()
            for u in issued[i:]:
                st = np.asarray(self.fm_step(st, u), np.float32)
            return st
        ...
        closed = [None]              # RUBATO: (executor, global step the span started at)
        ...
        # ACCELERANDO: the realised path is now recorded ALWAYS (it was already allocated
        # whenever `collect_traj` was set), because the listener's-clock grade reads it at every
        # cell. `out["traj"]` and the per-seam `traces` entry stay gated on `collect_traj`, so
        # nothing a donor consumer sees changes.
        traj_all = np.zeros((Bn, Kn * H, 2), np.float32)
        keep_traj = bool(collect_traj)
        ...
        for k in range(Kn):
            ...
            if len(need):
                ...
                if d["kind"] == "reflex":
                    ...
                    # RUBATO PHASE 3: a CLOSED-LOOP executor owns the span. Whole-batch only —
                    # every arm that uses it fixes ONE level, so `left` decrements uniformly and
                    # `need` is always the full batch; asserted rather than assumed. No `cmds`
                    # exist: the command is produced at every step from the executor's own
                    # forecast, which is the whole point of the arm. The feedback events are
                    # charged one per READ in the step loop, so the read ladder shows up in the
                    # ledger exactly as the reflex's per-step reads do.
                    assert len(need) == Bn, "closed executor requires a whole-batch decision"
                    closed[0] = (d["exec"], k * H)
                    d["exec"].begin(Bn)
                ...
                d2 = {kk: vv for kk, vv in d.items() if kk not in ("cmds", "exec")}
            ...
            for h in range(H):
                ...
                if closed[0] is not None:
                    ex, g_start = closed[0]
                    u_cl, did_read = ex.step((g0 + h) - g_start, obs())
                    a = np.asarray(u_cl, np.float32)
                    if did_read:
                        # every read is a feedback event, including the one at launch, so the
                        # ledger charges the read ladder exactly as it charges the reflex's
                        # per-step reads.
                        led.add(fb=Bn)
                elif is_reflex.any():
                ...
                if issued is not None:
                    # RUBATO: the efference copy, RAW (pre-noise) — what the agent knows it
                    # asked for. Appended AFTER every read of this step, so `issued[i:]` holds
                    # exactly the commands already applied when `obs()` is called.
        ...
        # ACCELERANDO: THE LISTENER'S CLOCK, computed at every cell from the TRUE trajectory.
        lis = listener_err(traj_all, self.wps, k0, Kn, H, int(self.w_listen))
        out = dict(wp_err=wp_err, e_seg=med,
                   e_listen=lis["e_listen"], ep_listen=lis["ep"], w_listen=int(self.w_listen),
...
# =========================================================================== #
# Everything above this line is `prestissimo/world.py` with the realised path
# always recorded. Everything below is new, and none of it is a forward model:
# the listener's grade reads the TRUE trajectory the experimenter already has,
# and the resampler is arithmetic on a stored command tape that reads no state.
# =========================================================================== #


def _boxcar(x, w):
    """Centred boxcar mean of width `w` along axis 1, with EDGE REPLICATION.

    Deterministic, arm-neutral and applied identically to the executed path and to the
    reference, which is the only property the grade needs. Accumulated in float64 so the
    prefix-sum is exact at these lengths.
    """
    w = int(max(1, w))
    if w == 1:
    ...
    pl = (w - 1) // 2
    pr = w - 1 - pl
    xp = np.concatenate([np.repeat(x[:, :1], pl, 1), x, np.repeat(x[:, -1:], pr, 1)], 1)
    c = np.concatenate([np.zeros_like(xp[:, :1]), np.cumsum(xp, axis=1)], 1)
    return (c[:, w:] - c[:, :-w]) / float(w)


def schedule_ref(wps, K, H):
    """The piece's own timed reference path: the target `reflex_cmd` tracks, one point per
    control step. `tgt(k, h) = wp_k + (h+1)/H * (wp_{k+1} - wp_k)`, which is the position the
    schedule asks for at the step whose realised state `traj` records."""
    w = np.asarray(wps, np.float64)
    out = np.empty((int(K) * int(H), 2), np.float64)
    for k in range(int(K)):
        f = (np.arange(int(H), dtype=np.float64) + 1.0) / float(H)
        out[k * H:(k + 1) * H] = w[k][None, :] + f[:, None] * (w[k + 1] - w[k])[None, :]
    return out


def listener_err(traj, wps, k0, K, H, w):
    """THE LISTENER'S-CLOCK GRADE, pre-fixed with the per-waypoint one and reported beside it.

    Both the executed path and the schedule's reference are low-passed with the SAME boxcar of
    fixed WALL-CLOCK width `w` control steps (accelerando's `W_LISTEN` = 16 = 384 ms = one slow
    note), then compared at every drilled control step. The window is a constant of the room, so
    at H = 16 it is one note and the grade is a mildly smoothed tracking error, while at H = 2 it
    spans eight notes and forgives within-phrase wiggle and timing slop while still requiring the
    phrase to be in the right place.

    WHY A SLIDING LOW-PASS RATHER THAN WINDOW ENDPOINTS. The figure is CLOSED, so a grade that
    samples only at window boundaries collapses at the fastest tempo (one window, whose endpoint
    is the start vertex — the do-nothing policy would score ~0). Low-passing both signals keeps
    the sample count and the do-nothing floor at every tempo; both floors are reported.

    Returns the median-over-performers-then-mean-over-steps scalar (mirroring `e_piece`'s
    median-then-mean) and the per-performer mean (mirroring `ep`, for pass fractions).
    """
    a = np.asarray(traj, np.float64)[:, int(k0) * int(H):, :]
    ref = schedule_ref(wps, int(K), int(H))[int(k0) * int(H):]
    b = np.broadcast_to(ref[None, :, :], a.shape)
    e = np.linalg.norm(_boxcar(a, w) - _boxcar(b, w), axis=2)
    return dict(e_listen=float(np.mean(np.median(e, 0))),
                ep=np.asarray(e.mean(1), np.float64),
                per_step=np.asarray(np.median(e, 0), np.float64))


# --------------------------------------------------------------------------- #
# the tempo resampler: what a TAPE is worth at a tempo it was not cut at
# --------------------------------------------------------------------------- #

def resample_cmds(cmds, h_src, h_dst, p=0.0):
    """Time-resample a command tape NOTE BY NOTE from `h_src` steps per note to `h_dst`.

    THE CHEAPEST POSSIBLE PROGRAM, and the control the `prog` head has to beat. Linear
    interpolation at CENTRE-ALIGNED sample positions (`x = (j + 0.5) * h_src/h_dst - 0.5`,
    clamped), so at `h_src == h_dst` and `p = 0` it is EXACTLY the identity — which gate P-R
    checks by rebuilding the source tempo's own library through the resampler.

    `p` is the amplitude exponent on the speed ratio `s = h_src / h_dst`:
      * `p = 0`  — play the tape faster, unchanged. What "the same commands, sooner" means.
      * `p = 2`  — multiply by `s**2`, the dimensional-analysis correction for an INERTIAL body
        (the same path in 1/s the time needs s^2 the acceleration). On a body with drag the
        exponent that holds a SPEED is 1 and the one that buys an ACCELERATION is 2, and a real
        traversal needs both, so neither exponent is right; that is the measured question, and
        both are run because both are free.
    Nothing here reads a state, so it is not an inverse model of the plant.
    """
    c = np.asarray(cmds, np.float32)
    hs, hd = int(h_src), int(h_dst)
    n_notes = c.shape[-2] // hs
    assert n_notes * hs == c.shape[-2], "tape length is not a whole number of notes"
    lead = c.shape[:-2]
    cn = c.reshape(lead + (n_notes, hs, 2)).astype(np.float64)
    x = (np.arange(hd, dtype=np.float64) + 0.5) * (hs / float(hd)) - 0.5
    x = np.clip(x, 0.0, hs - 1.0)
    i0 = np.floor(x).astype(int)
    i1 = np.minimum(i0 + 1, hs - 1)
    fr = (x - i0)[None, :, None]
    out = cn[..., i0, :] * (1.0 - fr) + cn[..., i1, :] * fr
    g = (hs / float(hd)) ** float(p)
    out = np.clip(out * g, -1.0, 1.0)
    return out.reshape(lead + (n_notes * hd, 2)).astype(np.float32)


def resample_library(src, h_src, h_dst, p=0.0):
    """Rebuild a `LevelLibrary` at a new tempo by resampling every member's tape note-wise.

    The slot structure — cells, ids, parents, poison twins — is carried over UNCHANGED, so the
    scaled library is the same vocabulary asked for at a different tempo. Because the resampling
    is note-wise, a level-l member's resampled tape is EXACTLY the weld of its parents' resampled
    tapes, so gate P-N holds on the output as it does on the input.

    Keys are the SOURCE tempo's launch states, carried verbatim: a recording cut at one tempo
    knows nothing about the hand-over distribution at another, and pretending otherwise would be
    the scaled arm quietly consuming information it has not paid for. `key` mode therefore
    selects on stale keys and `audit` mode on the plant, and both are reported.
    """
    dst = LevelLibrary(src.K, src.n_slot, src.n_levels, poison=src.layout.poison)
    for (ell, k) in sorted(src.layout.off):
        for sl in src.slots(ell, k, with_poison=True):
            mem = [dict(cmds=resample_cmds(m["cmds"], h_src, h_dst, p), key=m["key"],
                        traj=None, score=m["score"], spell=m["spell"]) for m in sl["members"]]
            dst.add_slot(ell, k, mem, sl["key"], parents=sl["parents"],
                         j=(None if sl["poison"] else sl["j"]), poison=sl["poison"],
                         meta=dict(sl["meta"], resampled_from=int(h_src), to=int(h_dst),
                                   amp_exponent=float(p),
                                   note=("scores and keys are the SOURCE tempo's and are not "
                                         "valid at the destination tempo")))
    return dst
...
#                          WHAT RUBATO ADDS                                   #
# =========================================================================== #
# Everything above this line is `accelerando/world.py` with two inert switches
# (`obs_predict` / `fm_step`) and one additive grounding (`rollout_path`).
# Everything below is new. The rule this node lifts, and the ONE way it lifts
# it: a body model (inverse and forward) may sit in the EXECUTOR, on the
# learner's side of the meter, converting a kinematic program into commands.
# It may NOT be handed to the incumbent as a planner — that is `offbook`'s
# `accompanist` confound. The reflex stays model-free and re-fit per tempo;
# `reflex_ec` is the one reported reference row where the incumbent gets the
# same model, and it is never headlined.
# =========================================================================== #


# --------------------------------------------------------------------------- #
# the body, as two constants and one discretisation
# --------------------------------------------------------------------------- #

def body_consts(pc):
    """The five numbers every executor in this node is built from, all read off the piece.

    `alpha = exp(-dt_ctrl / tau)` is the ZERO-ORDER-HOLD pole of the free-flight plant
    `m v' = gear u - c v` under a command held constant across one control step. On this body
    `dt_ctrl / tau = 0.8`, so alpha = 0.449: the control step is NOT small against the body, and
    a first-order (continuous-time) inverse is a different function from the exact one — by
    `(dt/tau) / (1 - alpha) = 1.45` on the acceleration term. That factor is why the derivative
    scheme is a measurement here and not a formality.
    """
    dt = float(pc.dt_ctrl)
    tau = float(pc.tau)
    return dict(dt=dt, tau=tau, alpha=float(np.exp(-dt / tau)),
                v_term=float(pc.v_terminal), accel=float(pc.accel),
                mass=float(pc.mass), gear=float(pc.gear), damping=float(pc.damping))


def fwd_zoh(bc, states, u):
    """THE FORWARD STEP of the free-flight body, exact under zero-order hold.

        v' = alpha v + (1 - alpha) v_term u
        x' = x + tau (1 - alpha) v + v_term (dt - tau (1 - alpha)) u

    (integrate `v(t) = v_term u + (v0 - v_term u) e^{-t/tau}` over the step). It knows the BODY
    and nothing about the world: the command-rotation region is not in it, exactly as the reflex
    law's own model of the plant is not, and `acappella`'s standing choice to leave the hard
    passage UNMODELLED rather than wrongly modelled is what that inherits.
    """
    st = np.asarray(states, np.float64)
    u = np.clip(np.asarray(u, np.float64), -1.0, 1.0)
    a, vt, tau, dt = bc["alpha"], bc["v_term"], bc["tau"], bc["dt"]
    v0 = st[..., 2:]
    v1 = a * v0 + (1.0 - a) * vt * u
    x1 = st[..., :2] + tau * (1.0 - a) * v0 + vt * (dt - tau * (1.0 - a)) * u
    return np.concatenate([x1, v1], axis=-1).astype(np.float32)


def inv_zoh(bc, v, v_next):
    """THE EXACT INVERSE of `fwd_zoh`'s velocity channel: the command that takes `v` to `v_next`
    in one control step. `u = (v_next - alpha v) / ((1 - alpha) v_term)`, clipped to [-1, 1].

    THE SCHEME OF RECORD. It inverts the discretisation the plant actually runs under, so on the
    free-flight body it is exact by construction and the only residual is the integrator's
    (RK4 at 0.002 s on a 0.03 s time constant) plus whatever the world adds that the body model
    does not contain — the rotation region and the walls. Gate P-K measures exactly that residual.
    """
    a, vt = bc["alpha"], bc["v_term"]
    u = (np.asarray(v_next, np.float64) - a * np.asarray(v, np.float64)) / ((1.0 - a) * vt)
    return np.clip(u, -1.0, 1.0)


def inv_ct(bc, v, v_next):
    """THE CONTINUOUS-TIME INVERSE the brief names: `u = (m a + c v) / gear` with
    `a = (v_next - v) / dt` and `v` read at the step's own start.

    Algebraically `u = a / A + v / v_term` against `zoh`'s `u = a dt / ((1-alpha) v_term) +
    v / v_term`: the two schemes agree exactly on the DRAG term and differ only in the
    coefficient on acceleration, by `(dt/tau) / (1 - alpha)`. Both are free, so which one the
    body wants is measured. Reported at every cell beside `zoh`, never headlined.
    """
    dt, vt, A = bc["dt"], bc["v_term"], bc["accel"]
    acc = (np.asarray(v_next, np.float64) - np.asarray(v, np.float64)) / dt
    return np.clip(acc / A + np.asarray(v, np.float64) / vt, -1.0, 1.0)


INV_SCHEMES = {"zoh": inv_zoh, "ct": inv_ct}


# --------------------------------------------------------------------------- #
# the kinematic unit: a path against phase, with the clock outside it
# --------------------------------------------------------------------------- #

def lp_path(path, w):
    """Low-pass a realised path on ITS OWN (source) grid with a centred boxcar of `w` samples and
    edge replication — `_boxcar`, the listener's-grade kernel, reused so the node has one filter.

    Position and velocity are filtered with the SAME kernel. A boxcar is linear and
    time-invariant, so filtering `x` and filtering `v = dx/dt` with one kernel keeps the pair
    consistent; the two channels are not independently smoothed into disagreement.

    WHY THE SOURCE GRID. Anti-aliasing has to happen before decimation, not after, and "one note"
    is `H_src` samples on the source grid whatever the destination tempo is — so both widths this
    node uses are source-grid widths and the operation has one home. Edge replication moves the
    unit's own endpoints, which is inherent to the treatment and is stated wherever it is used.
    """
    w = int(max(1, w))
    if w == 1:
        return np.asarray(path, np.float64)
    a = np.asarray(path, np.float64)
    lead = a.shape[:-2]
    flat = a.reshape((-1,) + a.shape[-2:]) if lead else a[None]
    out = np.stack([_boxcar(flat[..., c], w) for c in range(flat.shape[-1])], axis=-1)
    return out.reshape(a.shape)


def lp_width(kind, h_src, h_dst):
    """The two pre-fixed filter widths, in SOURCE samples.

      `aa`   — `round(h_src / h_dst)`, one DESTINATION control step. The anti-alias width the
               decimation itself requires: without it, phase resampling from 32 to 2 steps a note
               is a 16x decimation by point sampling, which aliases whatever the source path
               carries above the destination's Nyquist into the destination band. It is an
               APPARATUS check on the resampler, not a treatment.
      `note` — `h_src`, one NOTE. The treatment: it removes every feature finer than a note from
               the stored path, i.e. exactly the structure a destination tempo with few steps per
               note cannot express.
      `None` — width 1, the identity, which is what every arm before this round ran.
    """
    if kind in (None, "none"):
        return 1
    if kind == "aa":
        return int(max(1, round(float(h_src) / float(h_dst))))
    if kind == "note":
        return int(h_src)
    raise KeyError(kind)


def resample_path(path, n_dst, s):
    """Re-express a realised path on a DESTINATION step grid, with the tempo taken out of it.

    `path` is `(..., N_src + 1, 4)` — position and velocity at every control step of the unit's
    span, INCLUDING its launch. Phase is `phi_j = j / N_src`, so the path is a curve in phi and
    carries no tempo. On the destination grid `phi_n = n / n_dst` and

        x_n = X(phi_n)                 the same curve, sampled elsewhere
        v_n = s * V(phi_n)             s = N_src / n_dst = H_src / H_dst

    because `v = dx/dt = (dx/dphi) / T_span` and only `T_span` moved. Linear interpolation in
    phi; at `n_dst == N_src` (s = 1) the sample points are the source's own integers and this is
    EXACTLY the identity, which gate P-K checks.

    NOTE ON THE SCHEME, since the brief asks for it to be stated. The derivative that reaches the
    executor is never taken by differentiating an interpolant: the stored object is the pair
    (x, v) the body itself realised, so `v` is measured and not differenced, and the only
    numerical operation is interpolation of a measured curve. The acceleration the executor needs
    is then the destination grid's own one-step difference of the interpolated velocity, which is
    the quantity the ZOH inverse is exact for and the quantity a learned model fitted on
    (v_n, (v_{n+1} - v_n)/dt, u_n) triples was fitted on. Differentiating the interpolant instead
    would supply a continuous-time acceleration the discrete plant never sees.
    """
    pth = np.asarray(path, np.float64)
    n_src = pth.shape[-2] - 1
    phi = np.arange(int(n_dst) + 1, dtype=np.float64) / float(n_dst)
    j = phi * n_src
    j0 = np.clip(np.floor(j).astype(int), 0, n_src)
    j1 = np.minimum(j0 + 1, n_src)
    fr = (j - j0)[..., None]
    lo, hi = pth[..., j0, :], pth[..., j1, :]
    out = lo * (1.0 - fr) + hi * fr
    out[..., 2:] *= float(s)
...
def kin_cmds(bc, path, n_dst, s, scheme="zoh", lp=1):
    """THE EXECUTOR, feedforward: a stored path -> a command tape at the destination tempo.

    One command per destination control step, computed from the resampled velocity samples alone:
    `u_n = inv(v_n, v_{n+1})`. Position never enters the feedforward term — on this plant the
    velocity channel is autonomous (`m v' = gear u - c v` has no x in it) and position is its
    integral, so a feedforward executor that tracks the velocity curve exactly reproduces the
    position curve up to the launch offset it was handed. Correcting that offset is what a
    forecast and a PD term are for, and they are Phase 3's business, not the ceiling's.

    Returns `(cmds (..., n_dst, 2), sat)` where `sat` is the fraction of scalar command
    components the clip actually bound — the ceiling of the factored architecture is an actuator
    ceiling before it is anything else, and `accelerando`'s own tempo calibration found
    feasibility binding at 48 ms notes.
    """
    rp = resample_path(lp_path(path, lp), n_dst, s)
    v = rp[..., :-1, 2:]
    vn = rp[..., 1:, 2:]
    # `scheme` is a key into the two analytic oracles, or ANY callable with `inv_zoh`'s
    # signature — which is how Phase 2's FITTED inverse enters without the executor knowing
    # whether it is holding an oracle or a model.
    inv = INV_SCHEMES[scheme] if isinstance(scheme, str) else scheme
    raw = inv(bc, v, vn)
    u = np.clip(raw, -1.0, 1.0)
    return u.astype(np.float32), float(np.mean(np.abs(raw) > 1.0)), rp


def kin_library(src, paths, bc_dst, span_notes_of, h_src, h_dst, scheme="zoh", lp_kind=None):
    """Rebuild a `LevelLibrary` at a new tempo from its members' MEASURED PATHS.

    `paths[(level, seam, j, mi)]` is the `(N_src + 1, 4)` realised path of that member's own tape,
    executed from the launch it was cut at. `span_notes_of(level)` is the unit's span in notes.

    The slot structure — cells, ids, parents, poison twins, keys, scores — is carried over
    UNCHANGED, exactly as `resample_library` does, so a kinematic arm is the SAME VOCABULARY
    asked for at a different tempo and `LadderDecider` plays it with no changes at all. That is
    what makes the comparison against `rec`, `scl0` and `scl2` a one-variable comparison: only
    the content in the members changed.

    Keys are the SOURCE tempo's launch states, carried verbatim and NOT re-fit — a path cut at
    one tempo knows nothing about the hand-over distribution at another, and pretending otherwise
    would be the kinematic arm quietly consuming information it has not paid for. `key` mode
    therefore selects on stale keys and `audit` mode on the plant, and both are reported.
    """
    dst = LevelLibrary(src.K, src.n_slot, src.n_levels, poison=src.layout.poison)
    sat_rows = []
    sp_ratio = float(h_src) / float(h_dst)
    w_lp = lp_width(lp_kind, h_src, h_dst)
    for (ell, k) in sorted(src.layout.off):
        for sl in src.slots(ell, k, with_poison=True):
            mem = []
            for mi, m in enumerate(sl["members"]):
                pth = paths[(int(ell), int(k), int(sl["j"]), int(mi))]
                n_dst = int(span_notes_of(ell)) * int(h_dst)
                cm, sat, _ = kin_cmds(bc_dst, pth, n_dst, sp_ratio, scheme=scheme, lp=w_lp)
                sat_rows.append(dict(level=int(ell), seam=int(k), j=int(sl["j"]), m=int(mi),
                                     sat=sat, poison=bool(sl["poison"])))
                mem.append(dict(cmds=cm, key=m["key"], traj=None, score=m["score"],
                                spell=m["spell"]))
            dst.add_slot(ell, k, mem, sl["key"], parents=sl["parents"],
                         j=(None if sl["poison"] else sl["j"]), poison=sl["poison"],
                         meta=dict(sl["meta"], kin_from=int(h_src), to=int(h_dst),
                                   scheme=str(scheme), lp_kind=str(lp_kind),
                                   lp_width_src=int(w_lp),
                                   note=("content is the MEASURED PATH of this member's own "
                                         "tape, converted by inverse dynamics at the "
                                         "destination tempo; scores and keys are the SOURCE "
                                         "tempo's and are not valid at the destination")))
    return dst, sat_rows


def measure_paths(W, lib, led=None, charge=True):
    """Execute every member of `lib` from ITS OWN launch key and record the realised path.

    One batched grounding per cell. The launch is the member's stored `key`, which for a level-1
    member is the seam state the rendition was harvested from and for a welded level-l member is
    its FIRST parent's member key — so a level-l unit's path is what the WELD actually does, not
    the concatenation of two paths taken from two different launches. That distinction is real
    and is why the paths are re-measured rather than read off the harvest's `traj`: `build_level_
    cell` stores the parents' original trajectories, and the weld's second half does not follow
    them.

    Returns `{(level, seam, j, member): (L+1, 4)}` in float64.
    """
    out = {}
    for (ell, k) in sorted(lib.layout.off):
        for sl in lib.slots(ell, k, with_poison=True):
            if not sl["members"]:
                continue
            cm = np.stack([m["cmds"] for m in sl["members"]]).astype(np.float32)
            s0 = np.stack([np.asarray(m["key"], np.float32) for m in sl["members"]])
            st = W.rollout_path(s0, cm, led=led, charge=charge)
            for mi in range(len(sl["members"])):
                out[(int(ell), int(k), int(sl["j"]), int(mi))] = np.asarray(st[mi], np.float64)
    return out


# --------------------------------------------------------------------------- #
# the two diagnostics: is the path representable, and is the launch the same one
# --------------------------------------------------------------------------- #

def shape_distance(src_paths, dst_paths, lib, span_notes_of, h_src, h_dst, lp_kind=None):
    """(1) PATH REPRESENTABILITY. How far is the slow path, scaled to the target tempo, from a
    path the target tempo's own body actually produced?

    For each cell, every SOURCE member's path is resampled onto the destination grid and compared
    with every DESTINATION member's realised path, POSITION ONLY, on phase, after subtracting each
    path's own launch position — so the statistic is shape, not placement. Two numbers per pair
    set are kept: the NEAREST destination path (is there anything like this here at all?) and the
    MEAN over all of them (so the nearest is not a cherry-pick).

    The scale is supplied by the destination library against ITSELF: `d_within` is each
    destination member's distance to the nearest OTHER destination member in the same cell. A
    cross distance of `d` means nothing until it is read against the spread the destination's own
    content already has, and `d_cross / d_within` is the scale-free form.

    Returns per-(level, seam) rows in metres.
    """
    rows = []
    for (ell, k) in sorted(lib.layout.off):
        sls = lib.slots(ell, k, with_poison=False)
        if not sls:
            continue
        n_dst = int(span_notes_of(ell)) * int(h_dst)
        w_lp = lp_width(lp_kind, h_src, h_dst)
        ...
        for sl in sls:
            for mi in range(len(sl["members"])):
                key = (int(ell), int(k), int(sl["j"]), int(mi))
                if key in src_paths:
                    rp = resample_path(lp_path(src_paths[key], w_lp), n_dst,
                                       float(h_src) / float(h_dst))
                    A.append(rp[:, :2] - rp[0, :2])
                if key in dst_paths:
                    q = np.asarray(dst_paths[key], np.float64)
                    B.append(q[:, :2] - q[0, :2])
        if not A or not B:
            continue
        A = np.stack(A); B = np.stack(B)
        # (nA, nB) mean-over-phase euclidean distance between launch-aligned position curves
        D = np.linalg.norm(A[:, None, :, :] - B[None, :, :, :], axis=3).mean(2)
        W = np.linalg.norm(B[:, None, :, :] - B[None, :, :, :], axis=3).mean(2)
        np.fill_diagonal(W, np.inf)
        rows.append(dict(level=int(ell), seam=int(k), n_src=len(A), n_dst=len(B),
                         d_near=float(D.min(1).mean()), d_mean=float(D.mean()),
                         d_within=(float(W.min(1).mean()) if len(B) > 1 else float("nan"))))
    return rows


def launch_rows(W, lib, kin_lib, src_paths, span_notes_of, h_src, h_dst, hold, bc,
                lp_kind=None, led=None):
    """(2) LAUNCH MISMATCH. Is the unit failing because the path is wrong, or because it is
    launched from somewhere the path never starts?

    Two things per cell, both measured, neither predicted:
      * the SCALED INITIAL STATE of the stored unit — the source path's own first sample with its
        velocity multiplied by `s` — logged beside the ACTUAL hand-over states the target tempo
        delivers at that seam. `dx` and `dv` are the gaps; `dv_tau` is `|dv| * tau`, the position
        error a velocity mismatch leaves behind on a body that forgets velocity in one tau, which
        is the quantity that decides whether a launch gap can matter at all against the band.
      * the unit's converted commands EXECUTED from BOTH launches — from the actual hand-over
        (what the sweep does) and from the path's own scaled initial state (the diagnostic). Mean
        waypoint error over the unit's own span. At the top rung the span is the whole drilled
        figure and this is exactly `e_piece`.

    Batched one call per cell per launch, because 360 members x 4 tempi of single-state dispatches
    is pool overhead and nothing else. Not charged: an instrument, not a decision.
    """
    rows = []
    tau = bc["tau"]
    sc = float(h_src) / float(h_dst)
    w_lp = lp_width(lp_kind, h_src, h_dst)
    for (ell, k) in sorted(lib.layout.off):
        sls = kin_lib.slots(ell, k, with_poison=False)
        if not sls:
            continue
        S = np.asarray(hold[int(k)], np.float32)
        if not len(S):
            continue
        spn = int(span_notes_of(ell))
        n_dst = spn * int(h_dst)
        kw = int(W.k0) + int(k)
        cms, inits, ids = [], [], []
        for sl in sls:
            for mi, m in enumerate(sl["members"]):
                key = (int(ell), int(k), int(sl["j"]), int(mi))
                if key not in src_paths:
                    continue
                rp = resample_path(lp_path(src_paths[key], w_lp), n_dst, sc)
                inits.append(np.asarray(rp[0], np.float32))
                cms.append(np.asarray(m["cmds"], np.float32))
                ids.append((int(sl["j"]), int(mi)))
        if not cms:
            continue
        cm = np.stack(cms)
        s_init = np.stack(inits)
        nm, ns = len(cm), len(S)
        e_own, _ = W.rollout(s_init, cm, kw, spn, led, charge=False)          # (nm, spn)
        e_act, _ = W.rollout(np.tile(S[None], (nm, 1, 1)).reshape(nm * ns, 4),
                             np.repeat(cm, ns, axis=0), kw, spn, led, charge=False)
        e_act = e_act.mean(1).reshape(nm, ns)
        for i, (j, mi) in enumerate(ids):
            dx = np.linalg.norm(S[:, :2] - s_init[i, :2][None], axis=1)
            dv = np.linalg.norm(S[:, 2:] - s_init[i, 2:][None], axis=1)
            rows.append(dict(
                level=int(ell), seam=int(k), j=int(j), m=int(mi),
                e_from_own_launch=float(e_own[i].mean()),
                e_from_actual=float(e_act[i].mean()),
                dx=float(dx.mean()), dv=float(dv.mean()), dv_tau=float(dv.mean() * tau),
                x0=[float(v) for v in s_init[i, :2]], v0=[float(v) for v in s_init[i, 2:]],
                x_act=[float(v) for v in S[:, :2].mean(0)],
                v_act=[float(v) for v in S[:, 2:].mean(0)]))
    return rows


# =========================================================================== #
#              PHASE 3 — the CORRECTED executor and its read ladder           #
# =========================================================================== #

def kin_targets(src, paths, span_notes_of, h_src, h_dst, lp_kind=None):
    """The TARGET PATH each kinematic member is trying to follow at the destination tempo:
    `resample_path` of the stored path, on exactly the grid `kin_library` converts on and with
    exactly the same low-pass. Feedforward reads its velocity channel; the correction reads both.

    Returned per member so a per-performer decider can stack the ones it chose.
    """
    out = {}
    w_lp = lp_width(lp_kind, h_src, h_dst)
    for (ell, k) in sorted(src.layout.off):
        for sl in src.slots(ell, k, with_poison=True):
            for mi in range(len(sl["members"])):
                ...
                if key not in paths:
                    continue
                n_dst = int(span_notes_of(ell)) * int(h_dst)
                out[key] = resample_path(lp_path(paths[key], w_lp), n_dst,
                                         float(h_src) / float(h_dst))
    return out


class KinExecutor:
    """THE CORRECTED EXECUTOR: feedforward from the kinematic program, corrected online against a
    FORECAST of where the body is.

    Per control step `h` within the unit's span, for a batch of performers:
    ...
    `R` is the READ LADDER (decision 18): R = 1 reads every step and is the row directly
    comparable with `reflex_ec`; R >= span reads only at launch and is the strongest cerebellar
    claim; the ladder between them puts the level's economy on the meter's own coordinate.

    THE EFFERENCE BUFFER STARTS EMPTY AT THE UNIT'S LAUNCH. For the first `Delta` steps of a span
    the copy has fewer than `Delta` commands to roll forward, because the commands issued before
    the launch belong to the approach or to the previous unit and this executor did not issue
    them. That is the honest boundary condition — it is the learner's own copy — and it is the
    same disadvantage every other arm faces at a launch.

    `eps` perturbs the FEEDFORWARD command only, which is what P-E asks about: the correction is
    then measured on its ability to absorb a command error, not on a corrupted correction.
    """

    def __init__(self, paths, ff, kp, kd, R, D, fm, eps=0.0, rng=None):
        self.path = np.asarray(paths, np.float64)          # (n, N+1, 4)
        self.ff = np.asarray(ff, np.float64)               # (n, N, 2)
    ...
    def begin(self, n):
        assert n == self.path.shape[0], (n, self.path.shape)
        self.sh = None
        self.buf = []
        self.n_read = 0

    def step(self, h, obs_states):
        h = int(h)
        did = (h % self.R == 0)
        if did:
            st = np.asarray(obs_states, np.float64).copy()
            if self.D > 0 and self.buf:
                for u in self.buf[-self.D:]:
                    st = np.asarray(self.fm(st, u), np.float64)
            self.sh = st
            self.n_read += 1
        tgt = self.path[:, min(h, self.path.shape[1] - 1), :]
        u = self.ff[:, min(h, self.ff.shape[1] - 1), :] \
            + self.kp * (tgt[:, :2] - self.sh[:, :2]) \
            + self.kd * (tgt[:, 2:] - self.sh[:, 2:])
        if self.eps > 0.0:
            u = u + self.eps * self.rng.standard_normal(u.shape)
        u = np.clip(u, -1.0, 1.0)
        self.buf.append(u)
        self.sh = np.asarray(self.fm(self.sh, u), np.float64)
        return u.astype(np.float32), bool(did)


class CorrectedDecider:
    """`LadderDecider`'s KEY mode, handing the span to a `KinExecutor` instead of a command tape.

    KEY mode only, and the reason is measured rather than economic: `k1` found `key_kzsa` to BEAT
    `aud_kzsa` at three of the four unpractised tempi (0.0339 vs 0.0426 at H = 16 L2, 0.0498 vs
    0.0550 at H = 8 L4, 0.0486 vs 0.0555 at H = 4 L4), so selecting on the source tempo's frozen
    keys is not a handicap here. It also keeps the arm honest: auditioning a closed-loop policy on
    the plant would let the arm pay groundings the open-loop wedge it is compared against never
    paid.
    """

    def __init__(self, W, lib, targets, kp, kd, R, levels=(1,), obs_delay=0, fm=None,
                 eps=0.0, seed=0):
        self.W, self.lib, self.targets = W, lib, targets
        self.levels = tuple(int(x) for x in levels)
        self.kp, self.kd, self.R = float(kp), float(kd), int(R)
        self.D = int(obs_delay)
        self.fm = fm
        self.eps = float(eps)
    ...
    def __call__(self, k, states, need, rng, led):
        ...
        for ell in sorted(set(self.levels), reverse=True):
            for sl in self.lib.slots(ell, d, with_poison=False):
                legal.append((int(ell), sl))
        if not legal:
            return dict(kind="reflex", span=np.ones(n, int), src="no_content")
        keys = np.array([sl["key"] for _, sl in legal], np.float32)
        sd = keys.std(0) + 1e-6
        pick = np.argmin(np.linalg.norm((states[:, None, :] - keys[None]) / sd[None, None],
                                        axis=2), axis=1)
        P, F, lvl, chosen, mem = [], [], [], [], []
        for i in range(n):
            ...
            mk = np.array([mm["key"] for mm in sl["members"]], np.float32)
            mj = int(np.argmin(np.linalg.norm((states[i][None] - mk) / sd[None], axis=1)))
            key = (int(ell), int(d), int(sl["j"]), int(mj))
            P.append(self.targets[key])
            F.append(sl["members"][mj]["cmds"])
            lvl.append(ell); chosen.append(int(sl["slot"])); mem.append(mj)
        sp = int(LadderLayout.span(lvl[0]))
        ex = KinExecutor(np.stack(P), np.stack(F), self.kp, self.kd, self.R, self.D,
                         self.fm, eps=self.eps, rng=self.rng)
        self._execs.append(ex)
        lv = np.asarray(lvl, int)
        self.picks.append(dict(seam=int(k), drilled=int(d), n_legal=len(legal), n_aud=0.0,
                               slot=chosen, member=mem, level=[int(x) for x in lvl],
                               mean_level=float(lv.mean()),
                               ns=[sp] * n))
        return dict(kind="closed", exec=ex, span=np.full(n, sp, int), src="corrected",
                    n_cand=float(len(legal)), frac_prim=0.0, mean_level=float(lv.mean()))

    def reads(self):
        return sum(e.n_read for e in self._execs)
