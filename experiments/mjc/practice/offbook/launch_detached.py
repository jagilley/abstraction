...
Usage (from experiments/):
    python3 mjc/practice/offbook/launch_detached.py --fn gates   --tag g0 --seed 0 [modal args...]
    python3 mjc/practice/offbook/launch_detached.py --fn offbook --tag O1 --seed 0 [...]

Logs go to **experiments/.launch_logs/offbook_<tag>.log**, deliberately OUTSIDE the `mjc` package.
...
ENTRY = {"gates": ("gates.py", "offbook_gates"), "offbook": ("offbook.py", "offbook")}
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, f"offbook_{tag}.log")
    cmd = ["modal", "run", "--detach", f"mjc/practice/offbook/{mod}::{ep}"] + keep
