...
PRESTO NOTE (2026-08-26): a VERBATIM, byte-identical fork of `offbook/nets.py`. Presto's Phase A
gate uses only `SlotLayout`; the two ports themselves are dormant here and are carried so that a
Phase B port (if the delay gate licenses one) starts from the donor code rather than a rewrite.
--------------------------------------------------------------------------------------------------
