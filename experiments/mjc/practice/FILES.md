...
| `acappella/`[^private] | **Spec only.** The re-port with no forward model anywhere: `etude/`'s substrate and piece, a priced real-rollout incumbent under a declared budget (RHM's grounding economy in the body's currency), a reflex-law reference, plant audition, nested selection, routing π. |
