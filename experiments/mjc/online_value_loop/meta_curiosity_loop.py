"""The two-timescale ONLINE loop: a slow reward-driven outer loop self-tunes the
explore/exploit balance of the intrinsic drive, while the fast inner loop keeps a
LIVE forward model.

Program: `ideas/two_timescale_value_loop.md` — "putting a *value* signal into that
slow loop is the contribution." This is the KEYSTONE the whole arc approached from every
side but never closed: a SINGLE online run where reward continuously modulates the drive.

    WHAT WAS MISSING (the honest ledger, from the idea-doc back-translation).
      * Inner loop (fast, LIVE FM): DONE — `curiosity_control.py`'s RPF ensemble continually
        re-fit on a recency FIFO + `grounded@b` teleport-collection. But its balance `b` was
        HAND-SWEPT (a hyperparameter, not a learned quantity).
      * Efferent reward loop (value SHAPES the FM): DONE but OFFLINE — Cut #4e
        (`meta_value_learn.py`) chose the per-dim FM weight by an outer CEM over
        RETRAINED-from-scratch FMs. "Still a rung short of the fully-online single loop."
      * Afferent reward loop (value DIRECTS collection): the balance was swept, never learned.

    THIS CUT closes the afferent loop ONLINE: one continuous run, one live ensemble, a slow
    (every `outer_m` rounds) reward-driven gradient-bandit that nudges b = sigmoid(theta).
    The reward is ONLY real-env CEM-MPC control performance (-median goal-dist). Nothing tells
    the loop about the noise / the frontier / the sweep. This IS the two-timescale meta-RL
    structure (Wang et al. 2018): a slow scalar reward loop modulating a faster inner loop —
    the dopaminergic set-point analogy, no backprop through the simulator.

TWO CLAIMS.

  CLAIM 1 (reward DISCOVERS the set-point — closes the afferent loop). A 2x2 over
  {noise off, noise on}: the online loop should settle at an INTERMEDIATE b without noise
  (the mild inverted-U the sweep found) and DROP b toward grounded under the noisy TV
  (reward feels the seduction: pure-explore chases aleatoric noise -> control degrades ->
  the loop pulls b down). It matches/beats the best FIXED b (recovering the swept optimum
  with no sweep) and decisively beats pure-explore b=1 under noise. Reward reconstructs the
  grounding — the afferent analog of #4e's efferent "reward rediscovers V's support".

  CLAIM 2 (online TRACKS a MOVING optimum — the meta-layer/compounding signature a sweep
  CANNOT have). `--noise-onset 0.4`: the noise appears PARTWAY through, so the OPTIMAL b is
  itself NON-STATIONARY (intermediate -> grounded). The online loop rides it (b high-ish
  early, drops when noise appears); every FIXED b must compromise and is beaten over the full
  run. This is "value compounds only on a moving frontier" lifted to the META level: the drive
  SET-POINT is the moving target, tracked by reward. A swept scalar b structurally cannot do
  this — the reason the loop must be online.

INNER LOOP = `curiosity_control.py`, reused near-verbatim (self-contained duplication, the
established pattern here — cf. directed_readapt/meta_adapt): RPF ensemble (Osband 2018) on a
recency FIFO; teleport-region collection over a GxG grid scored by `grounded@b` =
b*reducible + (1-b)*exploit (reducible = error x disagreement = the e-tap; exploit =
start->goal path density = the p-tap); CEM-MPC control readout; unbiased frontier-tracking err.
ONLY the source of b differs across arms (the control-variable discipline): `online` learns it;
`b<val>` fixes it (the reference lines the loop should match/beat); `random` is the floor.

WIREHEADING (the non-optional control, #4e's discipline). Reward is grounded in REAL env
rollouts (not a learned value the loop can game) and b in (0,1) is a bounded, on-manifold knob
with NO loss-scale DOF (unlike #4e's per-dim weight, which needed a fixed budget) — so there is
structurally nothing to wirehead. Stated, not separately tested.
...
def run_meta_curiosity_loop(cfg: dict) -> dict:
    import os
    import numpy as np
    import torch
    ...
    def run_arm(arm):
        ...
        # ---- outer-loop (slow) state for the `online` arm ----
        theta = math.log(cfg["b_init"] / (1 - cfg["b_init"]))    # b_init -> theta
        cur_eps = 0.0
        cur_b = cfg["b_init"] if arm == "online" else (fixed_b if fixed_b is not None else 0.5)
        ...
        for rnd in range(cfg["rounds"]):
            ...
            # --- epoch boundary: control eval = REWARD; update the outer loop (online) ---
            if is_epoch_end:
                cdist = eval_control(fms, norm, rnd, cfg["seed"] + 7000 + rnd)
                fr_err = float(score_error(fms, rnd)[fc])
                recs.append({"round": rnd, "epoch": epoch, "control_dist": cdist,
                             "frontier_err": fr_err, "b": b_val, "noise": noise_on(rnd)})
                occ_hist.append(occ.copy())

                if arm == "online":
                    R = -cdist                                   # reward: higher = better control
                    adv = 0.0 if base is None else (R - base)
                    base = R if base is None else cfg["outer_rho"] * base + (1 - cfg["outer_rho"]) * R
                    # lazy-init the advantage-normalizer on the RIGHT scale (advantages are ~0.05,
                    # so a fixed run_sq=1 would kill adv_n for many epochs) — prime it during warmup.
                    if adv != 0.0:
                    ...
                    outer_hist.append({"epoch": epoch, "theta": float(theta), "b_used": float(b_r),
                                       "eps": float(cur_eps), "reward": float(R),
                                       "adv": float(adv), "b_theta": float(sigmoid(theta))})
                    print(f"  [online e{epoch:2d} r{rnd:3d}] b={b_r:.3f} theta={theta:+.2f} "
                          f"ctrl={cdist:.4f} R={R:+.4f} adv={adv:+.4f} noise={noise_on(rnd)}", flush=True)
                else:
                    print(f"  [{arm:7s} r{rnd:3d}] b={float(b_r) if fixed_b is not None else float('nan'):.3f} "
                          f"ctrl={cdist:.4f} front={fr_err:.4f}", flush=True)
    ...
    summary = {a: {"final_control": final_ctrl(a), "final_frontier_err": final_front(a),
               ...
               for a in arms}
    print("\n[summary] final control (lower=better) | frontier-err | learned-b | patch-frac | noise-frac", flush=True)
    for a in arms:
        s = summary[a]
        print(f"  {a:8s} ctrl={s['final_control']:.4f}  front={s['final_frontier_err']:.4f}  "
              f"b={s['final_b']:.3f}  patch={s['patch_frac_last']:.2f}  noise={s['noise_frac_last']:.2f}", flush=True)
    if "online" in arms:
        for base_a in ["b1.0", "b0.0", "b0.5", "random"]:
            if base_a in arms:
                print(f"  GAP online−{base_a}: control "
                      f"{summary[base_a]['final_control']-summary['online']['final_control']:+.4f} "
    ...
    outdir = os.path.join(DATA_DIR, "meta_curiosity_loop", cfg["tag"])
...
def _make_figures(R):
    ...
    COL = {"online": "#e8590c", "b1.0": "#0ca678", "b0.5": "#2f9e44", "b0.0": "#3d6fd1",
           "random": "#868e96"}
    ...
    # ---- fig1: the learned-b trajectory (the headline) ----
    fig, ax = plt.subplots(figsize=(7.8, 4.4))
    if "online" in R["results"] and R["results"]["online"]["outer_hist"]:
        oh = R["results"]["online"]["outer_hist"]
        ep = [h["epoch"] for h in oh]
        ax.plot(ep, [h["b_theta"] for h in oh], "o-", color=COL["online"], lw=2.4,
                label="online: b=σ(θ) (learned set-point)")
        ax.plot(ep, [h["b_used"] for h in oh], "x", color=COL["online"], alpha=0.4, ms=6,
                label="online: b sampled (exploration)")
    for a in arms:
        fb = _parse_fixed_b(a)
        if fb is not None:
    ...
    ttl = "reward self-tunes the drive set-point"
    if R["noise"]:
        ttl += "  [noise on → b should DROP toward grounded]"
    ax.set_title(ttl, fontsize=10); ax.legend(fontsize=7.5, loc="best")
    ...
    show = [a for a in ["online", "b1.0", "b0.0", "random"] if a in arms]
    n = len(show)
    ...
    for ax, a in zip(axes, show):
        ...
        lbl = a + (f" (b→{R['results'][a]['final_b']:.2f})" if a == "online" else "")
...
def meta_curiosity_loop(
    ...
    if quick:
        arm_list = arms.split(",") if arms else ["online", "b1.0", "b0.0"]
    ...
    out = run_meta_curiosity_loop.remote(cfg)
    localdir = os.path.join(os.path.dirname(__file__), "figures", "meta_curiosity_loop_" + tag)
