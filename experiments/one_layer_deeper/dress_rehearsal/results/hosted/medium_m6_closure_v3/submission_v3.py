...
Everything else about the prompt is read by **learned attention pools** — no field is
located by hard-coded id or position, which is why the cycle term's re-encode goes through
a learned `ReEncoder` grounded on the encoder's own output rather than through
`ballistic_depth`'s trick of writing soft digits into the prompt's `x` slot.
