...
| child | summary |
|---|---|
| [`horizon_convergence/`](horizon_convergence/README.md) | **Is the composition horizon a property of the substrate or of the budget?** No code of its own — re-reads existing tags. At `N=893` the horizon is **converged**: 2.5× the gradient steps (`iso_compute` 20k vs `deep60`/`cut1` 8k, 3 seeds each) leaves it at 13 and mildly *lowers* depth accuracy, which is what makes §1–§9's numbers safe. At `N=9853` it is **not**: 2× the budget (120k vs 60k) takes exact-match at T=7 from **0.176 to 0.996** and the horizon 7 → 10, still climbing, with in-distribution accuracy already 1.000 in *both* runs 60k steps apart. So **ID saturation does not signal that the horizon has converged**, and the budget at which it does scales with the state set. Invalidates §10's earlier "horizon halves 13 → 7, independent of capacity" — the capacity control was sound, the convergence inference was not — and makes every `N=9853` horizon a lower bound. Single seed per budget on the `N=9853` side, `base` arm only, and its 120k run was killed after `base` completed so those numbers come from the run log plus a saved checkpoint. |
| [`rule_structure/`](rule_structure/README.md) ([FILES](rule_structure/FILES.md)) | **Is the representation organised by the group, or is it a lookup table?** Pure re-analysis of saved checkpoints in the CRT coordinate `(Z/N)* ≅ Z/(p−1) × Z/(q−1)`, where squaring is exactly the doubling map and the image size is the `reachable_states` count the task already reports. Six trained encoders across 2 moduli, an 11.6× state range and a 4× parameter range show **no group organisation beyond a permutation null**, all sitting below the weakest synthetic group signal the same instrument still detects (translation R² 0.03–0.04 against 0.294). On the QR sublattice the rollout actually lives on, z ≈ 0 or negative in every arm. Scaling the state set 11.6× moved structure **down** (R² 0.040 → 0.007 = null) on an ID-perfect model, which is the opposite of what "lookup is merely cheaper" predicts. The cycle term's held-out-`x` gain (0.001 → 0.32, re-derived here as 0.349) is real and **not** group-based; a single linear `Enc(x²) ≈ M·Enc(x)` does not fit (held-out decode ≤ 0.084), killing that as a next training term. The instrument's untrained-encoder control also detects representational collapse, which caught §10's collapsed arm and established that **closure is only interpretable conditional on ID competence**. |
