"""ratchet — earning a level-indexed vocabulary on a LEARNING plant, one depth era at a time.
...
Run from experiments/:
  ...
  python3 rhm/practice/ratchet/launch_detached.py --fn ratchet --tag rr_s0 ...
"""
...
app = modal.App("rhm-practice-ratchet", image=image)
...
def ratchet(
    tag: str = "smoke", arms: str = "never_base,given,practice_gated,practice_early,practice_late",
    ...
    started = time.time()
    print(f"ratchet tag={tag} arms={arms} eras={ers} device={device}")
    ...
    outdir = f"{DATA_DIR}/rhm_practice_ratchet/{tag}"
