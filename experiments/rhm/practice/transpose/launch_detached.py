"""Session-isolated launcher for detached Modal runs (`recital`'s, retargeted at `transpose`).
...
The log is opened with "a", not "w" (`tall`'s launcher overwrote it), so a relaunch appends and
the file stays an archive rather than a live view.

Usage (from experiments/):
    python3 rhm/practice/transpose/launch_detached.py --fn transpose --tag tp_s0 [modal args]

Logs go to rhm/practice/transpose/results/launch_<tag>.log.
"""
...
def main():
    ...
    cmd = ["modal", "run", "--detach", f"rhm/practice/transpose/transpose.py::{fn}"] + args
