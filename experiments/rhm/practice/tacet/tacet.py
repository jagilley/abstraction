...
app = modal.App("rhm-practice-tacet", image=image)          # [tacet]
REMOTE = "rhm_practice_tacet"                                # [tacet]
...
def gate_features(out, succ, ps, n_pr, width):
    ...
    return {"v_tip": v_tip, "d_tip": y_tip - v_tip, "y_tip": y_tip,
...
def gate_order(mode, cand, score, margin, grng):
        ...
        while True:
            ...
            if gfe is not None:
                ...
                if gmode and sol_tip.shape[0]:
                    order = gate_order(gmode, sol_tip, d_tip, mg, ggrng)
                    kept = order[:n_keep]
...
def tacet_run(                                             # [tacet]
    ...
    era_caps: str = TACET_CAPS, total_cap: int = 0,
    ...
    eras: str = TACET_LADDER, seed: int = 0, era_cycles: int = 24, budget: int = 8,
...
def preflight(cycles: int = 2, eras: str = "1:25:6,2:12:6,3:6:5,4:3:5,5:1:5",
    ...
    ARM_SWEEP = ("outer_yield_m4", "gate_off_y", "gate_delta_hi", "gate_delta_lo",
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— tacet vs crescendo (gate_log ON in the fork; gate_mode unset)",
          flush=True)
    ...
    assert worst_fork <= worst_ctrl, (
        f"G-F FAILED: the tacet fork perturbed crescendo.py's code path "
        f"({worst_fork:.3e} > self-replay control {worst_ctrl:.3e}) — the exported tip values, "
        f"the gate's mining branch or the gate log is not transparent")
    print("G-F PASS — with `gate_mode` unset and `gate_log` ON, this fork replays crescendo.py.")
    return out
