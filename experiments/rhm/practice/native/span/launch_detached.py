"""Session-isolated launcher for detached Modal runs (`teacher_slot/handle/`'s, retargeted).
...
reaches it. Combined with the runner's per-cycle checkpoint commits, a lost client costs minutes.

Usage (from experiments/):
    python3 rhm/practice/native/span/launch_detached.py --fn span_run --tag sp_s0 ...

Logs go to rhm/practice/native/span/results/launch_<tag>.log.
"""
...
def main():
    ...
    cmd = ["modal", "run", "--detach", f"rhm/practice/native/span/span.py::{fn}"] + args
