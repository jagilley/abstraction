"""span — PORT 2 of `native`: the chunk's CORRIDOR piped back into the executor.

FORK NOTICE. This file is a VERBATIM COPY of `../../ratchet/ratchet.py` (2026-08-14) with the
span-head port added. `ratchet/` is not modified at all — its results stay byte-reproducible —
and `teacher_slot/handle/` is the fork convention being followed (semantically meaningful folders
beat de-duplication; `macros.py` is IMPORTED from ratchet, never copied). The fork's fidelity is
not assumed: the untreated arms here are re-run IN THIS TAG as the twins and as the noise floor,
and `rr_s0`'s stored numbers are used only as a sanity comparator, never as a twin.

WHAT IS NEW (everything else is ratchet's, unchanged):

 * `span_net.SpanHead` — given the masked span's context and WHICH committed macro is being
   called, emit the span's level-1 features in one pass, replacing the max-sum DP over
   T[l-1] plus the per-block infill read for that macro's execution. Read that module's header
   for the design decisions (shares the trunk on purpose; conditions on the macro slot, never on
   an entry index; joint emission; own RNG; closed slots run the ORIGINAL `apply_any`).
 * `finetune_generator_span` — the plant's own objective gains the span head's self-imitation
   cross-entropy at weight `span_lam`, in the same optimizer steps (`n_steps` untouched), on
   contexts drawn from the executor's own macro-call distribution with targets recomputed from
   the CURRENT executor.
 * THE PARITY GATE. A macro fires through the head only in cycles where its held-out exact-match
   parity against the DP is >= `span_tau`; below that it keeps DP+infill. Gating is PER MACRO
   (level, node), because that is the granularity at which the head is conditioned and at which
   the DP path can be kept.
 * Two treated arms (`span_true`, `span_mined`) and their in-tag untreated twins, plus
   `never_base` so the earned-vs-given fraction is computable in-tag.
 * `plant_probe` extended with the span-head-bypassed competence twin (`e_nospan`) and the
   parity/firing readouts.

PER-ARM STREAMS. ratchet consumed the GLOBAL torch RNG inside `finetune_generator`, so an arm's
trajectory depended on how many arms ran before it. Here every arm seeds its own torch stream
from its BASE (twin) name, so a treated arm and its untreated twin share a stream exactly and can
only diverge through the port. The head's construction and its training batches draw from their
OWN generators (`span_net.build_head`; a dedicated `np.random.Generator` per arm), so the shared
stream is undisturbed. That is the only reason the contrast is licensed.

PRICING, SAID OUT LOUD. The existing ledger is `n_ground * d_fb + n_mat * c_mat` and it charges
ONE materialisation per move application per row, whatever the move's span — so in that ledger a
span-head call and a DP call cost exactly the same, a grounding is still only a value/feedback
call (the head never calls the value), and the port buys NOTHING on priced time. That is the
number reported as `t_cum`, unchanged and comparable to `enum` and to `rr_s0`. Separately, and
never fed back into any decision, the runner tallies BLOCK-level materialisations —
`mat_base`/`mat_dp`/`mat_head` and `blk_base`/`blk_dp`/`blk_head`, where a level-l DP execution
costs its `span = s**(l-1)` blocks of infill and a head execution costs 1 — so the SPEC's
"one-pass materialisation against s blocks" reading is computable from the log as an explicit
counterfactual column rather than being smuggled into the headline.
...
Run from experiments/:
  ...
  python3 rhm/practice/native/span/launch_detached.py --fn span_run --tag sp_s0 ...
"""
...
app = modal.App("rhm-practice-native-span", image=image)
REMOTE = "rhm_practice_native_span"
...
def beam_moves(controller, generator, value, x0, roots, ms, rules_t, canon, depth, v, m, s,
    ...
    """ratchet's beam, with ONE change: every materialisation goes through an executor object
    (`ex`) instead of calling `macros.apply_any` directly. `PlainExecutor` calls exactly that,
    so an untreated arm is op-for-op ratchet; `SpanExecutor` substitutes the span head for a
    macro whose parity gate is open and falls back to `apply_any` otherwise."""
    import torch
    if ex is None:
        ex = SN.PlainExecutor()
    with torch.no_grad():
...
def finetune_generator_span(generator, head, ex, slots, opt, new_leaves, replay_leaves,
    ...
    """THE PLANT LEARNS, WITH A CORRIDOR HEAD. `finetune_generator` verbatim — same batches, the
    same number of optimizer steps, the same masking draws, the same level-1 loss — plus the
    span head's self-imitation cross-entropy, averaged over minted slots, at weight `lam`.

    Three properties make the twin comparison clean:
      * with no slots minted the graph is op-for-op the original, so a treated arm is
        bit-identical to its untreated twin until its head is minted and first stepped;
      * `n_steps` is untouched, so the port buys no extra optimizer steps — only an extra term
        in the loss (plant training is unpriced in this substrate for both arms alike);
      * the span batches are drawn from `span_rng`, a DEDICATED generator, so the plant's own
        `rng` draws are position-for-position what the twin's are."""
    import torch
    import torch.nn.functional as F
    powers = v ** torch.arange(s, device=device)
    generator.train()
...
ARMS = {
    "never_base":     {"vocab": "base",   "commit": None,   "span": False},
    "given":          {"vocab": "true",   "commit": None,   "span": False},
    "practice_gated": {"vocab": "earned", "commit": "delta", "span": False},
    "practice_early": {"vocab": "earned", "commit": "early", "span": False},
    "practice_late":  {"vocab": "earned", "commit": "late",  "span": False},
    # --- PORT 2: the same vocabulary, additionally executed by a span head once that macro
    #     reaches DP+infill parity. `span_true` is the SPEC's `span` row instantiated on the
    #     true table (`given`-style); `span_mined` is the same port on `practice_late`'s mined
    #     table. There is no third arm: `span` names the family, `span_true`/`span_mined` split
    #     it by which vocabulary the corridor content comes from (concentration vs coverage).
    "span_true":      {"vocab": "true",   "commit": None,   "span": True},
    "span_mined":     {"vocab": "earned", "commit": "late", "span": True},
}
...
def run_arm(label, base, overrides, shared, cfg, eras, refs, outdir, device):
    ...
    #      treated arm and its untreated baseline draw the identical sequence and can only
    ...
    # ---- PORT 2: the span head, its slots, and the executor -----------------------------
    head, slots = None, {}
    ...
    if use_span:
        ...
        for p in head.parameters():
            p.requires_grad_(True)
        # The head's OWN learning rate. It starts from a random init and has to reach DP
        # parity; the trunk stays at the plant's own `gen_lr` so the plant-guard contrast
        # against the untreated twin is not confounded by a different plant rate. Only the
        # NEW parameters move faster.
    ...
    def mint(level, table):
        """Give a freshly committed level its macro slots. The head's parameters already exist
        (minted once, off the shared stream); this registers WHICH (level, node) calls it is now
        responsible for, all initially CLOSED — the head cannot fire before parity."""
        if not use_span:
            return
        n_nodes = s ** (depth - level)
        for j in range(n_nodes):
    ...
    log = {"cycle": [], "era": [], "level": [], "t_cum": [], "e": [], "succ": [], "dres": [],
           ...
           "probe": [], "vocab": [], "span": [], "blocks": [], "m_per_solve": []}
    ...
    for era_i, era in enumerate(eras):
        ...
        for c_in_era in range(1, cfg["era_cycles"] + 1):
            ...
            # --- (c1/2) THE PARITY GATE. Measured every cycle, on the executor's own held-out
            #         10% split of its macro-call distribution, against what the DP would write
            #         with the CURRENT trunk. Gating is PER MACRO: a slot below `span_tau`
            #         simply keeps the DP+infill path. Re-checked every cycle rather than
            #         latched, so a slot the moving plant drifts away from closes again — the
            #         recert idiom the arc already uses for committed vocabulary.
            #         Instrument cost only; never priced (the plant's own probes are the
            #         precedent, and both arms pay nothing for them).
            par = {}
            ...
            b = beam_moves(controller, generator, value, mx, torch.from_numpy(mr_np), ms,
                           ...
                           beam_width=width, device=device, ex=ex)
            for key in counts:
                counts[key] += b["counts"][key]
            msucc, mdres = grade(b["x"].cpu().numpy(), mr_np, rules, s)
            e = 1.0 - float(msucc.mean())
            gps = b["counts"]["ground"] / mx.shape[0]
            # snapshot the block-level tally NOW: the probes below are unpriced instruments and
            # must not enter the counterfactual ledger any more than they enter the real one.
            blocks_cycle = dict(ex.counts)
            m_meter = b["counts"]["mat"] / mx.shape[0]
            ...
            if c_in_era == 1 or cyc % cfg["probe_every"] == 0 or c_in_era == cfg["era_cycles"]:
                ...
                for w in cfg["probe_widths"]:
                    bb = beam_moves(controller, generator, value, mx, torch.from_numpy(mr_np),
                                    ...
                                    beam_width=w, device=device, ex=ex)
                ...
                for j in range(len(eras)):
                    ...
                    bb = beam_moves(controller, generator, value, jx, torch.from_numpy(jr), ms,
                                    rules_t, canon, depth, v, m, s, budget=cfg["budget"],
                                    beam_width=wj, device=device, ex=ex)
                ...
                # THE SPAN-HEAD-BYPASSED TWIN of the competence readout: the same metering
                # beam with the head switched off, so what the port's FIRING costs or buys is
                # separable from what its gradient did to the trunk. (`parse_acc`/`infill_acc`
                # need no bypass here: the head is an output path only, so `block_logits` is
                # untouched and the plant guard is already the honest weights-alone number —
                # the `handle/` `*_nc` column, by construction.)
            ...
            log["m_per_solve"].append(m_meter)
            ...
            if use_span:
                pv = [x for x in log["span"][-1]["parity"].values() if x is not None]
                span_note = (f" | span {log['span'][-1]['n_open']}/{len(slots)} open "
                             f"par[min={min(pv):.3f} max={max(pv):.3f}] "
                             f"head_mat={blocks_cycle['mat_head']}" if pv else
                             f" | span 0/{len(slots)} open (no held-out yet)")
            else:
                span_note = ""
...
def span_run(
    tag: str = "smoke",
    arms: str = "never_base,given,span_true,practice_late,span_mined",
    ...
    span_tau: float = 0.95,
    span_min_hold: int = 256, span_capture: int = 24,
    ...
    if quick:
        cfg.update(controller_steps=800, generator_steps=800, value_steps=800,
                   ...
                   # exercise the firing path in the smoke: a real run cannot reach parity in
                   # five cycles, so the smoke opens the gate on presence of held-out data.
    ...
    started = time.time()
    print(f"span tag={tag} arms={arms} eras={ers} device={device}")
...
# --------------------------------------------------------------------------- #
# gates for the span port itself (CPU, no training loop)
# --------------------------------------------------------------------------- #

def span_selfcheck(v=8, s=2, depth=4, m=2, rule_seed=0, n=512, max_level=3, steps=400):
    """S-1 .. S-6. The properties the twin comparison and the parity gate rest on.

    S-1  minting a span head draws NOTHING from the shared torch stream (own generator), so a
         treated arm cannot diverge from its twin merely by having a head
    S-2  `span_net.trunk` is a bit-identical split of `BlockInfiller.block_logits`, and
         `span_net.dp_features` reproduces `macros.macro_features` exactly (max|d| == 0.0) --
         so the parity target IS the executor's own materialisation
    S-3  a CLOSED slot executes through `macros.apply_any` itself: `PlainExecutor` and a
         non-firing `SpanExecutor` are bit-identical to `apply_any` on every move
    S-4  an OPEN slot writes canon-rendered leaves at exactly the macro's own positions and
         nowhere else (the port cannot touch the rest of the configuration)
    S-5  the span loss reaches the shared TRUNK -- non-zero gradient on the core's parameters.
         That is the treatment; without it the plant-guard contrast would be vacuous
    S-6  the gate can open at all: from a random init, distilling the DP on a FROZEN trunk
         drives held-out exact-match parity from chance towards 1.0 at both levels"""
    import torch
    import torch.nn.functional as F
    ...
    rules = generate_rules_distinct(v, s, depth, m, seed=rule_seed)
    inv = build_inverse_maps(rules)
    canon = torch.from_numpy(np.ascontiguousarray(rules[depth - 1][:, 0, :])).to(dev)
    rules_t = [torch.from_numpy(np.ascontiguousarray(r)).to(dev) for r in rules]
    truth = MC.true_tables(rules, depth, s, v, m, max_level)
    torch.manual_seed(0)
    ...
    base_ms = build_move_set(depth, s, max_level=1)
    ms = build_ms(base_ms, {ell: truth[ell] for ell in range(2, max_level + 1)}, s, depth, dev)
    macros = [mv for mv in ms if mv.get("kind") == "macro"]

    # ---- S-1: the head costs the shared stream nothing --------------------------------- #
    torch.manual_seed(1234)
    before = torch.rand(5)
    torch.manual_seed(1234)
    head = SN.build_head(SN.slot_count(s, depth, max_level), v, 96, s ** (max_level - 1),
                         seed=99, device=dev)
    after = torch.rand(5)
    out["S1_stream_untouched_maxabs"] = float((before - after).abs().max())
    out["S1_head_params"] = int(sum(p.numel() for p in head.parameters()))
    assert out["S1_stream_untouched_maxabs"] == 0.0, "S-1 failed: the head drew from the stream"

    # ---- S-2: the trunk split and the DP replica ---------------------------------------- #
    with torch.no_grad():
        pooled, logits = SN.trunk(core, x)
        ref = core.block_logits(x)
    out["S2_trunk_logits_maxabs"] = float((logits - ref).abs().max())
    dpm = {}
    for mv in macros:
        with torch.no_grad():
            pos = SN.span_positions(mv, n, s, dev)
            obs = x.clone().scatter_(1, pos, torch.full_like(pos, -1))
            _, lg = SN.trunk(core, obs)
            mine = SN.dp_features(lg, mv, s)
            theirs, pos2 = MC.macro_features(core, x, mv, s, v)
        dpm[mv["name"]] = {"feat_maxabs": float((mine - theirs).abs().max()),
                           "pos_maxabs": float((pos - pos2).abs().max())}
    out["S2_dp_replica"] = dpm
    assert out["S2_trunk_logits_maxabs"] == 0.0
    assert all(c["feat_maxabs"] == 0.0 and c["pos_maxabs"] == 0.0 for c in dpm.values())

    # ---- S-3: closed slots run the original code path ----------------------------------- #
    slots = {SN.slot_key(mv["level"], mv["node"]):
             {"id": SN.slot_index(mv["level"], mv["node"], s, depth, max_level),
              "open": False, "move": mv, "level": mv["level"], "node": mv["node"]}
             for mv in macros}
    plain = SN.PlainExecutor()
    sx = SN.SpanExecutor(core, head, slots, cap=4096, hold_cap=1024, hold_frac=0.1,
                         per_call=32, seed=3, v=v, length=length)
    worst = 0.0
    for mv in ms:
        with torch.no_grad():
            a = MC.apply_any(core, x, mv, rules_t, canon, depth, v, m, s)
            b = plain.apply(core, x, mv, rules_t, canon, depth, v, m, s)
            c = sx.apply(core, x, mv, rules_t, canon, depth, v, m, s)
        worst = max(worst, float((a - b).abs().max()), float((a - c).abs().max()))
    out["S3_closed_path_maxabs"] = worst
    out["S3_counts_plain"] = plain.counts
    out["S3_counts_span_closed"] = dict(sx.counts)
    assert worst == 0.0, "S-3 failed: a closed slot is not the original code path"

    # ---- S-4: an open slot writes only its own span ------------------------------------- #
    s4 = {}
    for mv in macros:
        key = SN.slot_key(mv["level"], mv["node"])
        slots[key]["open"] = True
        with torch.no_grad():
            y = sx.apply(core, x, mv, rules_t, canon, depth, v, m, s)
        slots[key]["open"] = False
        pos = SN.span_positions(mv, n, s, dev)
        mask = torch.zeros_like(x, dtype=torch.bool).scatter_(1, pos, True)
        s4[mv["name"]] = {
            "outside_untouched": float((y[~mask] == x[~mask]).float().mean()),
            "leaves_on_grammar": on_grammar_rate(y.numpy(), inv[-1], v, s),
            "frac_changed": float((y[mask] != x[mask]).float().mean())}
        assert s4[mv["name"]]["outside_untouched"] == 1.0, f"S-4 failed at {mv['name']}"
    out["S4_open_slot"] = s4
    out["S4_head_mat"] = sx.counts["mat_head"]

    # ---- S-5: does the span loss reach the trunk? --------------------------------------- #
    ...
    pos = SN.span_positions(mv, n, s, dev)
    obs = x.clone().scatter_(1, pos, torch.full_like(pos, -1))
    pooled, lg = SN.trunk(core, obs)
    tgt = SN.dp_features(lg.detach(), mv, s)
    sid = torch.full((n,), slots[SN.slot_key(mv["level"], mv["node"])]["id"], dtype=torch.long)
    hl, _ = head(pooled, mv["blk0"], mv["span"], sid, teacher=tgt)
    F.cross_entropy(hl.reshape(-1, v), tgt.reshape(-1)).backward()
    out["S5_head_grad"] = float(sum(float(p.grad.abs().sum()) for p in head.parameters()
                                    if p.grad is not None))
    out["S5_core_grad"] = float(sum(float(p.grad.abs().sum()) for p in core.parameters()
                                    if p.grad is not None))
    assert out["S5_core_grad"] > 0.0, "S-5 failed: the span loss never reaches the trunk"
    core.zero_grad(set_to_none=True)
    head.zero_grad(set_to_none=True)
    for p in core.parameters():
        p.requires_grad_(False)

    # ---- S-6: can the gate open at all? -------------------------------------------------- #
    head2 = SN.build_head(SN.slot_count(s, depth, max_level), v, 96, s ** (max_level - 1),
                          seed=7, device=dev)
    opt = torch.optim.AdamW(head2.parameters(), lr=3e-3)
    pick = [macros[0], macros[-1]]
    tr, ho = {}, {}
    for mv in pick:
        key = SN.slot_key(mv["level"], mv["node"])
        pos = SN.span_positions(mv, n, s, dev)
    ...
    for step in range(steps + 1):
        if step % max(1, steps // 4) == 0:
            cell = {}
            for key, (obs, mv) in ho.items():
                with torch.no_grad():
                    pooled, lg = SN.trunk(core, obs)
                    t = SN.dp_features(lg, mv, s)
                    sid = torch.full((obs.shape[0],), slots[key]["id"], dtype=torch.long)
                    got = head2.emit(pooled, mv["blk0"], mv["span"], sid)
                cell[key] = float((got == t).all(-1).float().mean())
            prog[str(step)] = cell
        if step == steps:
            break
        loss = 0.0
        for key, (obs, mv) in tr.items():
            pooled, lg = SN.trunk(core, obs)
            t = SN.dp_features(lg, mv, s)
            sid = torch.full((obs.shape[0],), slots[key]["id"], dtype=torch.long)
            hl, _ = head2(pooled, mv["blk0"], mv["span"], sid, teacher=t)
            loss = loss + F.cross_entropy(hl.reshape(-1, v), t.reshape(-1))
        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()
    out["S6_parity_progress"] = prog
    print(json.dumps(out, indent=2, cls=NumpyEncoder))
    return out


@app.function(image=image, timeout=1800, memory=16384)
def span_selfcheck_remote():
    return span_selfcheck()


@app.local_entrypoint()
def main(quick: bool = True):
    span_run.remote(quick=quick)
