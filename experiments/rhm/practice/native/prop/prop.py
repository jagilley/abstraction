"""prop — PORT 1 of `native`: the earned vocabulary becomes the PLANNER's own unit.

FORK NOTICE. This file is a VERBATIM COPY of `../../ratchet/ratchet.py` (2026-08-14) with the
routing port added, following `../../teacher_slot/handle/handle.py`'s convention. `ratchet/` is
not modified at all — its results stay byte-reproducible — and `macros.py` is imported from
there rather than copied. The fork's fidelity is not assumed: the untreated arms here are re-run
IN THIS TAG as the twins every treated arm is compared against, and `rr_s0`'s stored numbers are
used only as a loose replicate reference, never as a twin.
...
app = modal.App("rhm-practice-native-prop", image=image)
REMOTE = "rhm_practice_native_prop"
...
def beam_moves_prop(controller, generator, value, x0, roots, ms, rules_t, canon, depth, v, m, s,
                    *, budget, beam_width, device, collect=False,
                    prop, slots, k, forced=(), n_slots, explore=0, erng=None):
    ...
    with torch.no_grad():
        ...
        for _ in range(budget):
            ...
            if explore and sel.shape[1] < n_moves:
                ex = PN.explore_moves(sel, n_moves, explore, erng, device)
                sel = torch.sort(torch.cat([sel, ex], dim=1), dim=1).values
            kk = sel.shape[1]
            child, n_mat = PN.expand_selected(generator, flat, sel, ms, MC.apply_any,
...
def plan(controller, generator, value, x0, roots, ms, rules_t, canon, depth, v, m, s,
         *, budget, beam_width, device, collect=False, port=None):
    """One entry point for every place the agent plans, so the port is either everywhere the
    beam runs or nowhere. `port=None` is ratchet's `beam_moves`, byte-for-byte."""
    if port is None:
...
ARMS = {
    # --- ratchet's five, verbatim: the untreated twins ------------------------------------
    "never_base":     {"vocab": "base",   "commit": None,    "prop_k": None},
    "given":          {"vocab": "true",   "commit": None,    "prop_k": None},
    "practice_gated": {"vocab": "earned", "commit": "delta", "prop_k": None},
    "practice_early": {"vocab": "earned", "commit": "early", "prop_k": None},
    "practice_late":  {"vocab": "earned", "commit": "late",  "prop_k": None},
    # --- PORT 1: identical vocabulary, plus a proposal head that decides what is expanded.
    #     `prop_k = -1` means k = n_moves, i.e. the filter is an exact no-op: the fidelity
    #     gate arm, which must reproduce its twin bit-for-bit.
    "given_prop_kN": {"vocab": "true", "commit": None, "prop_k": -1},
    "given_prop_k1": {"vocab": "true", "commit": None, "prop_k": 1},
    "given_prop_k2": {"vocab": "true", "commit": None, "prop_k": 2},
    "given_prop_k4": {"vocab": "true", "commit": None, "prop_k": 4},
    # the control that matters: the SAME head over base moves only. Matched exactly — under a
    # top-k filter the grounding cost depends on k, not on how many moves the arm holds, so
    # `never_base_prop_k4` and `given_prop_k4` spend the identical budget at the identical
    # width and differ only in what the head is allowed to propose.
    "never_base_prop_k1": {"vocab": "base", "commit": None, "prop_k": 1},
    "never_base_prop_k2": {"vocab": "base", "commit": None, "prop_k": 2},
    "never_base_prop_k4": {"vocab": "base", "commit": None, "prop_k": 4},
    # the same port on an EARNED table rather than the DGP's own
    "practice_late_prop_k2": {"vocab": "earned", "commit": "late", "prop_k": 2},
    "practice_late_prop_k4": {"vocab": "earned", "commit": "late", "prop_k": 4},
}
...
TWIN = {
    "given_prop_kN": "given", "given_prop_k1": "given", "given_prop_k2": "given",
    "given_prop_k4": "given",
    ...
    for era_i, era in enumerate(eras):
        ...
        for c_in_era in range(1, cfg["era_cycles"] + 1):
            ...
            b = plan(controller, generator, value, mx, torch.from_numpy(mr_np), ms,
                     ...
                     beam_width=width, device=device, port=port)
            for key in counts:
            ...
            if c_in_era == 1 or cyc % cfg["probe_every"] == 0 or c_in_era == cfg["era_cycles"]:
                ...
                for w in cfg["probe_widths"]:
                    bb = plan(controller, generator, value, mx, torch.from_numpy(mr_np),
                              ...
                              beam_width=w, device=device, port=pport)
                ...
                for j in range(len(eras)):
                    ...
                    bb = plan(controller, generator, value, jx, torch.from_numpy(jr), ms,
                              rules_t, canon, depth, v, m, s, budget=cfg["budget"],
                              beam_width=wj, device=device, port=pport)
            ...
            if cyc % cfg["checkpoint_every"] == 0:
                write_results(outdir, arm, cfg, eras, refs, log, events, complete=False,
                              extra={"prop_k": prop_k_spec, "twin": TWIN.get(base)})
...
def prop_run(
    tag: str = "smoke",
    arms: str = ("never_base,given,practice_late,"
                 "given_prop_kN,given_prop_k1,given_prop_k2,given_prop_k4,"
                 "never_base_prop_k1,never_base_prop_k2,never_base_prop_k4,"
                 "practice_late_prop_k2,practice_late_prop_k4"),
    ...
    import torch
    cfg = _cfg(c_prop=c_prop, prop_lr=prop_lr, prop_steps=prop_steps, prop_batch=prop_batch,
    ...
    started = time.time()
    print(f"native/prop tag={tag} arms={arms} eras={ers} device={device}")
...
def prop_selfcheck(v=8, s=2, depth=4, m=2, rule_seed=0, n=64, max_level=3, budget=3):
...
def prop_selfcheck_remote():
    return prop_selfcheck()


@app.local_entrypoint()
def main(quick: bool = True):
    prop_run.remote(quick=quick)
