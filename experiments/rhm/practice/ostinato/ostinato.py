"""ostinato (Track E' round 3) — THE BENCHMARK-ESTIMABILITY SWEEP: repetition per context as
the variable, testing whether delta_perf's benchmark `b(s)` pays exactly where the context
recurs often enough for `b(s)` to be estimable.

FORKED from `../intonation/intonation.py`, which is UNTOUCHED; every addition here is marked
`# [ostinato]`. Everything else in this file — including every `# [intonation]` marker below —
is the donor's, verbatim.

WHAT THIS NODE ASKS. `ma_s0` (the metered regime, `intonation` round A) found the gain trio
REVERSED against abundance: the un-benchmarked raw-`e` gain posted the only above-floor value
cells in the tag while the benchmarked delta gain — abundance's winner — dropped to the worst
L2 parity. The candidate mechanism, discussed and NOT measured, is S13(c)'s estimability
condition: `b(s)` is meaningful only under repetition per context, and scarcity is the removal
of repetition. This node measures that directly.

THE KNOB, AND WHY IT IS THAT ONE. Sized offline from `in_s0`'s and `ma_s0`'s own per-cycle
meter logs, which say something the design has to respect:

    per era-5 cycle          in_s0 (abundance)      ma_s0 (scarcity)
    calls / slot             14.85                  14.26          <-- essentially UNCHANGED
    rows / call              159.4                   75.4          <-- 2.1x fewer
    active slots             28                     28

So what scarcity removed was NOT the number of times a context recurs (`b(s)` got the same
number of EWMA updates in both regimes) but the NUMBER OF PERFORMANCES BEHIND EACH UPDATE.
The estimability quantity is therefore the effective sample count standing behind `b(s)` at
read time — (rows per update) x (1/alpha updates) — and the knob that moves it while holding
everything else fixed is `perf_bench_frac` (rho):

    the benchmark integrates a uniform random rho-subsample of each execution's rows.

Every row still gets a delta and still carries credit; only the BENCHMARK'S OWN EVIDENCE is
thinned. What that buys, and it is the whole design argument:

  * TOTAL VOLUME is exactly matched. Same instances, same beam, same executions, same rows
    trained on, same ledger, same grounding. Nothing about the world or the diet moves.
  * DIFFICULTY is exactly matched. The grammar, the damage ladder, the era schedule, the
    action set and every floor are the donor's, untouched.
  * COMPETENCE DRIFT is held fixed. The EWMA still updates at the same wall-clock instants
    with the same rate `alpha`, so the benchmark's STALENESS is identical across rungs and
    only its SAMPLING JITTER moves. S13(c) names both halves of the interior optimum; this
    isolates one of them.
  * rho = 1.0 is BIT-IDENTICAL to `intonation.py` (no RNG is drawn at all), which is what
    gate G-F certifies, on the metered arms.
  * The two comparator arms are RUNG-INVARIANT. `rawx` never reads `b(s)` and the uniform
    baseline never reads delta, so a rung costs exactly ONE arm rather than three. This is
    the reason the sweep fits a small budget at all, and it is a property of putting the knob
    on the benchmark's evidence rather than on the world's supply.

THE RUNG LADDER runs at ABUNDANCE (`in_s0`'s configuration verbatim), so the question is
sharp: can removing repetition ALONE, at abundance's volume, reproduce scarcity's reversal?
`in_s0`'s own era-5 numbers give the noise scale — SE(b) ~ sd(e)/sqrt(39 * rho * rows/call),
signal |b - e| ~ 0.012 — so rho in {1.0, 0.20, 0.04, 0.008} spans roughly SNR 7.6 -> 0.7 and
brackets SNR = 1 in the interior. `ma_s0`'s own measured thinning was 0.47, which sits between
the first two rungs and is reported as a landmark on the curve rather than paid for as an arm.

--- the donor's header follows, unchanged -------------------------------------------------
...
# [crescendo] a new app and a new remote root; A1's and A2's tags are never written to.
app = modal.App("rhm-practice-ostinato", image=image)        # [ostinato]
...
class PerfMeter:
    ...
    def __init__(self, alpha=0.05, g0=None, theta=None, calib_min=2048,
                 bench_frac=1.0, bench_seed=0):          # [ostinato]
        ...
        self.cal_event = None
        # [ostinato] THE RUNG. `bench_frac` (rho) is the fraction of each execution's rows the
        # BENCHMARK is allowed to integrate. Every row still receives a delta and still carries
        # credit; only b(s)'s own evidence is thinned, so the estimability of b(s) moves while
        # total volume, difficulty, the diet, and the EWMA's wall-clock update instants (hence
        # its staleness w.r.t. competence drift) are all held exactly fixed.
        #
        # The subsample is drawn on the METER'S OWN dedicated stream (`intonation`'s pricing
        # note 3: the meter draws no RNG on the shared stream), and at rho >= 1.0 NO DRAW
        # HAPPENS AT ALL — the identity rung is bit-identical to `intonation.py`, which is what
        # gate G-F certifies.
        self.bench_frac = float(bench_frac)
        self.brng = np.random.default_rng(int(bench_seed) + 5_150_820)
        self.reset_cycle()

    def _bench_mean(self, e):
        """[ostinato] the mean of `e` over the rho-subsample the benchmark is allowed to see.

        Returns (mean, n_seen). At rho >= 1 this is `np.mean(e)` with no RNG drawn, so the
        identity rung reproduces the donor bit for bit. At rho < 1 at least ONE row is always
        seen — a benchmark update that integrates nothing is not a thinner benchmark, it is a
        frozen one, and that would be a different treatment."""
        n = int(e.shape[0])
        if self.bench_frac >= 1.0 or n <= 1:
            return float(np.mean(e)), n
        k = int(max(1, round(self.bench_frac * n)))
        if k >= n:
            return float(np.mean(e)), n
        idx = self.brng.choice(n, size=k, replace=False)
        return float(np.mean(e[idx])), k
    ...
    def _cell(self, key):
        if key not in self.acc:
            self.acc[key] = {"n": 0, "n_fire": 0, "sum_e": 0.0, "sum_e2": 0.0, "sum_g": 0.0,
                             ...
                             "sum_w": 0.0,
                             # [ostinato] the rung's OWN measurement: how many calls updated
                             # this slot's benchmark this cycle, and how many rows those updates
                             # actually integrated. `n_bench / n_upd` is the realised sample
                             # count behind one b(s) update; `n_bench / n` is the realised rho.
                             "n_upd": 0, "n_bench": 0}
        return self.acc[key]
    ...
    def score(self, key, e, g, fired):
        ...
        # [ostinato] ONE subsampled mean serves both the first-sight init and the EWMA update,
        # so a rung is uniformly "the benchmark sees rho of this context's performances". At
        # rho = 1 `em` IS `np.mean(e)` by the same numpy reduction, so this line is the donor's.
        em, n_seen = self._bench_mean(e)
        b0 = self.bench.get(key)
        if b0 is None:
            b0 = em                     # a slot's first sight sets its own benchmark: delta ~ 0
            self.bench[key] = b0
        gate = (perf_gate_sigma(g, self.g0, self.theta) if self.calibrated
                else np.ones_like(g))
        delta = (b0 - e) * gate
        self.bench[key] = (1.0 - self.alpha) * b0 + self.alpha * em
...
ARMS = {
    ...
    "mperf_rawx":   {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     ...
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "rawx"}},
    # ---------------------------------------------------------------------------------------
    # [ostinato] THE ESTIMABILITY SWEEP. Abundance's configuration verbatim (`in_s0`'s flags),
    # one clock, and the RUNG on exactly one knob: `perf_bench_frac` (rho), the fraction of each
    # execution's rows the BENCHMARK integrates. Nothing about the world, the diet, the volume,
    # the difficulty or the EWMA's update instants moves between rungs.
    #
    # TWO OF THE SIX ARMS ARE RUNG-INVARIANT BY CONSTRUCTION, and that is the design:
    #   `os_log`  consumes no delta at all -> the uniform-plasticity control AND the clock every
    #             other arm yokes onto. Identical whatever rho is.
    #   `os_rawx` is `ma_s0`'s fixed hygiene control (`exp(+e/tau_w)`, same cap, same
    #             normaliser, no benchmark and no agency gate). It never READS b(s), so it is
    #             the same arm at every rung — one raw line the whole ladder is compared to.
    # A rung therefore costs ONE arm, not three, which is what makes a four-rung sweep fit.
    #
    # The ladder is sized on `in_s0`'s own era-5 meter numbers (159.4 rows/call, |b-e| ~ 0.012,
    # sd(e) ~ 0.13, EWMA variance factor alpha/(2-alpha) = 0.0256): SE(b) ~ 0.00165/sqrt(rho),
    # so SNR ~ 7.6*sqrt(rho) and rho in {1.0, 0.20, 0.04, 0.008} spans ~7.6 -> 0.7, bracketing
    # SNR = 1 in the interior. `ma_s0`'s own measured thinning was rho_eff = 75.4/159.4 = 0.47.
    "os_log":       {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True,
                     "loop": {"kind": "quiet", "read": "yield"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True}},
    "os_rawx":      {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True, "loop": {"kind": "yoke", "of": "os_log"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "rawx"}},
    # the four rungs. `os_r100` is `intonation`'s `perf_gain` exactly (rho = 1 draws no RNG),
    # so the ladder's top rung is also the in-tag reproduction of the abundance reference.
    "os_r100":      {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True, "loop": {"kind": "yoke", "of": "os_log"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "delta", "perf_bench_frac": 1.0}},
    "os_r020":      {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True, "loop": {"kind": "yoke", "of": "os_log"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "delta", "perf_bench_frac": 0.20}},
    "os_r004":      {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True, "loop": {"kind": "yoke", "of": "os_log"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "delta", "perf_bench_frac": 0.04}},
    "os_r001":      {"vocab": "earned", "commit": "loop", "prop_k": 4, "span": True,
                     "recert": True, "loop": {"kind": "yoke", "of": "os_log"},
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "delta", "perf_bench_frac": 0.008}},
    # [ostinato] PREFLIGHT ONLY, for the reason `perf_given*` exists in the donor: a loop arm
    # cannot commit on a forty-step substrate, so no slot is ever minted and the thinned
    # benchmark would go untested. `perf_given_rho` holds the true tables, so slots mint at c1
    # and the rho path executes before a paid setup. `given`'s stream, so it perturbs nothing.
    "perf_given_rho": {"vocab": "true", "commit": None, "prop_k": 4, "span": True,
                       "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                               "perf_gain": "delta", "perf_bench_frac": 0.04}},
    ...
    "perf_given_g": {"vocab": "true", "commit": None, "prop_k": 4, "span": True,
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "delta"}},
    # [intonation/A] the two arms that cover round A's OWN new code. Same reason as above: a
    # loop arm cannot arm on a forty-step substrate, so `mperf_rawx`/`mperf_gate` would exercise
    # neither the `rawx` transform nor the `perf_mean` ordering at preflight scale. These hold
    # the true tables, so slots mint at c1 and both paths run before a paid setup.
    "perf_given_x": {"vocab": "true", "commit": None, "prop_k": 4, "span": True,
                     "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                             "perf_gain": "rawx"}},
    "perf_given_pm": {"vocab": "true", "commit": None, "prop_k": 4, "span": True,
                      "cfg": {"span_tau_fire": 0.50, "perf_meter": True,
                              "gate_mode": "perf_mean"}},
...
TWIN = {
    ...
    "perf_given_x": "given", "perf_given_pm": "given",
    "mperf_log": "enum_live", "mperf_gain": "enum_live", "mperf_gate": "enum_live",
    "mout_gate": "enum_live", "mperf_rawx": "enum_live",
    # [ostinato] every rung arm takes the anchor's stream too, exactly as the donor's do, so
    # each is bit-identical to `os_log` until its own gain switches on and the RUNG is the only
    # thing separating one rung from the next.
    "os_log": "enum_live", "os_rawx": "enum_live",
    "os_r100": "enum_live", "os_r020": "enum_live",
    "os_r004": "enum_live", "os_r001": "enum_live",
    "perf_given_rho": "given",
}
...
def run_arm(label, base, overrides, shared, cfg, eras, refs, outdir, device):
    ...
    #        [ostinato] perf_bench_frac  the RUNG: how much of each execution the BENCHMARK
    #                     sees. 1.0 == the donor exactly (no RNG drawn).
    pmeter = (PerfMeter(alpha=cfg["perf_alpha"], g0=cfg.get("perf_g0"),
                        theta=cfg.get("perf_theta"), calib_min=cfg["perf_calib_min"],
                        bench_frac=cfg.get("perf_bench_frac", 1.0),          # [ostinato]
                        bench_seed=int(cfg["seed"]) * 7 + int(cfg.get("perf_bench_seed", 0)))
        ...
        while True:
            ...
            if pmeter is not None:
                mrow = {"c": cyc,
                        ...
                        "bench_frac": float(pmeter.bench_frac),     # [ostinato]
                  ...
                  extra={"prop_k": prop_k_spec, "span_mode": use_span,
                         ...
                         "perf_mode": (None if pmeter is None else
                                       {"tau_fire": cfg.get("span_tau_fire"),
                                        ...
                                        # [ostinato] the rung, on the arm's own record
                                        "bench_frac": cfg.get("perf_bench_frac", 1.0)}),
...
def _cfg(**kw):
    cfg = dict(
        ...
        # [ostinato] THE RUNG. `perf_bench_frac` (rho) is the fraction of each execution's rows
        # the BENCHMARK integrates — the estimability variable, moved with total volume,
        # difficulty, the diet and the EWMA's update instants all held fixed. 1.0 == the donor,
        # bit-for-bit (no RNG is drawn). `perf_bench_seed` keys the meter's own stream, which
        # touches no shared stream (`intonation` pricing note 3).
        perf_bench_frac=1.0, perf_bench_seed=20_260_831,
    )
    cfg.update(kw)
    return cfg
...
def ostinato_run(                                          # [ostinato]
    ...
    # [ostinato] the RUNG is a per-ARM property (`ARMS[...]["cfg"]["perf_bench_frac"]`) for the
    # same reason `perf_gain` is: an arm IS its rung. What the run owns is the meter stream's
    # key, so the whole ladder is reproducible from the command line.
    perf_bench_seed: int = 20_260_831,
    ...
    cfg = _d6_cfg(depth=depth, m=mm, n_corrupt=n_corrupt, prov_offset=prov_offset,
                  ...
                  perf_bench_seed=perf_bench_seed)                    # [ostinato]
    if quick:
...
def preflight(cycles: int = 2, eras: str = "1:25:6,2:12:6,3:6:5,4:3:5,5:1:5",
    ...
    ARM_SWEEP = ("perf_given", "perf_given_g", "perf_given_x", "perf_given_pm",
    ...
    for _lbl in ("perf_given", "perf_given_g", "perf_log", "perf_gain", "perf_raw",
                 ...
                 # [ostinato] the sweep's own arms
                 "perf_given_rho", "os_log", "os_rawx",
                 "os_r100", "os_r020", "os_r004", "os_r001"):
        if not _ran(_lbl):
            continue
        _r = json.load(open(f"{outdir}/{_lbl}/results.json"))
    ...
    for _lbl in ("perf_given_g", "perf_given_x", "perf_gain", "perf_raw", "mperf_rawx",
                 # [ostinato] every rung is a gain arm, and the budget normalisation has to be
                 # working at EVERY rung or the ladder compares plasticity budgets, not
                 # benchmarks. This is `in_s0`'s `perf_raw` lesson applied to the sweep.
                 "perf_given_rho", "os_rawx",
                 "os_r100", "os_r020", "os_r004", "os_r001"):
        if not _ran(_lbl):
            continue
        _r = json.load(open(f"{outdir}/{_lbl}/results.json"))
        _ws = [q["wstat"] for q in (_r["log"].get("perf") or []) if q.get("wstat")]
        assert _r["config"]["perf_gain"] in ("delta", "raw", "rawx"), \
            f"{_lbl}: no gain mode"
        _I[f"I-4 gain:{_lbl}"] = {
    ...
    _pk = {}
    for _lbl in ("perf_gate", "outcome_gate", "perf_given_pm"):
        if not _ran(_lbl):
            continue
        _g = json.load(open(f"{outdir}/{_lbl}/results.json"))["log"]["gate"]
        _pk[_lbl] = [(q["n_sol_tip"], q["n_keep_tip"]) for q in _g]
        _I[f"I-5 keeps:{_lbl}"] = {"last3": _pk[_lbl][-3:],
                                   "mode": (_g[-1].get("mode") if _g else None),
                                   # [intonation/A] the coverage fix's own number: ties on the
                                   # key the mode RANKS BY, i.e. within the executed set.
                                   "p_tie_last3": [q.get("p_tie") for q in _g[-3:]],
                                   "n_exec_cand_last3": [q.get("n_exec_cand") for q in _g[-3:]],
                                   "p_tie_exec_last3": [q.get("p_tie_exec") for q in _g[-3:]],
                                   "mine_cells_last3": [(q.get("mine_int"), q.get("mine_bad"),
                                                         q.get("mine_non")) for q in _g[-3:]]}
    if _ran("perf_gate") and _ran("outcome_gate"):
        _a, _b = _pk["perf_gate"], _pk["outcome_gate"]
    ...
    # ---- [ostinato] GATE O — THE RUNG. Three assertions, and they are the whole node:
    #      O-1  the knob BINDS: each rung arm carries its declared rho on its own record, and
    #           the REALISED thinning (`n_bench / n`, summed over the run) tracks it. This is
    #           the sweep's independent variable, measured rather than declared.
    #      O-2  the knob is INERT at rho = 1: the identity rung draws no RNG and its realised
    #           thinning is exactly 1.0. (The bit-identity half is G-F's.)
    #      O-3  the rungs DIFFER: two arms at different rho must not end with the same
    #           benchmark state. If they do, the ladder is one arm run four times.
    _O = {}
    _bench_end = {}
    for _lbl in ("perf_given_rho", "os_r100", "os_r020", "os_r004", "os_r001"):
        if not _ran(_lbl):
            continue
        _r = json.load(open(f"{outdir}/{_lbl}/results.json"))
        _pr = _r["log"].get("perf") or []
        _rho = float(_r["config"].get("perf_bench_frac", 1.0))
        _n = sum(int(c.get("n", 0)) for q in _pr for c in (q.get("slot") or {}).values())
        _nb = sum(int(c.get("n_bench", 0)) for q in _pr for c in (q.get("slot") or {}).values())
        _nu = sum(int(c.get("n_upd", 0)) for q in _pr for c in (q.get("slot") or {}).values())
        _O[f"O-1 rung:{_lbl}"] = {
            "rho_declared": _rho, "rows": _n, "rows_benchmarked": _nb, "n_updates": _nu,
            "rho_realised": (round(_nb / _n, 4) if _n else None),
            "rows_per_update": (round(_n / _nu, 2) if _nu else None),
            "benched_rows_per_update": (round(_nb / _nu, 3) if _nu else None)}
        assert float(_r["perf_mode"]["bench_frac"]) == _rho, \
            f"{_lbl}: the rung on the record disagrees with its config"
        if _n and _nu:
            # The realised fraction sits between the ONE-ROW-PER-UPDATE floor (`_nu / _n`,
            # which binds at the bottom rung and at small calls) and the declared rho, up to
            # integer rounding. Both bounds are recorded; the assertion is that the knob did
            # something and did not do MORE than it declared.
            _floor, _got = _nu / _n, _nb / _n
            assert _floor - 1e-9 <= _got <= max(_rho, _floor) + 0.02, (
                f"{_lbl}: realised thinning {_got:.4f} outside [floor {_floor:.4f}, "
                f"rho {_rho}] — the rung knob is not doing what it declares")
            if _rho >= 1.0:
                assert _nb == _n, f"{_lbl}: rho = 1 thinned {_n - _nb} rows — not the identity"
        _bench_end[_lbl] = (_pr[-1]["bench"] if _pr else {})
    if _ran("os_r100") and _ran("os_r001") and _bench_end.get("os_r100"):
        _O["O-3 rungs differ"] = {
            "os_r100_vs_os_r001": (_bench_end["os_r100"] != _bench_end.get("os_r001"))}
    ok["ostinato_gate_O"] = _O
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    # [ostinato] retargeted one more link along the chain: THIS fork's direct donor is
    # `intonation.py` (which replays `tacet.py` at 0.000e+00, which replays `crescendo.py`,
    # ...). And the gate is run in a STRONGER configuration than the donor's own, because it
    # can be: the only `# [ostinato]` addition is inside `PerfMeter`, and `perf_bench_frac`
    # defaults to 1.0, at which `_bench_mean` returns `np.mean(e)` by the same numpy reduction
    # WITHOUT DRAWING ANY RNG. So the gate runs the METERED arms (`perf_given` with the meter
    # alone, `perf_given_g` with the meter AND the delta gain consuming b(s)) rather than the
    # donor's un-metered `anchor`/`given_c1` — i.e. it certifies the exact code path the knob
    # lives on, at its identity setting, instead of a path the knob never touches.
    # `span_min_hold`/`span_tau` are dropped so slots actually OPEN at smoke scale; otherwise
    # the head never fires, `PerfMeter.score` never runs, and the gate would pass vacuously.
    # That is asserted, not hoped for (`n_fired > 0` below).
    from rhm.practice.intonation import intonation as SP
    ...
    cfg = _d6_cfg(era_cycles=cycles, seed=seed, probe_every=2, probe_widths=(1, 2),
                  ...
                  max_macro_level=4,
                  # [ostinato] so the span slots OPEN at smoke scale and the meter path runs
                  span_min_hold=8, span_tau=0.0, span_capture=8,
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— ostinato vs intonation, METERED path (perf_bench_frac = 1.0)",
          flush=True)
    ...
    SERIES = ["e", "succ", "dres", "t_cum", "n_moves", "width", "g_per_solve", "e_practice",
              "vloss", "gloss", "n_solved", "n_mined", "m_per_solve"]
    rows, worst_fork, worst_ctrl = [], 0.0, 0.0
    n_fired_total = 0                                             # [ostinato]
    # [ostinato] the METERED arms, per the note above: `perf_given` is the meter alone,
    # `perf_given_g` is the meter plus the delta gain, i.e. the full b(s) -> delta -> credit ->
    # plasticity chain the rung knob sits inside.
    for arm in ("perf_given", "perf_given_g"):
        ...
        ev = lambda r: [(e["era"], e["level"], e["cycle"], e["n_entries"])
                        for e in r["events"] if e["kind"] == "commit"]
        row["events_equal"] = bool(ev(a1) == ev(b))
        # [ostinato] the gate must not pass vacuously: the meter has to have RUN.
        row["n_fired"] = int(sum(q.get("n_fired", 0) for q in (b["log"].get("perf") or [])))
        row["n_misfire"] = int(sum(q.get("n_misfire", 0) for q in (b["log"].get("perf") or [])))
        n_fired_total += row["n_fired"]
        rows.append(row)
    ...
    out = {"scale": "smoke", "rows": rows, "worst_fork": worst_fork,
           ...
           "events_equal": all(r["events_equal"] for r in rows),
           "n_fired_total": n_fired_total,                          # [ostinato]
           "seconds": time.time() - started}
    with open(f"{outdir}/gate.json", "w") as fh:
        json.dump(out, fh, indent=2, cls=NumpyEncoder)
    volume.commit()
    print(f"\n[gf] === G-F (smoke scale) === max|fork - intonation| = {worst_fork:.3e}   "
          f"control = {worst_ctrl:.3e}   head rows fired = {n_fired_total}", flush=True)
    assert out["events_equal"], "G-F FAILED: commit events differ from intonation.py"
    # [ostinato] NOT VACUOUS: the meter, and therefore `PerfMeter.score` and `_bench_mean`,
    # actually ran on both sides of the comparison.
    assert n_fired_total > 0, (
        "G-F VACUOUS: the head never fired, so `PerfMeter.score` never ran and the identity "
        "rung was never exercised — raise `cycles` or drop `span_min_hold` further")
    assert worst_fork <= worst_ctrl, (
        f"G-F FAILED: the ostinato fork perturbed intonation.py's code path "
        f"({worst_fork:.3e} > self-replay control {worst_ctrl:.3e}) — `_bench_mean` is not "
        f"an identity at perf_bench_frac = 1.0, or the meter's extra accumulators are not "
        f"transparent")
    print("G-F PASS — at perf_bench_frac = 1.0, this fork replays intonation.py on the "
          "METERED path (meter + delta gain), with the head actually firing.")
    return out
...
def main(quick: bool = True):
    ostinato_run.remote(quick=quick)
