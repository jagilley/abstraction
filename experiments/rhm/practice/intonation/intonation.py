...
app = modal.App("rhm-practice-intonation", image=image)     # [intonation]
REMOTE = "rhm_practice_intonation"                           # [intonation]
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    from rhm.practice.tacet import tacet as SP
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— intonation vs tacet (gate_log ON; every [intonation] knob off)",
          flush=True)
    ...
    with open(f"{outdir}/gate.json", "w") as fh:
        json.dump(out, fh, indent=2, cls=NumpyEncoder)
    volume.commit()
    print(f"\n[gf] === G-F (smoke scale) === max|fork - tacet| = {worst_fork:.3e}   "
          f"control = {worst_ctrl:.3e}", flush=True)
    assert out["events_equal"], "G-F FAILED: commit events differ from tacet.py"
    assert worst_fork <= worst_ctrl, (
        f"G-F FAILED: the intonation fork perturbed tacet.py's code path "
        f"({worst_fork:.3e} > self-replay control {worst_ctrl:.3e}) — the executor subclass, "
        f"the beam's tracking branch or the meter's log row is not transparent")
    print("G-F PASS — with every [intonation] knob off, this fork replays tacet.py.")
    return out
...
def main(quick: bool = True):
    intonation_run.remote(quick=quick)
