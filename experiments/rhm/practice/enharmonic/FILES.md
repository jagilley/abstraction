...
| `results/RUN_en_s0.sh` … `RUN_en_s4.sh` | The exact commands of record. Logs beside them are untracked. |
...
| `figures/` | Compact mirrors of `en_s0`, `en_s1`, `en_s2` (`given_cat_tok` only), `en_s2b`, `en_s3`, `en_s4`, `fb_s0`, `fb_s1`, their `*_reduction.txt`, and `en_s3_alias_audit.txt`. `en_smoke` is on the Modal volume only (`rhm-scaling-data:/data/rhm_practice_enharmonic/`). |
