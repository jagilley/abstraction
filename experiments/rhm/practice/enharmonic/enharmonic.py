...
app = modal.App("rhm-practice-enharmonic", image=image)      # [enharmonic]
REMOTE = "rhm_practice_enharmonic"                            # [enharmonic]
...
MEASURED_FLOORS = {
    ...
    # and has no banked ancestor at all, so `enharmonic_run` takes both as parameters and the
...
# [figured_bass] the third child's tag, SOURCES BEFORE YOKES (`enharmonic_run` asserts a yoke's
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— enharmonic vs tutti (gate_log + slot_rec ON; every [enharmonic] knob off)",
          flush=True)
    ...
    print("G-F PASS — with every [enharmonic] knob off, this fork replays tutti.py.")
...
def main(quick: bool = True):
    enharmonic_run.remote(quick=quick)
