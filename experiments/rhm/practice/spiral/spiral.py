...
app = modal.App("rhm-practice-spiral", image=image)
...
def spiral_run(
    tag: str = "sp_s0",
    arms: str = SPIRAL_ARMS,
    eras: str = SPIRAL_LADDER, seed: int = 0, era_cycles: int = 24, budget: int = 8,
    ...
    """PHASE B. The spiral itself: earn level k live, consolidate it into the learner's own
    planner and executor, then earn level k+1 natively over the routed policy — on the depth-6
    ladder, where the frontier (L2-L3 earnable) and the grammar's ceiling (damage to L5) are
    in different places for the first time.

    Commit policy is FIXED across every earning arm: certify-else-provisional-at-boundary,
    recert live. The shadow certificate is evaluated per level in EVERY arm every cycle
    regardless of what actually caused the commit, because cycles-to-certification is the rate
    readout and it must not be conditional on the commit rule that consumed it."""
    import torch
    ...
    if quick:
        cfg.update(controller_steps=800, generator_steps=800, value_steps=800,
                   ...
                   # exercise the corridor's FIRING path in the smoke: no real run reaches
                   # parity in three cycles, so the smoke opens the gate on data presence.
    ...
    print(f"spiral/PHASE B tag={tag} depth={cfg['depth']} m={cfg['m']} "
          f"seqlen={cfg['s'] ** cfg['depth']} arms={arms}\n"
          f"  ladder={[(e_['name'], e_.get('cycles') or cfg['era_cycles']) for e_ in ers]} "
          f"= {total_cycles} cycles/arm   device={device}", flush=True)
    ...
    pricing = {"action_set": sizes,
               "width_enum_G482": {n_: fit_width(n_, cfg["budget"], 482) for n_ in sizes.values()},
               "width_enum_G722": {n_: fit_width(n_, cfg["budget"], 722) for n_ in sizes.values()},
               "width_route_G482": {k: PN.fit_width_k(k, cfg["budget"], 482)
    ...
    print(f"[pricing] {json.dumps(pricing, cls=NumpyEncoder)}", flush=True)
    ...
    with open(os.path.join(outdir, "setup.json"), "w") as fh:
        json.dump({"config": cfg, "eras": ers, "refs": refs, "plant": plant,
                   "pricing": pricing, "read_acc": float(shared["read_acc"]),
    ...
    # `summary.json` is written after EVERY arm, not at the end: a seven-arm run is long
    # enough that a timeout must not cost the arms that already finished their reduction.
    ...
    for label, base, ov in parse_arms(arms):
        ...
        if r["log"]["span"]:
            summary["arms"][label]["span_open_max"] = max(
                int(c["n_open"]) for c in r["log"]["span"])
        cs = r["shadow_cert"]
        print(f"[arm] {label}: {n} cycles at {summary['cycle_seconds'][label]:.1f} s/cycle; "
              f"cert L2 c{cs['2'].get('fired')} L3 c{cs.get('3', {}).get('fired')}; "
              f"span slots open (max) {summary['arms'][label]['span_open_max']}", flush=True)
...
def preflight(cycles: int = 2, eras: str = "1:25:3,2:12:4,3:6:2,4:3:2,5:1:2",
    ...
    # every arm the MAIN run carries, plus the two donor shapes the fidelity gates rest on
    for arm in ("given", "fid", "given_native", "enum_live", "enum_live_g722",
                "spiral_route", "spiral", "practice_late", "practice_late_prop_k4"):
        ...
        ok[f"run_arm:{arm}"] = {
            ...
            "identity": bool(res["log"]["miner"][-1].get("2", {}).get("keys_at_support")
                             is not None)}
    # the instruments must APPEAR, not merely fail to crash
    sp = json.load(open(f"{outdir}/spiral/results.json"))
    ...
    # the g722 control must actually BE the control: same arm, different declared budget, and
    # the width that budget buys at n=48 has to differ from G=482's — otherwise the arm is a
    # duplicate and the rate comparison it exists to de-confound is not de-confounded.
    g7 = json.load(open(f"{outdir}/enum_live_g722/results.json"))
    ok["g722_budget"] = g7["config"]["g_budget"]
    ok["g722_width_n48"] = fit_width(48, cfg["budget"], 722)
    ok["g482_width_n48"] = fit_width(48, cfg["budget"], 482)
    assert g7["config"]["g_budget"] == 722, "the g722 arm did not pick up its own budget"
    assert g7["config"].get("width_cap_ref_g") == 482, "the g722 arm lost its width cap"
    assert fit_width(48, 8, 722) == 2 and fit_width(48, 8, 482) == 1, \
        "the g722 control does not separate from G=482 at n=48 — the arm is pointless"
    # THE TWIN CLAIM, asserted rather than argued: with the width cap in place the g722 control
    # must be identical to `enum_live` up to and including the cycle of the first commit, and
    # must differ after it. Without the cap G=722 also buys width 3 at n=32 and the arms differ from
    # cycle 1, which would confound the cycles-to-certification comparison in the control's
    # favour — the exact failure mode the cap exists to prevent.
    el = json.load(open(f"{outdir}/enum_live/results.json"))
    c1_ = next((e["cycle"] for e in el["events"] if e["kind"] == "commit"), None)
    wa_, wb_ = el["log"]["width"], g7["log"]["width"]
    ok["g722_width_series"] = {"enum_live": wa_, "enum_live_g722": wb_,
                               "first_commit_cycle": c1_}
    assert c1_ is not None, "no commit in the enum arms — the twin claim is untestable"
    assert wa_[:c1_] == wb_[:c1_], (
        f"g722 diverges from enum_live BEFORE the commit: {wa_[:c1_]} vs {wb_[:c1_]}")
    assert wa_[c1_:] != wb_[c1_:], (
        f"g722 does not diverge from enum_live AFTER the commit: {wa_[c1_:]} vs {wb_[c1_:]}")
    # the corridor must be REACHABLE: the head has to be able to fire at least once, or the
    # corridor wall stays untestable exactly as it did in Phase A
    assert any(c["n_open"] > 0 for c in sp["log"]["span"]), \
        "no span slot ever opened — the corridor path is untested"
    ok["span_open_max"] = max(int(c["n_open"]) for c in sp["log"]["span"])
