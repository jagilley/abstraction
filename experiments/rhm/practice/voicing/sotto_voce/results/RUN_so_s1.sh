#!/bin/sh
# [so_s1] WHO GRADES THE BABBLE — seed 0. Three arms in one tag, all the composed chooser,
# all with the probe channel ON, all CLOCK-YOKED to the banked seed-0 anchor `vo_s3:voi3_dp`.
...
# All three are read at reduction time with `--bank`; `--yoke-from-tag vo_s3` resolves the
# source out of the PARENT node's volume dir (`YOKE_REMOTE`), so the arm this tag replays is
# byte-identical to the one `vo_s3b` replayed.
#
...
#
# Gates run before launch, all PASS (`_preflight_s1` + `so_gf1`):
...
python3 rhm/practice/voicing/sotto_voce/launch_detached.py --fn voicing_run --tag so_s1 \
