"""[voicing] The reducer for a `voicing` tag — `analyze_enharmonic.py`'s sections that still
apply, plus the five this node adds.
...
Usage (from experiments/):
    python3 rhm/practice/voicing/analyze_voicing.py --tag vo_s1 \\
        --bank en_s9:endo_ledger_open_ung5_ra,en_s8:endo_ledger_open_ung5
"""
...
def main():
    ...
    arms = [x for x in a.arms.split(",") if x] or sorted(
        d for d in os.listdir(os.path.join(FIG, a.tag))
        if os.path.isfile(os.path.join(FIG, a.tag, d, "results.json")))
    ...
    for arm in arms:
        ...
        o("      slot   reads  n(last) base(last)  auc f/med/last  >.5   "
          "n(last) base(last)  auc f/med/last  >.5")
        for k in sorted(per, key=lambda z: (int(z.split(":")[0]), int(z.split(":")[1]))):
            rows = per[k]
            au = [a for _, _, _, a, _, _, _ in rows if a is not None]
            pa = [a for _, _, _, _, _, _, a in rows if a is not None]
