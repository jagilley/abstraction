...
app = modal.App("rhm-practice-woodshed", image=image)   # [woodshed]
REMOTE = "rhm_practice_woodshed"                        # [woodshed]
            ...
            if prop is not None:
                ...
                if (rl and committed.get(rl) is not None
                    ...
                    print(f"[reh] arm={arm} c{cyc} L{rl} {cfg['reh_mode']:>6s} "
                          ...
                          f"f_level={reh_rec['frac_at_level']:.3f}", flush=True)
                    del rout, rtips
...
def woodshed_run(                                                       # [woodshed]
    tag: str = "wd_s0",
    arms: str = WOODSHED_ARMS,
    ...
    print(f"woodshed tag={tag} depth={cfg['depth']} m={cfg['m']} "
          ...
          f"= {total_cycles} cycles/arm   entry_rec={entry_rec}   device={device}", flush=True)
...
def main(quick: bool = True):
    woodshed_run.remote(quick=quick)
