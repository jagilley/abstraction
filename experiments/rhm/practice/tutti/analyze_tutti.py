...
def in_tag_floors(tag, A, setup, summary, quiet=False):
    ...
    for k in PANEL_KEYS:
        ...
        off = 0
        if k == "dsil" and any(x is None for x in ser) and any(x is not None for x in ser):
...
def main():
    ...
    ap.add_argument("--tag", default="tu_s0")             # [tutti]
    ap.add_argument("--gf-tag", default="tu_gf")          # [tutti]
    ap.add_argument("--donor-tag", default="ca_s0",       # [tutti]
                    help="the caesura tag the cross-tag gates replay against")
    ...
    # [tutti] the unification sections, LAST for the reason every donor gave: if the gates
...
TREAT, CEIL, SCHED, EXT = "tu_y_exo", "ceiling_m3", "dsil_sched", "outer_yield_m4x"
