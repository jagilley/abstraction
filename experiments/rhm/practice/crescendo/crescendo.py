...
app = modal.App("rhm-practice-crescendo", image=image)
...
def crescendo_run(
    ...
    era_caps: str = CRESCENDO_CAPS, total_cap: int = 0,
    ...
    eras: str = CRESCENDO_LADDER, seed: int = 0, era_cycles: int = 24, budget: int = 8,
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    """
    import torch
    # [crescendo] retargeted to THIS fork's direct donor, `maestro.py` (which in its own turn
    # replays `conductor.py` at 0.000e+00, which replays `assay.py`, so the chain is intact).
    # The gate runs at `max_macro_level=3` with `commit_max_level` unset, which is the exact
    # configuration in which this file IS its donor: `commit_maxl` collapses to `maxl`, no A3
    # arm is scheduled, and the only other addition is one extra logged panel key. If this is
    # not 0.000e+00 the fork has drifted and no A3 contrast means anything.
    from rhm.practice.maestro import maestro as SP
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— crescendo vs maestro (shadow panel ON in the fork; commit_max_level unset)",
          flush=True)
    ...
    with open(f"{outdir}/gate.json", "w") as fh:
        json.dump(out, fh, indent=2, cls=NumpyEncoder)
    volume.commit()
    print(f"\n[gf] === G-F (smoke scale) === max|fork - maestro| = {worst_fork:.3e}   "
          f"control = {worst_ctrl:.3e}", flush=True)
    assert out["events_equal"], "G-F FAILED: commit events differ from maestro.py"
    assert worst_fork <= worst_ctrl, (
    ...
    print("G-F PASS — with the loop off and the shadow panel ON, this fork replays maestro.py.")
    return out
...
def main(quick: bool = True):
    crescendo_run.remote(quick=quick)
