"""Session-isolated launcher for detached Modal runs (`../ratchet/launch_detached.py`'s
pattern, retargeted at this node — the donor hardcodes its own module path, so it is copied
...
reaches it. Combined with the runner's per-pass checkpoint commits, a lost client costs
minutes.

Usage (from experiments/):
    python3 rhm/practice/reread/launch_detached.py --fn reread --tag rr_s0 [extra modal args...]

Logs go to rhm/practice/reread/results/launch_<tag>.log.
"""
...
def main():
    ...
    cmd = ["modal", "run", "--detach", f"rhm/practice/reread/reread.py::{fn}"] + args
