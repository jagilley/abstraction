"""Session-isolated launcher for detached Modal runs (`../launch_detached.py`'s pattern,
retargeted at the LM twin; invoked with modal's `-m` module form, which is what
`conditional_revision` uses).
...
Usage (from experiments/):
    python3 rhm/practice/reread/lm/launch_detached.py --fn lm_reread --tag lm0 [modal args...]

Logs go to rhm/practice/reread/lm/results/launch_<tag>.log.
"""
...
MODULE = "rhm.practice.reread.lm.lm_reread"


def main():
    args = sys.argv[1:]
    tag, fn = "default", "lm_reread"
