"""antiphon_p — shape P prime: the PER-EXECUTION PROVENANCE JOIN, as a logging-only instrument.

FORK NOTICE. This file forks `../../woodshed/woodshed.py` VERBATIM and adds exactly one thing,
marked `# [antiphon_p]` at each insertion point. `woodshed/`, `assay/`, `census/`, `spiral/`
and everything upstream are NOT modified. The addition draws no RNG, changes no numerics, and
enters no ledger; the fidelity gate is that every arm run out of this file with the join
recorder ON reproduces `wd_s1`'s banked series bit-for-bit (max|d| = 0.000e+00 over 13 series
x 116 cycles). That gate is what makes the tag readable.

  THE ONE ADDITION — WHICH ENTRY THE EXECUTOR SERVED, BESIDE WHO ASKED FOR IT.

  `antiphon/provenance`'s offline pass (`prov_tag.py`) could only use the MINER's record as
  the efference copy: the spellings the agent's own solved rollouts emitted. That is a record
  of ANSWERS the agent produced, not of QUESTIONS it posed, and it is downstream of use — an
  entry becomes "self" only by being served often enough to be re-derived. Measured
  consequence: every provenance statistic came out ANTI-correlated with trust (Kendall tau_b
  -0.29..-0.47 vs pi's L3 argmax over `as_s0`'s six arms).

  The genuine efference copy of a POSED QUESTION on this substrate is pi's PROPOSAL: at each
  beam node pi emits an action slot (level, node) — a claim that a macro at that slot applies
  here — and the executor DP answers by serving a table ENTRY. Both halves are already
  computed in the run loop; only the aggregates are logged (`probe.pi.argmax_hist` and
  `entry.hist`), never the join. This file logs the join:

      log["ap_join"][c] = {"keys": {level: [flat key per entry index]},
                           "hist": {phase: {level: {source: [count per entry index]}}}}

  `source` is who put the move on the beam at the moment the entry was served:
      prop     pi's own top-k proposal            — efferent: pi asked this question
      forced   the forced-expansion window        — exogenous: the schedule asked it
      explore  the exploration draw               — exafferent w.r.t. pi: nobody asked
      unknown  a call outside the proposal path   — must be 0 in the beam/probe phases

  Keys are PLAIN FLAT KEYS (the level-1 feature tuple), index-aligned with `hist`, so this
  instrument and the node root's posed-vs-landed delivery ledger read as one primitive at two
  grains: the question grain (which repair instance was posed) and the proposal grain (which
  slot pi asked for, and which spelling came back).

  ------------------------------------------------------------------------------------------
  The donor's own notice follows verbatim.
  ------------------------------------------------------------------------------------------

woodshed — F1 part 2: does REHEARSAL compress the trust clock that gift could not skip?

FORK NOTICE. This file forks `../assay/assay.py` VERBATIM and adds exactly one thing, marked
`# [woodshed]` at each insertion point. `assay/`, `census/`, `spiral/` and everything upstream
are NOT modified, and every arm of the donor runs out of this file bit-for-bit (the knobs-off
gate: with `reh_level` unset not one line below executes, and `analyze_woodshed.py` asserts
the cross-tag replay of `as_s0`'s `anchor` / `exact` / `given_c1` over all 116 cycles).

  THE ONE ADDITION — TARGETED REHEARSAL OF A RECEIVED VOCABULARY.

  `census`/`assay` established that arrival dominates: a table whose CONTENT is perfect but
  which arrives at the commit cycle (`exact`) leaves 72-82% of the deep-era gap standing
  against the same table arriving at cycle 1 (`given_c1`), and the carrier is pi's per-level
  proposal mass (0.580 from birth vs <=0.276 for every late arrival). `woodshed`'s offline
  reduction (`reduce_trust.py`) put a clock on that: after a commit-time arrival pi first
  DE-FUNDS the new level to ~0.13x its chance share, takes a median ~30 cycles to climb back
  to chance, and ends near 1.7x chance, while a c1 arrival never dips and ends at 3.3x.

  This file asks whether the missing thing is time or USE. Rehearsal is extra practice, drawn
  at the received level's own damage cells across ALL of that level's nodes (varied demand
  rather than the era's single node), run through the same beam at the same width, and mined
  into pi's self-imitation buffer by the same `prop_pairs` path the ordinary cycle uses. No
  new learning rule, no new signal, no oracle.

  Two knobs, and the second is the control the roadmap asks for:
    `reh_mode="credit"`  pi imitates the rehearsal trajectories that SOLVED  (credited use)
    `reh_mode="expose"`  pi imitates the SAME NUMBER of rehearsal trajectories drawn at
                         random from all survivors of the same episodes  (exposure without
                         credit). Identical episodes, identical states, identical pair count,
                         identical gradient steps; the success filter is the only difference.

  Deliberately NOT touched, so the intervention is on the trust half alone:
    ...
    * rehearsal draws from its OWN numpy and torch streams, so the arm-shared stream position
      is untouched and the twin discipline still holds.
...
FORK NOTICE. `assay.py` forks `../census/census.py` VERBATIM and adds four things, each marked
...
app = modal.App("rhm-practice-antiphon-p", image=image)  # [antiphon_p]
REMOTE = "rhm_practice_antiphon_p"                       # [antiphon_p]
WOODSHED_REMOTE = "rhm_practice_woodshed"                # [antiphon_p] the gate donor
...
def beam_moves_prop(controller, generator, value, x0, roots, ms, rules_t, canon, depth, v, m, s,
    ...
    with torch.no_grad():
        ...
        beams = x0[:, None, :]
        seqs = torch.zeros(batch, 1, 0, dtype=torch.long, device=device)
        srcs = torch.zeros(batch, 1, 0, dtype=torch.long, device=device)   # [antiphon_p]
        width = 1
        ...
        for _ in range(budget):
            ...
            kk = sel.shape[1]
            # [antiphon_p] who put each selected move on the beam, aligned with `sel`. Pure
            # tensor ops on a fresh tensor: no RNG, no effect on any value below.
            src = _ap_source_of(sel, n_moves) if _APREC["on"] else None
            ...
            prev = seqs.gather(1, parent[:, :, None].expand(-1, -1, seqs.shape[2])) \
                if seqs.shape[2] else seqs.new_zeros(batch, keep, 0)
            seqs = torch.cat([prev, mv[:, :, None]], dim=2)
            if src is not None:                                            # [antiphon_p]
                # the SAME two gathers `mv` uses, applied to the source codes, so the carried
                # source is exactly the source of the move the beam kept.
                msrc = src.reshape(batch, width, kk).gather(
                    1, parent[:, :, None].expand(-1, -1, kk)).gather(
                    2, col[:, :, None]).squeeze(2)
                prev_s = srcs.gather(1, parent[:, :, None].expand(-1, -1, srcs.shape[2])) \
                    if srcs.shape[2] else srcs.new_zeros(batch, keep, 0)
                srcs = torch.cat([prev_s, msrc[:, :, None]], dim=2)
            width = keep
            if collect:
                hist_par.append(parent)
                hist_x.append(beams.clone())
        ...
        out = {"x": beams[rows, best], "seq": seqs[rows, best], "counts": counts,
               "tips_x": beams, "tips_seq": seqs}
        if srcs.shape[2] == seqs.shape[2] and seqs.shape[2]:               # [antiphon_p]
            out["tips_seq_src"] = srcs
            out["seq_src"] = srcs[rows, best]
        if collect:
            idx = torch.arange(width, device=device)[None, :].expand(batch, -1).contiguous()
            traj = [None] * (budget + 1)
            for t in range(budget, -1, -1):
...
def prop_pairs(out, roots, succ, slots, budget, train_on="solved", gate=None):
    ...
    # [antiphon_p] an extra (B, W) conjunct on the SAME selection step. `gate=None` is the
    # donor's behaviour byte for byte. Every kept trajectory contributes exactly `budget`
    # pairs, so matching the trajectory count matches the pair count exactly.
    if gate is not None:
        keep = keep & gate.to(keep.device).reshape(B, W)
...
ARMS = {
    ...
    # ---------------------------------------------------------------------------------------
    # [woodshed] THE REHEARSAL PAIR. Both are `exact` -- the same perfect table installed at
    # the same commit cycles -- plus extra targeted practice at the received level from its
    # arrival onward. They differ from each other in ONE BIT: whether pi imitates the
    # rehearsal trajectories that solved, or the same number of trajectories drawn at random
    # from the same episodes' survivors. Everything else (episodes, states, pair count,
    # gradient steps, RNG consumption) is identical by construction.
    # ---------------------------------------------------------------------------------------
    "exact_reh": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                  "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                          "reh_mode": "credit"}},
    "exact_exp": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                  "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                          "reh_mode": "expose"}},
    # [antiphon_p] THE PROVENANCE PAIR (P"). Both are `exact` + rehearsal at L3, and differ
    # in ONE BIT: whether the trajectories pi imitates are chosen BY PROVENANCE (every macro
    # in them was pi's own proposal) or by a uniform draw of the SAME SIZE from the same
    # solving set. Identical episodes, identical states, identical trajectory count and
    # therefore identical pair count, identical gradient steps.
    "exact_reh_s": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                    "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                            "reh_mode": "credit_self"}},
    "exact_reh_m": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                    "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                            "reh_mode": "credit_match"}},
    # [antiphon_p] pp_s1: the LEVEL-RESTRICTED gate. Only the REHEARSED level's macros must
    # be pi-proposed, and BOTH arms draw from the trajectories that actually applied that
    # level — so the vacuous class (no received-level macro at all, 21.7% of solves in
    # pp_s0) is excluded from treatment and control alike.
    "exact_rehL_s": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                     "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                             "reh_mode": "credit_self", "reh_gate": "level"}},
    "exact_rehL_m": {"vocab": "earned", "commit": "delta_prov", "prop_k": 4, "span": False,
                     "recert": True, "cfg": {"surgery": "exact", "reh_level": 3,
                                             "reh_mode": "credit_match",
                                             "reh_gate": "level"}},
...
TWIN = {
    ...
    # [woodshed] the rehearsal pair twins onto `spiral_route`'s stream exactly as every other
    # surgery arm does, so each differs from the in-tag anchor by the surgery + rehearsal and
    # by nothing else, and from each other by the credit filter alone.
    "exact_reh": "enum_live", "exact_exp": "enum_live",
    "exact_reh_s": "enum_live", "exact_reh_m": "enum_live",       # [antiphon_p]
    "exact_rehL_s": "enum_live", "exact_rehL_m": "enum_live",     # [antiphon_p]
}
...
# =========================================================================== #
# [antiphon_p] INSTRUMENT: the per-execution PROVENANCE JOIN (proposal x served entry)
# =========================================================================== #

# Source codes, in accumulator row order. `unknown` covers any macro execution reached from
# outside the proposal path (it must read 0 in the beam and probe phases, and the reduction
# checks that).
AP_SRC = ("prop", "forced", "explore", "unknown")

# On-device accumulator, keyed (phase, level) -> LongTensor (4, n_entries). `on` gates
# recording. Draws no RNG, enters no ledger, and is asserted inert by the fidelity gate.
_APREC = {"on": False, "phase": "beam", "acc": {},
          "forced": [], "explore_idx": None, "n_moves": None, "src_rows": None}


def _ap_reset():
    _APREC["acc"] = {}
    _APREC["forced"] = []
    _APREC["explore_idx"] = None
    _APREC["n_moves"] = None
    _APREC["src_rows"] = None


def _ap_take():
    """Pull the join accumulator to host once and clear it. One sync per cycle."""
    out = {}
    for (phase, lvl), t in _APREC["acc"].items():
        h = t.cpu().numpy()
        out.setdefault(phase, {})[str(lvl)] = {AP_SRC[i]: [int(x) for x in h[i]]
                                               for i in range(len(AP_SRC))}
    _ap_reset()
    return out


def select_moves_rec(plogits, k, forced=None, explore=None):
    """[antiphon_p] `PN.select_moves`, unchanged, plus a note of WHO is on the beam.

    Records the forced set and the action-set width for this expansion step and clears any
    stale exploration draw, so `expand_selected_rec` can label each selected move's source.
    Returns the donor's output object itself — no copy, no numeric touch."""
    out = _AP_ORIG["select_moves"](plogits, k, forced=forced, explore=explore)
    if _APREC["on"]:
        _APREC["forced"] = list(forced or [])
        _APREC["n_moves"] = int(plogits.shape[1])
        _APREC["explore_idx"] = None
    return out


def explore_moves_rec(sel_base, n_moves, n_explore, rng, device):
    """[antiphon_p] `PN.explore_moves`, unchanged, plus a note of which draws were exploratory.

    The donor excludes already-selected moves from the draw, so the proposed set and the
    explored set are DISJOINT — which is what lets the source be recovered by membership,
    without tracking the ascending re-sort's permutation."""
    out = _AP_ORIG["explore_moves"](sel_base, n_moves, n_explore, rng, device)
    if _APREC["on"]:
        _APREC["explore_idx"] = out
    return out


def expand_selected_rec(generator, flat, sel, ms, apply_any, rules_t, canon, depth, v, m, s):
    """[antiphon_p] A byte-identical copy of `PN.expand_selected` plus the source hand-off.

    Copied rather than wrapped because the source label must be in place around each
    `apply_any` call, and the donor's loop is where the (row, col) -> move correspondence
    exists. Everything outside the two marked lines is the donor's body verbatim; the P-gate
    in `preflight` asserts the two produce identical children and identical `n_mat`."""
    import torch
    n, kk = sel.shape
    out = torch.zeros(n, kk, flat.shape[1], dtype=flat.dtype, device=flat.device)
    n_mat = 0
    src = None
    if _APREC["on"]:                                                   # [antiphon_p]
        src = _ap_source_of(sel, len(ms))                              # [antiphon_p]
    for j in range(len(ms)):
        hit = sel == j
        if not bool(hit.any()):
            continue
        rows, cols = hit.nonzero(as_tuple=True)
        _APREC["src_rows"] = None if src is None else src[rows, cols]  # [antiphon_p]
        child = apply_any(generator, flat[rows], ms[j], rules_t, canon, depth, v, m, s)
        _APREC["src_rows"] = None                                      # [antiphon_p]
        out[rows, cols] = child
        n_mat += int(rows.numel())
    return out, n_mat


def _ap_source_of(sel, n_moves):
    """[antiphon_p] (N, k) source codes aligned with `sel`, by set membership.

    `select_moves` returns [top-k | forced] and the caller then concatenates the exploration
    draw and re-sorts ascending by move index. Because the three sets are disjoint, the source
    of `sel[i, c]` is a property of the MOVE INDEX in row i alone — so the permutation never
    has to be tracked, and the donor's line stays `.values` exactly as written."""
    import torch
    if _APREC["n_moves"] is None or int(_APREC["n_moves"]) != int(n_moves):
        return torch.full_like(sel, AP_SRC.index("unknown"))
    full = torch.zeros(sel.shape[0], n_moves, dtype=sel.dtype, device=sel.device)
    if _APREC["forced"]:
        full[:, list(_APREC["forced"])] = AP_SRC.index("forced")
    ei = _APREC["explore_idx"]
    if ei is not None and ei.shape[0] == sel.shape[0] and ei.shape[1]:
        full.scatter_(1, ei.to(sel.dtype),
                      torch.full_like(ei.to(sel.dtype), AP_SRC.index("explore")))
    return full.gather(1, sel)
...
def macro_features_rec(generator, x, move, s, v):
    ...
    # ---- [antiphon_p] the JOIN: the same served entry, binned by WHO put the move on the
    # beam. Same `best`, same tensor, one extra bincount; no RNG, no numeric effect. -------- #
    if _APREC["on"]:
        n = int(flat.shape[0])
        b = best.reshape(-1)
        sr = _APREC.get("src_rows")
        if sr is None or int(sr.numel()) != int(b.numel()):
            sr = torch.full_like(b, AP_SRC.index("unknown"))
        key = (_APREC["phase"], int(move["level"]))
        acc = _APREC["acc"].get(key)
        if acc is None or acc.shape[1] != n:
            acc = torch.zeros(len(AP_SRC), n, dtype=torch.long, device=x.device)
            _APREC["acc"][key] = acc
        acc.view(-1).index_add_(0, sr.reshape(-1).to(torch.long) * n + b,
                                torch.ones(b.numel(), dtype=torch.long, device=x.device))
    return flat[best.reshape(-1)].view(batch, span), pos


# [antiphon_p] the donor callables the wrappers stand in front of, captured at install time
_AP_ORIG = {}
...
def _install_entry_recorder():
    ...
    [antiphon_p] also swaps `PN.select_moves` / `PN.explore_moves` / `PN.expand_selected` for
    the note-taking copies above. `beam_moves_prop` resolves all three as module globals at
    call time, so this reaches the proposal path without touching `prop_net.py`, and all three
    return the donor's own values."""
    ...
    _AP_ORIG["select_moves"] = PN.select_moves               # [antiphon_p]
    _AP_ORIG["explore_moves"] = PN.explore_moves             # [antiphon_p]
    _AP_ORIG["expand_selected"] = PN.expand_selected         # [antiphon_p]
    PN.select_moves = select_moves_rec                       # [antiphon_p]
    PN.explore_moves = explore_moves_rec                     # [antiphon_p]
    PN.expand_selected = expand_selected_rec                 # [antiphon_p]
...
def entry_recorder_check(v=8, s=2, depth=6, m=2, max_level=3, n=48, rule_seed=0):
    ...
    for rec_on in (False, True):
        ...
        _rec_reset()
        _APREC["on"] = rec_on                                        # [antiphon_p]
        _ap_reset()                                                  # [antiphon_p]
        for mv in ms:
        ...
        if rec_on:
            ...
            # [antiphon_p] the join must total the same executions and, since these calls
            # bypass the proposal path entirely, must attribute all of them to `unknown`.
            jt = _ap_take()
            tot = {src: int(sum(sum(lv[l][src]) for lv in jt.values() for l in lv))
                   for src in AP_SRC}
            out["ap_join_total"] = int(sum(tot.values()))
            out["ap_join_by_src"] = tot
    _ENTRY_REC["on"] = False
    _rec_reset()
    _APREC["on"] = False                                             # [antiphon_p]
    _ap_reset()                                                      # [antiphon_p]
    ...
    # [antiphon_p] the join sees exactly the same executions as the entry recorder, and
    # attributes every one of these (non-beam, direct) calls to `unknown`.
    assert out["ap_join_total"] == want, \
        f"the join recorded {out['ap_join_total']} selections, expected {want}"
    assert out["ap_join_by_src"]["unknown"] == want, \
        f"direct macro_features calls must bin as `unknown`: {out['ap_join_by_src']}"
    out["verdict"] = "PASS"
    return out
...
def run_arm(label, base, overrides, shared, cfg, eras, refs, outdir, device):
    ...
    # [woodshed] rehearsal draws from its OWN streams so the arm-shared position is untouched
    # and the twin discipline still licenses every donor comparison in the tag.
    rehrng = np.random.default_rng(cfg["seed"] + 606_060)
    reh_erng = np.random.default_rng(cfg["seed"] + 707_070)
    ...
    for era_i, era in enumerate(eras):
        ...
        for c_in_era in range(1, ec + 1):
            ...
            _ap_reset()                                                 # [antiphon_p]
            _APREC["on"] = bool(cfg.get("ap_rec", True))                # [antiphon_p]
            _APREC["phase"] = "beam"                                    # [antiphon_p]
            if (cfg.get("preflight_seed_miner")
            ...
            pinfo, fallback, reh_rec = {"k": None}, False, None   # [woodshed] reh_rec
            if prop is not None:
                ...
                # --- (c'') [woodshed] REHEARSAL OF THE RECEIVED VOCABULARY -----------
                #     Extra practice at the received level's OWN damage cells, drawn across
                #     ALL of that level's nodes (varied demand, not the era's single node),
                #     run through the same beam at the same width, harvested by the same
                #     `prop_pairs` path as the ordinary cycle. It reaches pi's buffer and
                #     NOTHING else -- the miner, the value buffer and the generator never see
                #     a rehearsal solve -- so coverage and the plant are held fixed and the
                #     only thing that can move is trust.
                #
                #     `reh_mode` is the one-bit exposure-vs-credit control:
                #       credit  pi imitates the rehearsal trajectories that SOLVED
                #       expose  pi imitates the SAME NUMBER of trajectories drawn at random
                #               from the same episodes' survivors
                #     Identical episodes, identical states, identical pair count, identical
                #     gradient steps; the success filter is the only difference. The
                #     permutation is drawn in BOTH modes so RNG consumption matches too.
                #
                #     The pairs go in BEFORE `prop_train`, so this cycle's update already sees
                #     them, and NO extra gradient steps are taken: the rehearsal arms run the
                #     same `prop_steps` on the same `prop_batch` as every other arm. What
                #     changes is the composition of the buffer, which is the whole claim.
                rl = int(cfg.get("reh_level") or 0)
                if (rl and committed.get(rl) is not None
                        and cyc % max(1, int(cfg["reh_every"])) == 0):
                    # VARIED DEMAND MEANS ONE NODE PER INSTANCE, VARIED ACROSS INSTANCES --
                    # not every node at once. `corrupt_hier` damages EVERY node in the list
                    # it is handed (k = len(nodes), all k replaced in every instance), and
                    # `era_ctx` hands it a single-element list. `wd_s0` handed it all
                    # s**(depth-l) nodes, so every rehearsal instance carried that many
                    # SIMULTANEOUS level-l corruptions -- needing >= that many macro
                    # applications against a budget of exactly `budget`. Measured cost:
                    # rehearsal solve rate 0.003 against the main path's 0.29, i.e. the
                    # treatment was delivered at ~2% of its design dose. The batch is now
                    # split across nodes, one node per sub-batch, at the era's own difficulty.
                    n_nodes = s ** (depth - rl)
                    per = max(1, int(cfg["reh_n"]) // n_nodes)
                    rr_l, rx_l = [], []
                    for j_ in range(n_nodes):
                        rj, xj = context_instances(
                            rules, {"name": f"reh_L{rl}n{j_}", "level": rl, "nodes": [j_]},
                            per, s, depth, v, m,
                            seed=cfg["seed"] + 700_000 + 1000 * cyc + 37 * j_)
                        rr_l.append(rj)
                        rx_l.append(xj)
                    rr_np = np.concatenate(rr_l)
                    rx_np = np.concatenate(rx_l)
                    port_reh = (dict(port_pr, erng=reh_erng)
                                if isinstance(port_pr, dict) and "erng" in port_pr
                                else port_pr)
                    rout = plan(controller, generator, value, torch.from_numpy(rx_np),
                                torch.from_numpy(rr_np), ms, rules_t, canon, depth, v, m, s,
                                budget=cfg["budget"], beam_width=p_width, device=device,
                                collect=True, port=port_reh, ex=ex)
                    rtips = rout["tips_x"]
                    Br, Wr, Tr = rtips.shape
                    rsucc, _ = grade(rtips.reshape(Br * Wr, Tr).cpu().numpy(),
                                     np.repeat(rr_np, Wr), rules, s)
                    rslots = PN.move_slots(ms, offsets)
                    p_all = prop_pairs(rout, torch.from_numpy(rr_np), rsucc, rslots,
                                       cfg["budget"], train_on="tips")
                    p_cr = prop_pairs(rout, torch.from_numpy(rr_np), rsucc, rslots,
                                      cfg["budget"], train_on="solved")
                    n_all = 0 if p_all is None else int(p_all["x"].shape[0])
                    n_cr = 0 if p_cr is None else int(p_cr["x"].shape[0])
                    # ---- [antiphon_p] THE PROPOSAL-GRAIN PROVENANCE GATE ---------------
                    #  A rehearsal trajectory is PROVENANCE-SELF iff every level>=2 (macro)
                    #  move in its chosen action sequence came from pi's OWN top-k proposal:
                    #  none drawn by the exploration draw, none handed over by the forced-
                    #  expansion window. Level-1 primitives are exempt — they carry no table
                    #  entry, are never received vocabulary, and so have no provenance. A
                    #  solving trajectory with no macro at all passes vacuously (it used only
                    #  vocabulary the agent has always had) and is counted separately.
                    #  This is the arity-2 law as a filter: credit requires pi's own act in
                    #  the datum's causal history, read at the grain where the act happens.
                    #  Computed unconditionally — pure tensor ops, no RNG — so the dose
                    #  ledger exists on every mode.
                    dev_ = rout["tips_seq"].device
                    solved_m = torch.as_tensor(rsucc > 0.5, device=dev_).reshape(Br, Wr)
                    gate_m, prov_rec = None, None
                    if "tips_seq_src" in rout:
                        tsq, tsr = rout["tips_seq"], rout["tips_seq_src"]
                        lvl_t = torch.as_tensor([int(mv_.get("level", 1)) for mv_ in ms],
                                                dtype=torch.long, device=dev_)
                        is_macro = lvl_t[tsq] >= 2
                        is_prop = tsr == AP_SRC.index("prop")
                        gate_m = ((~is_macro) | is_prop).all(dim=2)
                        # [antiphon_p] a SECOND, looser variant, logged but NOT used as the
                        # gate: only the REHEARSED level's macros must be pi-proposed. Kept
                        # on the record because it is the natural alternative reading, and
                        # because its pass rate bounds how much of the strict gate's
                        # exclusion comes from the lower (also-gifted) level.
                        at_level = lvl_t[tsq] == rl
                        used_lv = at_level.any(dim=2)      # applied the RECEIVED level at all
                        gate_lv = used_lv & ((~at_level) | is_prop).all(dim=2)
                        n_sol = int(solved_m.sum())
                        prov_rec = {
                            "n_traj": int(Br * Wr), "n_solved": n_sol,
                            "n_self": int((solved_m & gate_m).sum()),
                            "n_nomacro": int((solved_m & ~is_macro.any(dim=2)).sum()),
                            "n_macro_appl": int((is_macro & solved_m[:, :, None]).sum()),
                            "n_macro_prop": int((is_macro & is_prop
                                                 & solved_m[:, :, None]).sum()),
                            "n_macro_forced": int(
                                (is_macro & (tsr == AP_SRC.index("forced"))
                                 & solved_m[:, :, None]).sum()),
                            "n_macro_explore": int(
                                (is_macro & (tsr == AP_SRC.index("explore"))
                                 & solved_m[:, :, None]).sum()),
                            "n_self_level": int((solved_m & gate_lv).sum()),
                            "n_eligible": int((solved_m & used_lv).sum()),
                            "n_no_atlevel": int((solved_m & ~used_lv).sum()),
                            "n_atlevel_appl": int((at_level & solved_m[:, :, None]).sum()),
                            "n_atlevel_prop": int((at_level & is_prop
                                                   & solved_m[:, :, None]).sum()),
                            "pass_rate": (float((solved_m & gate_m).sum()) / n_sol
                                          if n_sol else None),
                            "pass_rate_level": (float((solved_m & gate_lv).sum()) / n_sol
                                                if n_sol else None)}
                    perm = rehrng.permutation(max(n_all, 1))        # drawn in BOTH modes
                    use, gate_na = None, False
                    if cfg["reh_mode"] == "credit":
                        use = p_cr
                    elif cfg["reh_mode"] in ("credit_self", "credit_match"):
                        # [antiphon_p] the one-bit provenance pair. Both modes select a
                        # trajectory-level (B, W) mask of the SAME size out of the SAME
                        # solving set and hand it to the SAME `prop_pairs` call; the only
                        # difference is whether the mask is the provenance filter or a
                        # uniform draw. The draw is made in BOTH modes so RNG matches.
                        # The proposal grain only exists while pi is GATING the action set.
                        # Before `prop_warmup` expires the beam is the enumerated one, nobody
                        # "proposed" anything, and there is nothing to gate on — so the cycle
                        # is NOT consumed, and it is recorded and printed rather than being
                        # silently credited. At this round's configuration rehearsal starts at
                        # c71 and the filter has been on since c5, so this branch is dead in
                        # the round and live only in the quick smoke.
                        if gate_m is None:
                            gate_na = True
                            print(f"[reh] arm={arm} c{cyc} GATE UNAVAILABLE (port off) — "
                                  f"rehearsal NOT consumed this cycle", flush=True)
                        else:
                            # [antiphon_p] `reh_gate` picks the gate AND the pool the
                            # count-matched control draws from. "macro" is pp_s0's semantics
                            # verbatim; "level" restricts both sides to trajectories that
                            # actually applied the received level, so the vacuous class is
                            # removed from the treatment and the control alike.
                            if str(cfg.get("reh_gate", "macro")) == "level":
                                g_use, pool_m = gate_lv, (solved_m & used_lv)
                            else:
                                g_use, pool_m = gate_m, solved_m
                            sel_m = pool_m & g_use
                            n_traj_self = int(sel_m.sum())
                            cand = pool_m.nonzero(as_tuple=False)
                            pick = rehrng.permutation(max(int(cand.shape[0]), 1))
                            if cfg["reh_mode"] == "credit_match":
                                sel_m = torch.zeros_like(pool_m)
                                if int(cand.shape[0]) and n_traj_self:
                                    take = np.sort(pick[:min(n_traj_self,
                                                             int(cand.shape[0]))]).copy()
                                    cc = cand[torch.from_numpy(take).to(dev_)]
                                    sel_m[cc[:, 0], cc[:, 1]] = True
                            if int(sel_m.sum()):
                                use = prop_pairs(rout, torch.from_numpy(rr_np), rsucc,
                                                 rslots, cfg["budget"],
                                                 train_on="solved", gate=sel_m)
                            prov_rec["n_traj_used"] = int(sel_m.sum())
                    elif n_all and n_cr:
                        idx = torch.from_numpy(np.sort(perm[:min(n_cr, n_all)]).copy())
                        use = {k: p_all[k][idx] for k in ("x", "r", "a")}
                    n_used = 0 if use is None else int(use["x"].shape[0])
                    if n_used:
                        n_cap = cfg["prop_buf_cap"]
                        pbuf["x"] = torch.cat([pbuf["x"], use["x"]])[-n_cap:]
                        pbuf["r"] = torch.cat([pbuf["r"], use["r"]])[-n_cap:]
                        pbuf["a"] = torch.cat([pbuf["a"], use["a"]])[-n_cap:]
                    lo_l = offsets[rl]
                    hi_l = lo_l + s ** (depth - rl)
                    reh_rec = {
                        "level": rl, "mode": cfg["reh_mode"], "n_ep": int(cfg["reh_n"]),
                        "succ": float((rsucc > 0.5).mean()),
                        "n_pairs_all": n_all, "n_pairs_credit": n_cr, "n_pairs_used": n_used,
                        # what fraction of the rehearsal supervision actually points at a
                        # macro slot, and at the REHEARSED level's slots in particular. This
                        # is the direct readout of whether "exposure" carried any credit.
                        "frac_macro": (float((use["a"] >= offsets[2]).double().mean())
                                       if n_used else 0.0),
                        "frac_at_level": (float(((use["a"] >= lo_l) & (use["a"] < hi_l))
                                                .double().mean()) if n_used else 0.0),
                        # rehearsal groundings are recorded, never added to `counts`: this is
                        # extra practice COMPUTE, reported so the price is on the record, and
                        # it does not enter the arm's declared per-solve budget (which prices
                        # a beam width, not a cycle count).
                        "g": int(sum(rout["counts"].values())),
                        "pbuf_n": int(pbuf["x"].shape[0]),
                        "prov": prov_rec,                             # [antiphon_p]
                        "reh_gate": str(cfg.get("reh_gate", "macro")),  # [antiphon_p]
                        "gate_unavailable": bool(gate_na)}            # [antiphon_p]
                    # [woodshed] the DOSE is printed, not only logged. `wd_s0` delivered the
                    # treatment at ~2% of design and nothing on stdout said so -- the ledger
                    # lives in `results.json`, which is only readable after the arm finishes.
                    # A degenerate dose must be visible from the launch log, in flight.
                    print(f"[reh] arm={arm} c{cyc} L{rl} {cfg['reh_mode']:>6s} "
                          f"succ={reh_rec['succ']:.4f} offered={n_all} used={n_used} "
                          f"f_macro={reh_rec['frac_macro']:.3f} "
                          f"f_level={reh_rec['frac_at_level']:.3f}"
                          + ("" if prov_rec is None else
                             f" | prov pass={prov_rec['pass_rate']} "
                             f"passL={prov_rec['pass_rate_level']} "
                             f"self={prov_rec['n_self']}/{prov_rec['n_solved']} "
                             f"macro prop/forced/expl={prov_rec['n_macro_prop']}/"
                             f"{prov_rec['n_macro_forced']}/"
                             f"{prov_rec['n_macro_explore']}"), flush=True)
                    del rout, rtips
            ...
            log["reh"].append(reh_rec)                                # [woodshed]
            ...
            # [antiphon_p] the join, with the committed table's PLAIN FLAT KEYS beside it and
            # index-aligned, so the offline read is a lookup and the format is the same key
            # grain the node root's delivery ledger uses.
            _APREC["on"] = False
            apk = {}
            for ell in range(2, maxl + 1):
                if committed.get(ell) is None:
                    continue
                apk[str(ell)] = [[int(x) for x in r] for r in committed[ell]["flat"]]
            log["ap_join"].append({"hist": _ap_take(), "keys": apk})
...
def _cfg(**kw):
    cfg = dict(
        ...
        # --- [woodshed] the rehearsal knobs. `reh_level=None` is OFF and is the default on
        #     every donor arm, which is what makes this file a bit-identical fork.
        reh_level=None, reh_mode="credit", reh_n=64, reh_every=1,
...
def _d6_cfg(**kw):
    ...
    cfg.update(n_corrupt=1, budget=8, g_budget=482, max_macro_level=3,
               ...
               ap_rec=True, reh_gate="macro")                       # [antiphon_p]
...
# [woodshed] Arm ORDER is the same budget-risk decision the donor made. The in-tag fidelity
# carriers come first (`anchor`, `exact`, `given_c1` must each replay `as_s0`'s arm of the same
# name bit-for-bit, which is what licenses reading anything else in the tag); then the
# treatment; then its exposure control, which is the most cuttable because the treatment's
# own sign is readable without it.
WOODSHED_ARMS = "anchor,exact,given_c1,exact_reh,exact_exp"
# [antiphon_p] `wd_s1`'s arm set and ORDER, verbatim — the gate is a bit-identity replay of
# that tag, so the four arms and their positions have to match it exactly.
ANTIPHON_P_ARMS = "exact_reh,exact,exact_exp,given_c1"
# [antiphon_p] P": the provenance pair. The unchanged arms are NOT re-run — `ap_s0`'s banked
# `exact` is the twin these two must reproduce through the last pre-rehearsal cycle, and that
# twin gate is what licenses reading them against `ap_s0`'s `exact_reh` / `exact_exp`.
ANTIPHON_PP_ARMS = "exact_reh_s,exact_reh_m"
ANTIPHON_PPL_ARMS = "exact_rehL_s,exact_rehL_m"           # [antiphon_p] pp_s1


@app.function(volumes={DATA_DIR: volume}, gpu="L4", timeout=36000, memory=32768)
def antiphon_p_run(                                                     # [antiphon_p]
    tag: str = "ap_s0",
    arms: str = ANTIPHON_P_ARMS,
    ...
    ap_rec: bool = True,                                            # [antiphon_p]
    reh_n: int = 64, reh_every: int = 1,
    ref_tag: str = "", rule_seed: int = 0, train_seed: int = 1, quick: bool = False,
):
    """WOODSHED. Does rehearsal compress the trust clock that gift could not skip?

    Five arms at `as_s0`'s exact configuration and ladder, all routing-only:

      anchor      own mined table, no rehearsal        the floor, and a fidelity carrier
      exact       the full true table at commit time   late arrival, perfect content
      given_c1    the full true table at cycle 1       the ceiling (the clock already paid)
      exact_reh   `exact` + targeted rehearsal (CREDIT)      the treatment
      exact_exp   `exact` + targeted rehearsal (EXPOSE)      the exposure-vs-credit control

    `anchor`, `exact` and `given_c1` carry no rehearsal at all and must replay `as_s0`'s arms
    of the same name over all 116 cycles; `analyze_woodshed.py` asserts it. The pair
    `exact_reh` / `exact_exp` differs in one bit -- whether pi imitates the rehearsal
    trajectories that solved, or the same number drawn at random from the same episodes.

    The donor's own docstring follows.

    ASSAY. Which ingredient of the complete vocabulary does the deep-era bracket pay for?

    Six arms crossing amount x truth x arrival, all at `cs_s0`'s exact configuration and
    ladder, all routing-only:

      anchor      own mined table          the reference and the fidelity carrier
      complete    own U all missing true    amount up, truth up
      exact       the full true table       complete content, commit-time ARRIVAL
      junk_dose   own U K realistic junk    amount up, truth down (K = complete's addition count)
      given_c1    true tables from c1       the c1-arrival ceiling, in-tag
      strip       own minus its junk        truth up, amount down
    """
    import torch
    cfg = _d6_cfg(depth=depth, m=mm, n_corrupt=n_corrupt, prov_offset=prov_offset,
                  ...
                  ap_rec=ap_rec)                                    # [antiphon_p]
    # [woodshed] Rehearsal SIZE and CADENCE are run-level knobs; they are inert on any arm
    # whose `reh_level` is None, which is every donor arm. `reh_level` and `reh_mode` are
    # deliberately NOT exposed on the CLI: a run-level `reh_level` would silently switch
    # rehearsal on for the fidelity carriers too. They are declared per arm in `ARMS`, and
    # per-run they can still be moved with the donor's own `arm:key=value` override syntax
    # (e.g. `--arms "exact_reh:reh_level=2"`), which reaches exactly one arm.
    cfg.update(reh_n=int(reh_n), reh_every=int(reh_every))
    if quick:
    ...
    print(f"woodshed tag={tag} depth={cfg['depth']} m={cfg['m']} "
          f"seqlen={cfg['s'] ** cfg['depth']} arms={arms}\n"
          f"  ladder={[(e_['name'], e_.get('cycles') or cfg['era_cycles']) for e_ in ers]} "
          f"= {total_cycles} cycles/arm   entry_rec={entry_rec}   ap_rec={ap_rec}   "
    ...
    for label, base, ov in parse_arms(arms):
        ...
        print(f"[arm] {label}: {n} cycles at {summary['cycle_seconds'][label]:.1f} s/cycle; "
              ...
              f"surgery={summary['arms'][label]['surgery']}; "
...
def full_selfcheck(v=8, s=2, depth=4, m=2, rule_seed=0, n=64, max_level=3, budget=3,
    ...
    # [antiphon_p] P-7: the forked `expand_selected` must reproduce the DONOR's children and
    # `n_mat` exactly, with the join recorder both OFF and ON. `_AP_ORIG` holds the donor
    # callable captured at install time, so this compares copy against original, not copy
    # against itself.
    donor_es = _AP_ORIG.get("expand_selected", PN.expand_selected)
    sel7 = PN.select_moves(rnd, 3)
    ref7, nm7 = donor_es(gen, flat, sel7, ms, MC.apply_any, rules_t, canon, depth, v, m, s)
    out["P7_expand_maxabs"], out["P7_n_mat_delta"] = 0.0, 0
    for rec_on in (False, True):
        _APREC["on"] = rec_on
        _ap_reset()
        got7, nmg = expand_selected_rec(gen, flat, sel7, ms, MC.apply_any, rules_t, canon,
                                        depth, v, m, s)
        out["P7_expand_maxabs"] = max(out["P7_expand_maxabs"],
                                      float((got7.long() - ref7.long()).abs().max()))
        out["P7_n_mat_delta"] = max(out["P7_n_mat_delta"], abs(int(nmg) - int(nm7)))
    _APREC["on"] = False
    _ap_reset()
    assert out["P7_expand_maxabs"] == 0.0 and out["P7_n_mat_delta"] == 0, "P-7 failed"
...
def preflight(cycles: int = 2, eras: str = "1:25:3,2:12:4,3:6:2,4:3:2,5:1:2",
    ...
    for arm in ("anchor", "complete", "exact", "junk_dose", "strip", "given_c1",
                "given_c1_j", "exact_j", "spiral_route", "given_native"):
    ...
    # ---- [assay] THE BURN MUST ACTUALLY DISPLACE ---------------------------------------
    # A burn that lands in a stream nothing consumes produces a bit-identical twin and
    # measures nothing. Asserted as NON-identity, as early in the run as the burn can reach:
    # the plant's masking is the first consumer, at block (c) of cycle 1, so the metering `e`
    # at block (d) of cycle 1 is the earliest observable. The first differing cycle is
    # reported so a burn that only bites late would be visible rather than silently passing.
    ok["burn_divergence"] = {}
    for twin, orig in (("given_c1_j", "given_c1"), ("exact_j", "exact")):
        tj = json.load(open(f"{outdir}/{twin}/results.json"))
        oj = json.load(open(f"{outdir}/{orig}/results.json"))
        assert (tj["stream_burn"]["draws"] > 0 and tj["stream_burn"]["cpu"]), \
            f"{twin} recorded no burn"
        assert oj["stream_burn"]["draws"] == 0, f"{orig} was burned and should not have been"
        first = None
        per = {}
        for k in ("e", "gloss", "succ", "n_solved", "vloss"):
            x = np.asarray(tj["log"][k], float); y = np.asarray(oj["log"][k], float)
            n = min(len(x), len(y))
            d = np.abs(x[:n] - y[:n])
            per[k] = float(d.max())
            nz = np.nonzero(d)[0]
            if nz.size:
                first = nz[0] + 1 if first is None else min(first, int(nz[0]) + 1)
        ok["burn_divergence"][twin] = {"vs": orig, "first_differing_cycle": first,
                                       "max_abs_delta": per,
                                       "burn": tj["stream_burn"]}
        assert first is not None, (
            f"{twin} is BIT-IDENTICAL to {orig} — the burn landed in a stream nothing "
            f"consumes and would measure nothing")
        assert first <= 3, (
            f"{twin} does not diverge from {orig} until c{first}; the burn should bite at "
            f"the first plant fine-tune")
    ok["gy_observed"] = {a_: json.load(open(f"{outdir}/{a_}/results.json"))["gy_final"]["n_obs"]
                         for a_ in ("anchor", "complete", "exact", "junk_dose", "given_c1")}
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    """
    import torch
    from rhm.practice.census import census as SP
    ...
    torch.set_float32_matmul_precision("high")
    started = time.time()
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} — assay vs census",
          flush=True)
    ...
    SERIES = ["e", "succ", "dres", "t_cum", "n_moves", "width", "g_per_solve", "e_practice",
              "vloss", "gloss", "n_solved", "n_mined", "m_per_solve"]
    rows, worst_fork, worst_ctrl = [], 0.0, 0.0
    for arm in ("spiral_route", "given_route"):
    ...
    assert out["events_equal"], "G-F FAILED: commit events differ from census.py"
    assert worst_fork <= worst_ctrl, (
        f"G-F FAILED: the assay fork perturbed census.py's code path "
        f"({worst_fork:.3e} > self-replay control {worst_ctrl:.3e})")
    print("G-F PASS — with the surgery off, this fork is a replay of census.py.")
    return out
...
def main(quick: bool = True):
    antiphon_p_run.remote(quick=quick)
