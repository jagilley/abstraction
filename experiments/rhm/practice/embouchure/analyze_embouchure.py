...
# [embouchure] THIS fork's direct donor's fetched copies: `if_q1c_yk` is the banked design this
# round re-runs one arm short of, and `given_rule_s` / `leaf_s` are read from THERE rather than
# re-run (the coordinator's decision, 2026-09-10).
IF_FIG = os.path.join(os.path.dirname(HERE), "inflection", "figures")
...
def main():
    ...
    ap.add_argument("--tag", default="em_q1")             # [embouchure]
    ap.add_argument("--gf-tag", default="em_gf")          # [embouchure]
    ...
    ap.add_argument("--merge-tag", default="",
                    help="one or more comma-separated tags whose arms are merged into this "
                         "reduction, each after asserting it trained the SAME substrate and "
                         "that every arm it shares with this tag is bit-identical")
    # [embouchure/Q2.2] the READER-B tag is a MODIFIED SUBSTRATE and its arms must never be
    # merged with a reader-A tag's. `--vs-tag` is therefore NOT `--merge-tag`: it loads the
    # other tag into its own dict, prints the cross-tag `canon_s` comparison as a MEASUREMENT
    # rather than an assertion — that comparison is expected to fail, and the failure is the
    # record of the substrate change — and then prints the same E-sections for it, beside
    # this tag's, under its own banner. Nothing is pooled and no arm crosses over.
    ap.add_argument("--vs-tag", default="",
                    help="one or more comma-separated tags to print BESIDE this one (never "
                         "merged): the substrate-change record for Q2.2")
    ...
    if a.fetch:
        ...
        for _mt in ([x.strip() for x in (a.merge_tag or "").split(",") if x.strip()]
                    + [x.strip() for x in (a.vs_tag or "").split(",") if x.strip()]):
            try:
                fetch(_mt)
            except subprocess.CalledProcessError:
                print(f"[warn] no {_mt} on the volume")
    root = os.path.join(FIG, a.tag)
    setup, summary = _load(root, "setup.json"), _load(root, "summary.json")
    _substrate_banner(a.tag, setup)                              # [embouchure/Q2.2]
    ...
    # [embouchure] `--merge-tag` takes a COMMA-SEPARATED LIST. The round has three tags —
    # `em_q1` (the corner), `em_q1c` (the objective axis) and `em_q1d` (the keying axis) — and
    # each carries `canon_s`, so each merge re-runs the substrate assertion and the cross-tag
    # bit-identity check on every arm the two tags share. A tag that fails either is refused.
    for _mt in [x.strip() for x in (a.merge_tag or "").split(",") if x.strip()]:
            m_root = os.path.join(FIG, _mt)
            m_setup, m_summary = _load(m_root, "setup.json"), _load(m_root, "summary.json")
            if m_summary is None:
                print(f"[warn] {_mt} not present locally — twins not merged")
            else:
                ...
                print(f"\n[merge] {_mt} -> {a.tag}: refs identical = {not bad}"
                      f"{'' if not bad else ' (differ: ' + str(bad) + ')'}, "
                      f"stale buffer identical = {same_buf}")
                assert not bad and same_buf, (
                    f"{_mt} did not train the same substrate as {a.tag} — the twins are "
                    f"not comparable to their originals")
                # [embouchure] THE CROSS-TAG ANCHOR. `em_q1` and `em_q1b` are ONE design run on
                # two L4s, and what makes that true is that the arm they share is bit-identical:
                # `canon_s` takes `enum_live`'s stream in both, on a substrate the assertions above
                # have already shown is the same object. If any logged series differs the two tags
                # are two experiments and no cross-tag rank means anything.
                shared_arms = [x for x in m_summary["order"] if x in A]
                for arm in shared_arms:
                    ...
                    worst, wk = 0.0, None
                    for k in GF_SERIES:
                        x = np.asarray(A[arm]["log"][k], float)
                        y = np.asarray(B["log"][k], float)
                        L = min(len(x), len(y))
                        d = float(np.abs(x[:L] - y[:L]).max()) if L else float("nan")
                        if d > worst:
                            worst, wk = d, k
                    print(f"[cross-tag] {arm}: max|{a.tag} - {_mt}| = {worst:.3e}"
                          + (f" (worst series {wk})" if wk else "")
                          + f" over {min(len(A[arm]['log']['e']), len(B['log']['e']))} cycles")
                    assert worst == 0.0, (
                        f"CROSS-TAG FAILED: {arm} differs between {a.tag} and {_mt} "
                        f"({worst:.3e} on {wk}) — the two tags are not one design")
                added = []
                for arm in m_summary["order"]:
                    fp = os.path.join(m_root, arm, "results.json")
                    if not os.path.isfile(fp) or arm in A:
                        continue
    ...
    # [embouchure] this fork's own sections, last again.
    own_twin(a.tag, A, setup, summary)
    own_time_course(a.tag, A, setup, summary)
    own_misphrase(a.tag, A, setup, summary)
    own_miskey(a.tag, A, setup, summary)
    shared_cell_K(a.tag, A, setup, summary)
    e_floor_in_tag(a.tag, A, setup, summary)
    mined_precision_two_spaces(a.tag, A, setup, summary)
    own_cell_counts(a.tag, A, setup, summary)                    # [embouchure/Q2.2]
    gen_ear_drift(a.tag, A, setup, summary)                      # [embouchure/Q2.2]
    own_volume(a.tag, A, setup, summary)
    for _vt in [x.strip() for x in (a.vs_tag or "").split(",") if x.strip()]:
        vs_tag_beside(a.tag, A, setup, summary, _vt)             # [embouchure/Q2.2]
    for _rbk, _rbl in (("readback_flip", ""),
                       ("readback_flip_derived", "  [scored against the DERIVED feature, "
                                                 "Q2.2]")):
        _rb = (setup or {}).get(_rbk)
        if _rb and _rbk == "readback_flip_derived":
            print(f"\n=== [E] GATE RB-1 (derived truth){_rbl} ===")
            print(f"  pooled: {json.dumps(_rb['pooled'])}")
    rb = (setup or {}).get("readback_flip")
    if rb:
        print("\n=== [E] GATE RB-1  the CONTROLLED read-back: flip one block to the other "
              "synonym of its own feature and re-read ===")
        print(f"  pooled: {json.dumps(rb['pooled'])}")
        for c_, r_ in sorted(rb["by_register"].items(), key=lambda kv: int(kv[0])):
            print(f"    rho={c_} practised={r_['practiced']!s:5s} n={r_['n']:4d}  "
                  f"flipped read changed {r_['flipped_read_changed']}  "
                  f"wrong {r_['flipped_read_wrong']}  "
                  f"collateral {r_['collateral_read_changed']:.5f}")
    if a.figures:
...
# (the entrypoint guard lives at the end of the file)


# =========================================================================== #
# [embouchure] THE PRODUCTION ROUTE — the three sections this node adds
# =========================================================================== #

OWN_SIGNALS = ("own_scalar", "own_readback")            # [embouchure]


def _own_arms(A):
    return sorted(k for k in A
                  if str(A[k].get("fit_signal") or "surface") in OWN_SIGNALS)


def _substrate_banner(tag, setup):                             # [embouchure/Q2.2]
    """The first thing the reduction prints, when and only when the tag CHANGED THE SUBSTRATE.

    `setup.json`'s first key. Silent on every reader-A tag, so no banked reduction moves."""
    sm = (setup or {}).get("substrate_mod")
    if not sm:
        return
    print("\n" + "!" * 78)
    print(f"!!  {tag}: MODIFIED SUBSTRATE — {sm.get('what')} = {sm.get('value')}")
    for e in sm.get("ears_moved") or []:
        print(f"!!  EAR MOVED: {e}")
    print(f"!!  {sm.get('means')}")
    print(f"!!  {sm.get('do_not')}")
    print(f"!!  unchanged: {'; '.join(sm.get('unchanged') or [])}")
    print("!" * 78)


def _loser_cells(setup):                                       # [embouchure/Q2.2]
    """The (feature, register) cells on a shared form whose feature is NOT the one the
    last-writer table keeps — the owner the arc's own reader can never return. `cells` and
    `shared_forms` are both in `setup.json`, so this is a lookup and not a derivation."""
    lex = (setup or {}).get("lexicon") or {}
    keep = {}
    for code, rec in (lex.get("shared_forms") or {}).items():
        for f_, _k in rec.get("owners") or []:
            keep[(int(f_), int(code))] = int(rec.get("last_writer_keeps"))
    winners = {int(r.get("last_writer_keeps")) for r in (lex.get("shared_forms") or {}).values()}
    owners_by_f = {}
    for code, rec in (lex.get("shared_forms") or {}).items():
        for f_, _k in rec.get("owners") or []:
            owners_by_f[int(f_)] = int(rec.get("last_writer_keeps"))
    out = set()
    for f_, r_ in (lex.get("cells") or []):
        if int(f_) in owners_by_f and owners_by_f[int(f_)] != int(f_):
            out.add((int(f_), int(r_)))
    return out, winners


def own_cell_counts(tag, A, setup, summary, arms=("own_readback_s", "own_scalar_rec_s")):
    """[E] §9E — THE LOSER CELLS' OWN-WRITE COUNTS, the readout Q2.2 was built for.

    Q2.1 (reader A): at the practised registers the loser owner of each shared form is all
    but absent from the learner's own intents — f0@3 68 writes against f1's 2,883, f2@3 1,
    f5@3 2 in `own_readback_s`. Whether that is the ear's doing is what a reader-B tag
    answers, and the answer is these same counts. Printed per arm, per cell, with the
    winner's count beside it, and with the practised registers marked.
    """
    lose, _ = _loser_cells(setup)
    if not lose:
        return
    lex = (setup or {}).get("lexicon") or {}
    part = {}
    for code, rec in (lex.get("shared_forms") or {}).items():
        w = int(rec.get("last_writer_keeps"))
        for f_, _k in rec.get("owners") or []:
            if int(f_) != w:
                part[int(f_)] = w
    prac = set(int(x) for x in (setup.get("practiced") or []))
    print("\n=== [E] §9E  the LOSER cells' own-write counts (winner beside, practised "
          "registers starred) ===")
    print(f"  loser cells: {sorted(lose)}")
    for arm in arms:
        if arm not in A:
            continue
        cells = _cells_pooled(A[arm])
        if not cells:
            continue
        print(f"\n  {arm}")
        print("    cell        n(loser)   n(winner)   loser share   off-rule(loser)")
        for f_, r_ in sorted(lose):
            nl = cells.get((f_, r_), {}).get("n", 0)
            nw = cells.get((part[f_], r_), {}).get("n", 0)
            orl = cells.get((f_, r_), {}).get("offrule")
            tot = nl + nw
            star = "*" if r_ in prac else " "
            print(f"    f{f_}:r{r_}{star}  {nl:9d}   {nw:9d}   "
                  f"{(nl / tot if tot else float('nan')):11.5f}   "
                  f"{'   —  ' if orl is None else f'{orl:6.4f}'}")
        _pl = sum(cells.get((f_, r_), {}).get("n", 0) for f_, r_ in lose if r_ in prac)
        _pw = sum(cells.get((part[f_], r_), {}).get("n", 0) for f_, r_ in lose if r_ in prac)
        print(f"    POOLED over practised loser cells: loser {_pl}  winner {_pw}  "
              f"loser share {(_pl / (_pl + _pw) if (_pl + _pw) else float('nan')):.5f}")


def _cells_pooled(rec):                                        # [embouchure/Q2.2]
    """`cell_n` / `cell_offrule` / `cell_misread` summed over the whole ladder, keyed by
    (intended feature, register). These are the in-run tallies `cell_tally` writes — COUNTS
    in a (v, n_ctx) nested list, the same three arrays `_cell_table` prints — pooled here and
    nothing re-derived."""
    rows = [r for r in ((rec.get("log") or {}).get("own") or [])
            if isinstance(r, dict) and "cell_n" in r]
    if not rows:
        return {}
    cn = np.sum([np.asarray(r["cell_n"], float) for r in rows], axis=0)
    co = np.sum([np.asarray(r["cell_offrule"], float) for r in rows], axis=0)
    cm = np.sum([np.asarray(r["cell_misread"], float) for r in rows], axis=0)
    out = {}
    for f in range(cn.shape[0]):
        for r in range(cn.shape[1]):
            n = int(cn[f, r])
            out[(f, r)] = {"n": n,
                           "offrule": (co[f, r] / n) if n else None,
                           "misread": (cm[f, r] / n) if n else None}
    return out


def gen_ear_drift(tag, A, setup, summary):                     # [embouchure/Q2.2]
    """[E] §11E — THE CHOOSER'S EAR OVER THE RUN, at the loser cells.

    Only the `both` tag has anything here. `plant_probe` records, at every `probe_every`
    cycle and on the blocks where the two teachers disagree (which ARE the loser-cell blocks),
    what the GENERATOR's parse returns: the derived feature the world meant, or the
    last-writer one. The generator's setup supervision is the derived features and its in-run
    target is `bottom_map`, so this is the drift back — measured, not assumed (DESIGN.md §12).
    The frozen reader's own numbers sit beside it and must NOT move.
    """
    sm = (setup or {}).get("substrate_mod") or {}
    rows_any = False
    for arm in summary["order"]:
        pr = [p for p in ((A[arm].get("log") or {}).get("probe") or [])
              if isinstance(p, dict) and isinstance(p.get("plant"), dict)
              and p["plant"].get("loser_cells")]
        if not pr:
            continue
        if not rows_any:
            print("\n=== [E] §11E  the CHOOSER's ear at the loser cells, over the run ===")
            print(f"  reader_target = {sm.get('value', 'last_writer')}; the generator's setup "
                  f"supervision is the derived features only at `both`, and its IN-RUN target "
                  f"is `bottom_map` in every tag")
            rows_any = True
        print(f"\n  {arm}")
        print("     cyc      n   gen->derived  gen->last_writer   read->derived  "
              "read->last_writer")
        for i, p in enumerate(pr):
            lc = p["plant"]["loser_cells"]
            c = p.get("cycle", i)
            f = lambda k: ("   —   " if lc.get(k) is None else f"{lc[k]:7.4f}")
            print(f"    {c:4}  {lc.get('n', 0):5d}      {f('gen_derived')}         "
                  f"{f('gen_last_writer')}        {f('read_derived')}          "
                  f"{f('read_last_writer')}")
        first, last = pr[0]["plant"]["loser_cells"], pr[-1]["plant"]["loser_cells"]
        if first.get("gen_derived") is not None and last.get("gen_derived") is not None:
            print(f"    DRIFT over the run: gen->derived {first['gen_derived']:.4f} -> "
                  f"{last['gen_derived']:.4f} "
                  f"({last['gen_derived'] - first['gen_derived']:+.4f}); "
                  f"gen->last_writer {first['gen_last_writer']:.4f} -> "
                  f"{last['gen_last_writer']:.4f} "
                  f"({last['gen_last_writer'] - first['gen_last_writer']:+.4f})")


def vs_tag_beside(tag, A, setup, summary, vs):                 # [embouchure/Q2.2]
    """[E] §10E — THE OTHER TAG, BESIDE THIS ONE AND NEVER MERGED INTO IT.

    `--merge-tag` pools arms and asserts bit-identity; this does neither. It prints the
    cross-tag `canon_s` comparison as a measurement — on a reader-B tag it is EXPECTED to
    differ, and the size of the difference is the record of the substrate change — and then
    re-runs the same E-sections on the other tag under its own banner, so every readout can
    be read column for column without a single number crossing between the two dicts.
    """
    m_root = os.path.join(FIG, vs)
    m_setup, m_summary = _load(m_root, "setup.json"), _load(m_root, "summary.json")
    if m_summary is None:
        print(f"\n[vs] {vs} not present locally — nothing printed beside {tag}")
        return
    B = {arm: json.load(open(os.path.join(m_root, arm, "results.json")))
         for arm in m_summary["order"]
         if os.path.isfile(os.path.join(m_root, arm, "results.json"))}
    m_summary["order"] = [x for x in m_summary["order"] if x in B]
    print("\n" + "=" * 78)
    print(f"=== [E] §10E  {vs} BESIDE {tag} — printed, never merged ===")
    print("=" * 78)
    sm_a = (setup or {}).get("substrate_mod")
    sm_b = (m_setup or {}).get("substrate_mod")
    print(f"  {tag:10s} substrate_mod: "
          f"{'none (the arc''s own)' if not sm_a else sm_a.get('what') + '=' + str(sm_a.get('value'))}")
    print(f"  {vs:10s} substrate_mod: "
          f"{'none (the arc''s own)' if not sm_b else sm_b.get('what') + '=' + str(sm_b.get('value'))}")
    # the world, which must NOT have moved: same lexicon, same rule, same practised set
    for k in ("merge", "n_forms", "n_shared_forms"):
        va = ((setup or {}).get("lexicon") or {}).get(k)
        vb = ((m_setup or {}).get("lexicon") or {}).get(k)
        print(f"  lexicon.{k:16s} {tag}: {va!r}   {vs}: {vb!r}   {'SAME' if va == vb else 'DIFFER'}")
    same_world = (((setup or {}).get("lexicon") or {}).get("merge")
                  == ((m_setup or {}).get("lexicon") or {}).get("merge")
                  and (setup or {}).get("practiced") == (m_setup or {}).get("practiced"))
    print(f"  -> the WORLD is {'the same object' if same_world else 'NOT the same object'}; "
          f"what differs is the learner's ear")
    # THE CROSS-TAG canon_s COMPARISON, measured and not asserted.
    print(f"\n  the cross-tag `canon_s` check, MEASURED (on a reader-B tag it is expected to "
          f"fail;\n  the failure IS the record that the substrate changed):")
    for arm in [x for x in m_summary["order"] if x in A]:
        worst, wk, div = 0.0, None, None
        for k in GF_SERIES:
            x = np.asarray(A[arm]["log"].get(k) or [], float)
            y = np.asarray(B[arm]["log"].get(k) or [], float)
            L = min(len(x), len(y))
            if not L:
                continue
            d = float(np.abs(x[:L] - y[:L]).max())
            if d > worst:
                worst, wk = d, k
            nz = np.flatnonzero(np.abs(x[:L] - y[:L]) > 0)
            if nz.size and (div is None or int(nz[0]) < div):
                div = int(nz[0])
        verdict = ("IDENTICAL" if worst == 0.0 else
                   f"DIFFERS from cycle {div} (0-based) onward")
        print(f"    {arm:18s} max|{tag} - {vs}| = {worst:.3e}"
              + (f" on `{wk}`" if wk else "") + f"   -> {verdict}")
    print(f"\n  --- the same E-sections, on {vs} ---")
    _substrate_banner(vs, m_setup)
    own_misphrase(vs, B, m_setup, m_summary)
    own_miskey(vs, B, m_setup, m_summary)
    shared_cell_K(vs, B, m_setup, m_summary)
    e_floor_in_tag(vs, B, m_setup, m_summary)
    mined_precision_two_spaces(vs, B, m_setup, m_summary)
    own_cell_counts(vs, B, m_setup, m_summary)
    gen_ear_drift(vs, B, m_setup, m_summary)
    own_volume(vs, B, m_setup, m_summary)
    rb = (m_setup or {}).get("readback_flip")
    if rb:
        print(f"\n  [{vs}] GATE RB-1 pooled: {json.dumps(rb['pooled'])}")


def own_twin(tag, A, setup, summary):
    """§1E — THE IN-TAG TWIN GATE. Before its first fit `own_scalar`'s `Khat` is None and the
    Renderer's `fit` branch writes synonym 0, which IS `canon`. So the two arms must be
    bit-identical until the fit first moves a write, and `log["ans_hash"]` (a crc32 of the
    practice beam's answer, logged in every ruled arm and consumed by nothing) makes that a
    measured cycle rather than an argument — Q3's `_sp`-pair idiom, on a different pair.

    Also reported: EB-1 / EB-2's whole-arm tallies, which were asserted every cycle in the run.
    """
    own = _own_arms(A)
    if not own or "canon_s" not in A:
        return
    print("\n=== [E] §1E  the in-tag twin gate: own_scalar renders at canon until its "
          "first fit ===")
    ch = A["canon_s"]["log"].get("ans_hash") or []
    for arm in own:
        oh = A[arm]["log"].get("ans_hash") or []
        n = min(len(ch), len(oh))
        div = next((i + 1 for i in range(n) if ch[i] != oh[i]), None)
        same = sum(1 for i in range(n) if ch[i] == oh[i])
        g = A[arm].get("own_gate") or {}
        print(f"  {arm:16s} vs canon_s: first divergence at cycle {div}  "
              f"({same}/{n} cycles identical)")
        print(f"  {'':16s} EB-1 mismatches {g.get('eb1_mismatch')} / EB-2 "
              f"{g.get('eb2_scored_full')} over {g.get('n_cycles')} cycles; "
              f"{g.get('n_cells')} labelled cells")
        if g.get("blk_readback"):
            print(f"  {'':16s} read-back ledger: {g['blk_readback']} blocks read, "
                  f"{g['blk_readback_used']} used as labels, priced={g.get('priced')}; "
                  f"{g.get('n_ambiguous', 0)} cells ambiguous by recall")


def own_time_course(tag, A, setup, summary):
    """§2E — THE READOUT THAT DECIDES Q2: the time course. `own_scalar`'s written spelling
    error and recovered theta against `surface`'s, cycle by cycle, and the cycle at which each
    first reaches (and holds) Q0's endpoint.

    Q0's endpoint for this world, head class and practised subset: `acc_practiced` 1.0000,
    `acc_heldout` 0.8750, `theta_hat` [2,2,5,5,5,5,5,2] (`../embouchure/FILES.md` §Q0.1). The
    question this section answers is not WHETHER the production route gets there — Q0 showed
    the per-block channel does, in the same cycle the surface does — but whether the
    BAG-LEVEL one does, and how long it takes, because the ladder advances on its own clock and
    a renderer that converges before the seat is live cannot be a signal at the pacer's
    timescale (`inflection` finding 7).
    """
    arms = [k for k in A if (A[k]["log"].get("render") or [])
            and any(isinstance(r, dict) and "acc_heldout" in r for r in A[k]["log"]["render"])]
    if not arms:
        return
    print("\n=== [E] §2E  the time course: written spelling error and theta-hat by cycle ===")
    marks = [1, 2, 5, 10, 20, 40, 60, 85, 100, 120, 140]
    for arm in sorted(arms):
        log = A[arm]["log"]
        R = log["render"]
        sp = [r.get("spell_err") if isinstance(r, dict) else None for r in log["spell"]]
        sig = A[arm].get("fit_signal") or "surface"
        print(f"\n  {arm}  (fit_signal={sig})")
        print(f"    {'cyc':>4s} {'era':>3s} {'n_buf':>7s} {'wr_err':>7s} {'e_sp_prac':>9s} "
              f"{'acc_prac':>8s} {'acc_held':>8s} {'det_th':>6s}  theta_hat")
        for i, r in enumerate(R):
            c = int(log["cycle"][i])
            if c not in marks or not isinstance(r, dict) or "acc_heldout" not in r:
                continue
            print(f"    {c:4d} {int(log['era'][i]):3d} {int(r.get('n') or 0):7d} "
                  f"{(sp[i] if sp[i] is not None else float('nan')):7.4f} "
                  f"{float(log['e_sp_practice'][i]):9.5f} "
                  f"{r['acc_practiced']:8.4f} {r['acc_heldout']:8.4f} "
                  f"{r.get('det_theta', float('nan')):6.3f}  {r.get('theta_hat')}")
        # first cycle at the endpoint, and held to the end
        acc = [(int(log["cycle"][i]), r["acc_heldout"]) for i, r in enumerate(R)
               if isinstance(r, dict) and "acc_heldout" in r]
        if acc:
            end = acc[-1][1]
            first = next((c for j, (c, _) in enumerate(acc)
                          if all(x >= end - 1e-9 for _, x in acc[j:])), None)
            lifer = [x for x in sp if x is not None]
            print(f"    endpoint acc_heldout {end:.4f}, first reached and held at cycle "
                  f"{first}; lifetime written spelling error "
                  f"{float(np.mean(lifer)) if lifer else float('nan'):.4f}")


def own_misphrase(tag, A, setup, summary):
    """§4E — THE MIS-PHRASING RATE: own written blocks whose read-back is not the intent, per
    arm over the ladder, and the read-back confusion per (feature, register) at the end.

    This is gate RB-1's quantity measured on the arms' own writes rather than on a controlled
    flip, and it is the readout the retraction route learns from. It is logged in every own
    arm (`log["own"]`), so `own_scalar` — which has no reader in its LABEL — still reports the
    rate its writes would have been graded at.
    """
    own = _own_arms(A)
    if not own:
        return
    print("\n=== [E] §4E  mis-phrasing: own blocks whose read-back is not the intent ===")
    for arm in own:
        log = A[arm]["log"]
        rows = [(i, r) for i, r in enumerate(log.get("own") or []) if isinstance(r, dict)]
        if not rows or "misphrase_rate" not in rows[0][1]:
            # `own_scalar` logs the same quantity under its own name
            rr = [(i, r) for i, r in rows if "readback_drop" in r]
            if rr:
                era = np.asarray(log["era"], int)
                print(f"\n  {arm:16s} (read-back DROP, the `agood` form) by era: "
                      + "  ".join(f"era{e} "
                                  f"{np.mean([r['readback_drop'] for i, r in rr if era[i] == e]):.5f}"
                                  for e in sorted(set(era.tolist()))))
                _cell_table(arm, [r for i, r in rr], setup)
            continue
        era = np.asarray(log["era"], int)
        print(f"\n  {arm}")
        print(f"    {'era':>3s} {'cycles':>6s} {'misphrase':>9s} {'ambiguous':>9s} "
              f"{'cells/cyc':>9s} {'reader agrees':>13s}")
        for e in sorted(set(era.tolist())):
            sel = [r for i, r in rows if era[i] == e]
            if not sel:
                continue
            g = lambda k: float(np.mean([r[k] for r in sel if r.get(k) is not None]))
            print(f"    {e:3d} {len(sel):6d} {g('misphrase_rate'):9.5f} "
                  f"{np.mean([r['n_ambiguous'] for r in sel]):9.2f} "
                  f"{g('n_cells'):9.1f} {g('cells_reader_agrees'):13.5f}")
        _cell_table(arm, [r for i, r in rows], setup)


def _cell_table(arm, rows, setup):
    """[embouchure] DECISION 3 (2026-09-11): the two instruments SIDE BY SIDE on the same
    blocks, per (feature, register).

    `off` is the grader's: the synonym written is not the one the rule calls for. `mis` is the
    reader's: the read-back feature is not the one intended. Under homophony they come apart —
    a renderer that departs from the rule in order to be HEARD correctly scores on `off` and
    not on `mis`, and one that keeps the rule's synonym and is misheard scores on `mis` and not
    on `off`. Summing them would hide exactly the thing Q2 is for, so they are never summed.

    (`mis` is identically 0 for `own_scalar`, whose cells are keyed on the reader's own parse;
    that is a property of the arm and is printed rather than omitted.)
    """
    have = [r for r in rows if "cell_n" in r]
    if not have:
        print("    (no per-cell tally in this tag — it predates the 2026-09-11 addition)")
        return
    cn = np.sum([np.asarray(r["cell_n"]) for r in have], axis=0)
    co = np.sum([np.asarray(r["cell_offrule"]) for r in have], axis=0)
    cm = np.sum([np.asarray(r["cell_misread"]) for r in have], axis=0)
    prac = set(setup.get("practiced") or [])
    lab = ("off-rule / mis-KEYED" if any(r.get("miskey_rate") is not None for r in have)
           else "off-rule / mis-read")
    # [embouchure/Q2] the HOMOPHONOUS cells, read from `setup.json` rather than re-derived.
    hom = {(int(f_), int(r_)) for f_, r_ in
           ((setup or {}).get("lexicon") or {}).get("cells", [])}
    res = {(int(f_), int(r_)) for f_, r_ in
           ((setup or {}).get("lexicon") or {}).get("cells_rule_resolves", [])}
    print(f"    per (intended feature, register), whole ladder — {lab}, n in brackets"
          + ("   [H] = the feature spells a SHARED form here; [H!] = shared and the rule "
             "still resolves it" if hom else "") + ":")
    print("      f\\rho " + " ".join(f"{r}{'*' if r in prac else ''}".rjust(16)
                                      for r in range(cn.shape[1])))
    for f in range(cn.shape[0]):
        cells = []
        for r in range(cn.shape[1]):
            if cn[f, r] == 0:
                ...
                tagc = ("[H!]" if (f, r) in res else "[H]" if (f, r) in hom else "")
                cells.append(f"{co[f, r] / cn[f, r]:.3f}/{cm[f, r] / cn[f, r]:.3f}"
                             f"({int(cn[f, r]):5d}){tagc}")
        print(f"      f{f}    " + " ".join(cells))
    tot = max(int(cn.sum()), 1)
    print(f"      pooled: off-rule {co.sum() / tot:.5f}  mis-read {cm.sum() / tot:.5f}  "
          f"n {int(cn.sum())}   (* = practised register)")
    dis = int(((co > 0) & (cm == 0)).sum()), int(((cm > 0) & (co == 0)).sum())
    print(f"      cells where ONLY the rule is violated: {dis[0]}; "
          f"only the reader: {dis[1]}")
    if hom:
        hm = np.zeros_like(cn, bool)
        for f_, r_ in hom:
            if f_ < hm.shape[0] and r_ < hm.shape[1]:
                hm[f_, r_] = True
        for nm_, sel_ in (("SHARED-form cells", hm & (cn > 0)),
                          ("unshared cells", (~hm) & (cn > 0))):
            n_ = cn[sel_].sum()
            if n_:
                print(f"      {nm_:18s} off-rule {co[sel_].sum() / n_:.5f}  "
                      f"mis {cm[sel_].sum() / n_:.5f}  n {int(n_)}")


def shared_cell_K(tag, A, setup, summary):
    """§6E — EACH ROUTE'S RECOVERED K AT THE SHARED CELLS, against the unshared ones, by cycle.

    Under homophony a (feature, register) cell whose form is shared is the cell the perceptual
    route cannot resolve from the surface alone: two meanings write one form, and the reader
    returns the owner its own teacher labelled (`em_q2p`: 24 of 24 cells, deterministically).
    A route that learns from the WORLD'S blocks therefore has a relabelled diet exactly there,
    while a route that learns from its own writes does not. This splits every fitted route's
    recovered K into the shared cells and the rest, so the fingerprint — if there is one — is
    where it would have to be.
    """
    lex = (setup or {}).get("lexicon")
    if not lex or not lex.get("cells"):
        return
    K = np.asarray(setup["rule"]["K"], int)
    hm = np.zeros_like(K, bool)
    for f_, r_ in lex["cells"]:
        hm[int(f_), int(r_)] = True
    rs = np.zeros_like(K, bool)
    for f_, r_ in lex.get("cells_rule_resolves", []):
        rs[int(f_), int(r_)] = True
    prac = np.zeros(K.shape[1], bool)
    prac[np.asarray(setup["practiced"], int)] = True
    print("\n=== [E] §6E  recovered K at the SHARED-form cells, by cycle ===")
    print(f"  the lexicon: {lex['n_shared_forms']} shared forms of {lex['n_forms']}; "
          f"{int(hm.sum())} (feature, register) cells sit on one, "
          f"{int(rs.sum())} of them the rule still resolves")
    for c_, rec in sorted(lex["shared_forms"].items()):
        print(f"    form {c_}: owners {rec['owners']} -> last writer keeps "
              f"f={rec['last_writer_keeps']}")
    arms = [k for k in sorted(A) if any(isinstance(r, dict) and r.get("K_hat")
                                        for r in (A[k]["log"].get("render") or []))]
    if not arms:
        print("  (no fitted route in this tag)")
        return
    marks = [1, 5, 10, 20, 40, 60, 85, 100, 120, 140]
    for arm in arms:
        log = A[arm]["log"]
        sig = A[arm].get("fit_signal") or "surface"
        print(f"\n  {arm}  (fit_signal={sig})")
        print(f"    {'cyc':>4s} {'acc SHARED':>10s} {'acc SHARED/prac':>15s} "
              f"{'acc rule-resolves':>17s} {'acc unshared':>12s} {'acc all':>7s}")
        for i, r in enumerate(log["render"]):
            if not isinstance(r, dict) or not r.get("K_hat"):
                continue
            if int(log["cycle"][i]) not in marks:
                continue
            Kh = np.asarray(r["K_hat"], int)
            ag = Kh == K
            g = lambda m_: (float(ag[m_].mean()) if m_.any() else float("nan"))
            print(f"    {int(log['cycle'][i]):4d} {g(hm):10.4f} "
                  f"{g(hm & prac[None, :]):15.4f} {g(rs):17.4f} "
                  f"{g(~hm):12.4f} {float(ag.mean()):7.4f}")


def e_floor_in_tag(tag, A, setup, summary, ref="canon_s"):
    """§7E — THE MEANING FLOOR, RE-DERIVED IN TAG. Every prior reading of the two-cluster split
    used `if_q1c_yk`'s null-ABBA floor on `e` (v_tol 0.0173), measured on a different world.
    Q2's world is a different lexicon, so the floor is re-derived here on THIS tag's reference
    arm, by the donors' own `null_abba` at the same span and window, with era boundaries and
    commit cycles dropped exactly as `in_tag_floors` does.
    """
    if ref not in A:
        return
    # THE DONOR'S OWN IDIOM, not a re-derivation of it: the full series with `skip=`, so a
    # dropped cycle removes the WINDOWS that touch it rather than being deleted from the
    # series. Deleting splices non-adjacent cycles together and inflates sd(N) — measured, on
    # `em_q1/canon_s`, at 0.0521 spliced against the donor's 0.0173 skipped, a 3x error.
    e = [float(x) for x in A[ref]["log"]["e"]]
    skip = _series_skip(A[ref])
    N, D = null_abba(e, skip=skip, span=1, W=4)
    v_tol = float(_sd(N) / (4 ** 0.5))
    print("\n=== [E] §7E  the meaning floor, re-derived IN TAG ===")
    print(f"  reference arm {ref}: {len(N)} null-ABBA windows over {len(e)} cycles "
          f"({len(skip)} era-boundary/commit cycles skipped); span 1, W 4, v_tol = sd(N)/2")
    print(f"  >>> v_tol(e) = {v_tol:.7f}   (the donor tag `if_q1c_yk` measured 0.0173 on ITS "
          f"world; this tag's lexicon is not that world)")
    rows = [(k, float(A[k]["log"]["e"][-1])) for k in sorted(A)]
    base = dict(rows).get(ref)
    print(f"    {'arm':20s} {'e(end)':>8s} {'- ref':>8s} {'in floors':>9s}")
    for k, v_ in sorted(rows, key=lambda kv: kv[1]):
        print(f"    {k:20s} {v_:8.4f} {v_ - base:+8.4f} "
              f"{(v_ - base) / max(v_tol, 1e-9):+9.2f}")
    return v_tol


def mined_precision_two_spaces(tag, A, setup, summary):
    """§8E — THE MINED TABLE IN FEATURE SPACE AGAINST TOKEN SPACE, at the shared cells.

    `enharmonic`'s junk channel with a knob on it. The miner keys on the READER's parse of the
    learner's own solved configurations; under homophony the reader returns the last-writer
    owner at a shared form, so a chunk the learner actually produced as (f1, f2) can be banked
    as (f1', f2'). In FEATURE space that entry is false. In TOKEN space it may be
    indistinguishable — its rendering is a string the world also writes — which is exactly the
    entry a precision-in-feature-space readout calls junk and the learner cannot tell from a
    true one.

    The bottom map is rebuilt offline from `setup["config"]["rule_seed"]` and
    `setup["lexicon"]["merge"]`, so nothing has to be logged in-run for this.
    """
    lex = (setup or {}).get("lexicon")
    if not lex:
        return
    try:
        from rhm.rhm_data import generate_rules_distinct
        from rhm.practice.embouchure.embouchure import apply_lexicon_merge, make_rule
    except Exception as exc:                       # pragma: no cover - needs modal
        print(f"\n[warn] §8E needs the substrate module: {exc!r}")
        return
    cfg = setup["config"]
    v, s_, m_ = int(cfg["v"]), int(cfg["s"]), int(cfg["m"])
    rules = apply_lexicon_merge(
        generate_rules_distinct(v, s_, int(cfg["depth"]), m_, seed=int(cfg["rule_seed"])),
        lex.get("merge"))
    bot = rules[-1]
    pw = v ** np.arange(s_)
    rule = make_rule(setup["rule"]["name"], v, m_, int(setup["rule"]["n_ctx"]))
    prac = [int(x) for x in setup["practiced"]]

    def render(tup, rho):
        """the code string a feature tuple spells at register rho, under the true rule"""
        return tuple(int((bot[int(f_), int(rule.K[int(f_), rho])] * pw).sum()) for f_ in tup)

    print("\n=== [E] §8E  the mined table: precision in FEATURE space vs TOKEN space ===")
    print("  a mined key is true-in-feature if it is one of the world's own tuples; "
          "true-in-token if\n  what it SPELLS at a practised register is what some true tuple "
          "spells there. The gap is\n  the junk the learner cannot hear.")
    for arm in sorted(A):
        log = A[arm]["log"]
        rows = []
        for ell in ("2", "3"):
            tru = {tuple(int(z) for z in r)
                   for r in (setup.get("true_tables", {}).get(ell, {}).get("flat") or [])}
            if not tru:
                continue
            tru_tok = {rho: {render(t, rho) for t in tru} for rho in prac}
            keys = set()
            for rec in log.get("miner") or []:
                if isinstance(rec, dict) and ell in rec:
                    for k_ in (rec[ell].get("keys_at_support") or []):
                        keys.add(tuple(int(z) for z in k_))
            if not keys:
                continue
            nf = sum(1 for k_ in keys if k_ in tru)
            nt = sum(1 for k_ in keys
                     if any(render(k_, rho) in tru_tok[rho] for rho in prac))
            rows.append((ell, len(keys), nf / len(keys), nt / len(keys)))
        if rows:
            print(f"  {arm}")
            for ell, n_, pf, pt in rows:
                print(f"    L{ell}: {n_:4d} keys at support   precision(feature) {pf:.4f}   "
                      f"precision(token) {pt:.4f}   gap {pt - pf:+.4f}")


def own_miskey(tag, A, setup, summary):
    """§5E — THE KEYING AXIS. Every own arm records both keys for its own written blocks: the
    reader's parse of what it wrote (`recall`, what `em_q1` ran) and the efference copy
    (`record`). `miskey_rate` is their disagreement on the blocks where both are defined —
    a row for a block the learner meant as f but its reader heard as f' trains cell (f', rho)
    with f's synonym, and that is a different failure from a dropped cell.

    The arm's `intent` says which key its fit actually used; the rate is logged either way, so
    the keying axis is measurable without changing the arm.
    """
    own = [k for k in _own_arms(A) if any(
        isinstance(r, dict) and r.get("miskey_rate") is not None
        for r in (A[k]["log"].get("own") or []))]
    if not own:
        return
    print("\n=== [E] §5E  mis-keying: the reader's parse of its own write against the "
          "efference copy ===")
    print(f"  {'arm':18s} {'intent':>8s} {'era':>3s} {'both keys':>10s} {'mis-keyed':>10s} "
          f"{'rate':>8s} {'cells used':>10s}")
    for arm in own:
        log = A[arm]["log"]
        era = np.asarray(log["era"], int)
        rows = [(i, r) for i, r in enumerate(log["own"])
                if isinstance(r, dict) and r.get("miskey_rate") is not None]
        it = A[arm].get("own_gate", {}).get("intent") or rows[0][1].get("intent", "?")
        for e in sorted(set(era.tolist())):
            sel = [r for i, r in rows if era[i] == e]
            if not sel:
                continue
            nb = float(np.mean([r["n_both_keys"] for r in sel]))
            nm = float(np.mean([r["n_miskey"] for r in sel]))
            print(f"  {arm:18s} {it:>8s} {e:3d} {nb:10.1f} {nm:10.2f} "
                  f"{nm / max(nb, 1e-9):8.5f} {float(np.mean([r['n_cells'] for r in sel])):10.1f}")
        g = A[arm].get("own_gate") or {}
        if g.get("n_both_keys"):
            print(f"  {arm:18s} {'':>8s} ALL {g['n_both_keys']:10d} {g['n_miskey']:10d} "
                  f"{g['n_miskey'] / max(g['n_both_keys'], 1):8.5f}")
        _cell_table(arm, [r for _, r in rows], setup)


def own_volume(tag, A, setup, summary):
    """§3E — WHAT THE TWO CHANNELS ACTUALLY ATE, per cycle. Q0 estimated the own-write count
    from `e_sp_practice / spell_err`; the own arm now COUNTS it (`log["own"]`), so the ratio
    that Q0 had to infer is measured here — including the part Q0 could not see, the read-back
    drop on the learner's own writes (`written & agood` against `written`)."""
    own = _own_arms(A)
    if not own:
        return
    print("\n=== [E] §3E  volume: what each channel ate, per cycle ===")
    for arm in own:
        log = A[arm]["log"]
        rows = [r for r in (log.get("own") or []) if isinstance(r, dict)]
        if not rows:
            continue
        era = np.asarray(log["era"], int)
        print(f"\n  {arm}")
        print(f"    {'era':>3s} {'cyc':>4s} {'own blk/inst':>12s} {'cells/cyc':>9s} "
              f"{'rows w/ 0 writes':>16s} {'unusable cells':>14s}")
        oi = [i for i, r in enumerate(log.get("own") or []) if isinstance(r, dict)]
        for e in sorted(set(era.tolist())):
            sel = [i for i in oi if era[i] == e]
            if not sel:
                continue
            def g(k, d=float("nan")):
                vals = [log["own"][i].get(k) for i in sel]
                vals = [x for x in vals if x is not None]
                return float(np.mean(vals)) if vals else d
            # `own_scalar` reports `readback_drop` (cells its reader-keyed parse could not
            # use); `own_readback` reports `n_ambiguous` (cells its RECALL could not resolve,
            # which is 0 on an injective lexicon). One column, named for what it is.
            un = g("readback_drop")
            if un != un:
                un = g("n_ambiguous", 0.0) / max(g("n_cells", 1.0), 1.0)
            print(f"    {e:3d} {len(sel):4d} {g('n_own_mean'):12.2f} {g('n_cells'):9.1f} "
                  f"{g('n_own_zero'):16.2f} {un:14.5f}")
    # the surface side, from the banked donor design and from any surface arm in tag
    for arm in sorted(k for k in A if str((A[k].get("fit_signal") or "surface")) == "surface"
                      and (A[k]["log"].get("render") or [])
                      and any(isinstance(r, dict) and "n" in r for r in A[k]["log"]["render"])):
        n = [r.get("n") for r in A[arm]["log"]["render"] if isinstance(r, dict)]
        if not n or n[0] is None:
            continue
        d = np.diff(np.concatenate([[0], np.asarray(n, float)]))
        era = np.asarray(A[arm]["log"]["era"], int)
        print(f"\n  {arm} (surface): harvested blocks/cycle by era "
              + "  ".join(f"era{e} {d[era == e].mean():.0f}" for e in sorted(set(era.tolist()))))
