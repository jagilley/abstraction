...
app = modal.App("rhm-practice-inflection", image=image)     # [inflection]
...
def sample_derivations_ruled(rules, roots, s, rng, rule, ctx):       # [inflection]
...
def _sample_pool_r(rules, n, s, seed, rule=None, n_ctx=1, practiced=None):   # [inflection]
    ...
    leaves = sample_derivations_ruled(rules, roots, s, rng, rule, ctx)
...
def build_shared(cfg, device):
    ...
    return {"rules": rules, "rules_t": rules_t, "canon": canon, "inverse_maps": inverse_maps,
            ...
            "probe_by_reg": {int(c): _sample_pool_r(
                rules, max(64, cfg["n_probe_clean"] // 4), s, cfg["seed"] + 707_000 + c,
                rule=rule, n_ctx=n_ctx, practiced=[c])[1] for c in range(n_ctx)} if rule else None}
...
def _spiral_shared(cfg, device, eras=None):
    ...
    shared["probe_clean"] = torch.from_numpy(                              # [inflection]
        _sample_pool_r(shared["rules"], cfg["n_probe_clean"], cfg["s"], cfg["seed"] + 31337,
                       ...
                       practiced=shared.get("practiced"))[1])
        ...
        while True:
            ...
            if rule is not None and (rhead is not None or spell_store is not None):
                ...
                if rhead is not None and cyc > int(cfg.get("fit_warmup") or 0):
                    rinfo = rule_head_fit(rhead, ropt, rbuf,
...
def gates_cpu(v=8, s=2, depth=6, m=2, rule_seed=0, n=384, n_ctx=3, seed=0,
    ...
    if verbose:
        print(f"\n[gates_cpu] {'ALL PASS' if not fails else 'FAILED: ' + ', '.join(fails)} "
              ...
              f"checks)", flush=True)
    assert not fails, f"[inflection] offline gates FAILED: {fails}"
    return out
...
def fidelity_smoke(tag: str = "gf_smoke", cycles: int = 4, seed: int = 0):
    ...
    from rhm.practice.tutti import tutti as SP
    ...
    print(f"[gf] depth={cfg['depth']} m={cfg['m']} cycles/era={cycles} maxl={cfg['max_macro_level']} "
          f"— inflection vs tutti (gate_log ON; every [inflection] knob off)",
          flush=True)
    ...
    assert worst_fork <= worst_ctrl, (
        f"G-F FAILED: the inflection fork perturbed tutti.py's code path "
        f"({worst_fork:.3e} > self-replay control {worst_ctrl:.3e}) — the context column, the "
        f"renderer duck, one of the `_bind` sites or one of the three new log rows is not "
        f"transparent")
    print("G-F PASS — with every [inflection] knob off, this fork replays tutti.py.")
...
def main(quick: bool = True):
    inflection_run.remote(quick=quick)     # [inflection]
