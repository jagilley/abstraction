...
| Folder | Summary |
|---|---|
| [`temporal/`](temporal/README.md) ([FILES](temporal/FILES.md)) | The same battery with the FM's conditioning gap shifted from six blocks to one token, both arms on one frozen M at matched report positions, plus the exact BP oracle. Privileged access survives the axis change (+0.014 to +0.027) but its partial `R²` against oracle belief revision at matched surprisal is 0.0000 — self-report and observer track `B_t` equally, so the charge is real and public. Supplies this battery's first validated ceiling (`O_h6`). Its own child [`temporal/epistemics/`](temporal/epistemics/README.md) repoints the harness from privacy to **composition**: a source × target decode matrix at matched readout capacity, asking whether the temporal FM *concentrates* revision (`r` vs `Δ`) or merely inherits it, and running `conditional_revision`'s never-run Gate C. |
| [`../../a2a_forward/fsm_part3/dispositional_grammar/`](../../a2a_forward/fsm_part3/dispositional_grammar/FILES.md) | **Part 3, grammar child (2026-09-13/14; lives with its language siblings under the program's home).** This battery over a checkpointed wake, CL and OL arms, two horizons, dispositional targets read within hierarchy level with historian and activation-access ceilings. On OL the implementation rows carry +0.10 to +0.29 at L3–L5 in both time directions against every ceiling while behavior is flat at every level; on the rank readout the advantage is positive at every level and rises with depth. Writeup: [`../../a2a_forward/fsm_part3/README.md`](../../a2a_forward/fsm_part3/README.md). |
