...
We are **not** crystallizing a belief yet — the compounding question (next) is what would elevate this from "self-knowledge appears" to "self-knowledge does work."

## Caveats

- **Directional, not magnitude-precise.** Directionally consistent across four runs and two regimes, and the harness is deterministic (the sweep's `ntp_cl@1.0` reproduced the m2 control's root/SK exactly). The signs and the mechanism are the claim, not the exact magnitudes; the m4 SK magnitudes shift ~0.1 between an A10G and an L4 run (sign/direction stable).
...
- **The one directionally-correct signal**: under sparsity, KL *did* pull root toward the teacher's exact injected value (0.855 → 0.866, ceiling 0.871). Real transfer, but into a nearly-solved problem — tiny headroom. Distillation isn't broken on RHM; it's solving an already-solved problem.
