"""Confidence threshold experiment: aggressive gradient reallocation from easy to hard positions.

The focal loss sweep (γ=0-5) showed the right *direction* — levels 3-4 slightly
improved while L0 got worse — but the magnitude was tiny at 2.7M params because
the model was capacity-saturated. This experiment pushes both levers:

1. **More aggressive loss masking**: zero out the gradient for any position where
   p(correct) > τ. This creates a natural curriculum — as level-0 positions become
   confident during training, they drop out entirely, forcing all gradient toward
   harder higher-level positions. No DGP knowledge needed.

2. **Larger model**: 8L/8H/256D (~6.5M params) gives more capacity headroom so
   there's actually something to redistribute.

Sweep: τ ∈ {1.0 (standard CE), 0.5, 0.3, 0.15} + focal γ=2 for comparison
All at L=6/m=4, 8L/8H/256D, 20M tokens, L4 GPUs
...
app = modal.App("rhm-confidence-threshold", image=image)
...
def _threshold_loss(logits, targets, threshold, vocab_size):
    """CE loss with confidence masking: zero gradient for positions where p(correct) > τ."""
    import torch
...
# ---------------------------------------------------------------------------
# Phase 1: Train model with modified loss + per-level trajectory
# ---------------------------------------------------------------------------
...
def train_and_eval(
    ...
    """Train model with modified loss, evaluate per-level loss at checkpoints.

    loss_type: "ce" (standard cross-entropy), "threshold" (confidence masking),
               "focal" (focal loss)
    """
    import torch
    import torch.nn.functional as F
    ...
    run_label = f"{loss_type}"
    if loss_type == "threshold":
        run_label = f"τ={conf_threshold}"
    elif loss_type == "focal":
        run_label = f"γ={focal_gamma}"
    ...
    for step in range(n_steps + 1):
        if step > 0:
            ...
            if loss_type == "threshold" and conf_threshold < 1.0:
                loss, mf = _threshold_loss(logits, y, conf_threshold, v)
                if step % 2000 == 0:
                    mask_fracs.append((step, mf))
                    print(f"    mask_frac={mf:.3f} (fraction of positions contributing to loss)")
            elif loss_type == "focal":
    ...
    save_dir = f"{DATA_DIR}/rhm_confidence_threshold"
    os.makedirs(save_dir, exist_ok=True)
    safe_label = run_label.replace("=", "").replace(".", "p")
    model_path = os.path.join(save_dir, f"model_{safe_label}.pt")
    ...
    with open(os.path.join(save_dir, f"trajectory_{safe_label}.json"), "w") as f:
...
def fm_eval(
    ...
    n_layer: int = 8, n_head: int = 8, n_embd: int = 256,
    run_label: str = "ce",
    predict_from: str = "post_block0", predict_to: str = "post_block4",
    fwd_n_layer: int = 2, fwd_d_head: int = 32, fwd_n_head: int = 1,
    ...
    os.makedirs(save_dir, exist_ok=True)
    safe_label = run_label.replace("=", "").replace(".", "p")
    with open(os.path.join(save_dir, f"fm_eval_{safe_label}.json"), "w") as f:
        json.dump(result, f, indent=2, cls=NumpyEncoder)
    volume.commit()
...
def confidence_threshold_sweep(
    ...
    """Sweep confidence thresholds + focal baseline, measuring per-level loss and FM residuals."""
    import numpy as np

    L = depth
    key = setting_key(v, s, L, m)

    # Define all runs: (loss_type, conf_threshold, focal_gamma, label)
    runs = [
        ("ce",        1.0, 0.0, "ce"),
        ("focal",     1.0, 2.0, "γ=2"),
        ("threshold", 0.5, 0.0, "τ=0.5"),
        ("threshold", 0.3, 0.0, "τ=0.3"),
        ("threshold", 0.15, 0.0, "τ=0.15"),
    ]

    print(f"\n{'='*100}")
    print(f"CONFIDENCE THRESHOLD SWEEP: {key}, {n_layer}L/{n_head}H/{n_embd}D")
    print(f"  runs: {[r[3] for r in runs]}")
    ...
    train_handles = []
    for loss_type, ct, fg, label in runs:
        h = train_and_eval.spawn(
            v=v, s=s, depth=depth, m=m, n_tokens=n_tokens,
            n_layer=n_layer, n_head=n_head, n_embd=n_embd,
            loss_type=loss_type, conf_threshold=ct, focal_gamma=fg,
        )
        train_handles.append((label, h))

    train_results = {}
    for label, h in train_handles:
        result = h.get()
    ...
    fm_handles = []
    for label in [r[3] for r in runs]:
        h = fm_eval.spawn(
            model_path=train_results[label]["model_path"],
            v=v, s=s, depth=depth, m=m, n_tokens=n_tokens,
            n_layer=n_layer, n_head=n_head, n_embd=n_embd,
            run_label=label,
        )
    ...
    print(f"\n{'='*140}")
    print(f"CONFIDENCE THRESHOLD SWEEP RESULTS: {key}, {n_layer}L/{n_head}H/{n_embd}D (~{sum(p.numel() for p in []) if False else '6.5M'} params)")
    print(f"Uniform baseline: {uniform:.4f}")
    print(f"{'='*140}")

    # Table 1: Per-level loss at convergence
    print("\n--- Per-level loss at convergence (standard CE eval) ---")
    header = (f"{'run':>8}  {'val':>7}  {'acc':>6}  {'H':>6}"
    ...
    for label in run_labels:
        ...
        pl = final["per_level"]
        row = (f"{label:>8}  {final['val_loss']:>7.4f}  "
    ...
    header2 = (f"{'run':>8}  {'cos':>7}  {'1-cos':>7}  {'res_norm':>9}  "
               f"{'rank%':>6}  {'top1%':>6}")
    for ell in range(L):
    ...
    for label in run_labels:
        ...
        row = (f"{label:>8}  {fm['fm_cosine']:>7.4f}  {1-fm['fm_cosine']:>7.4f}  "
    ...
    # Table 3: Per-level loss CHANGE relative to baseline (ce)
    ...
    # Table 4: mask fraction over training (for threshold runs)
    print("\n--- Mask fraction (fraction of positions contributing to loss) ---")
    for label in run_labels:
        mf = train_results[label].get("mask_fracs", [])
        if mf:
            print(f"  {label}: {['step={} frac={:.3f}'.format(s, f) for s, f in mf]}")

    # Save combined results
    save_dir = f"{DATA_DIR}/rhm_confidence_threshold"
    os.makedirs(save_dir, exist_ok=True)
    combined = {
        "experiment": "confidence_threshold_sweep",
