...
## Reframe (2026-07-18): the shared-DGP result is the *enabling condition*, not a deficiency

The arc above reads RHM's single shared ruleset as a **limitation** — "no domain-specific deep structure → concentration can't beat breadth → bolt on genuine domains (cut-3)." A 2026-07-17/18 discussion inverts that reading; recorded here because it re-specs cut-3.

- **Shared compositional depth is what makes sample-efficient generalization *possible*, not a defect.** A small slice of data is informative about the whole DGP *only because* the deep structure is shared. So "broad beats narrow" (Student-B > Student-A) is a positive result about **transfer through shared abstraction**, mislabeled by asking the specialization question — and it returns RHM to its native Cagnetta–Wyart question (sample complexity of hierarchical compositional learning), which the specialization lens had drifted off.
- **The load-bearing contrast is model-vs-human, not full-vs-subtree.** *Neither* full nor subtree recovers the deep structure here — both stall at ~d3.5 under NTP (root at chance even on S); only the oracle *direct* target breaks it (0.08 → 0.80). The stall stops being a nuisance and becomes **the phenomenon**: humans recover deep structure from a *subtree* with no deep label; our NTP learners can't.
- **Human "little data" is conditional on priors → the analog is a cascade, not tabula-rasa few-shot.** The clean RHM version: *a learner that already holds level ℓ recruits ℓ+1 from a small marginal sample* — and since RHM's low and deep rules are one shared set, "already holds ℓ" is just the post-warmup state. The sharp question is **what recruits ℓ+1 once ℓ is consolidated** — NTP stalls, the oracle target cheats (no learner gets deep labels), and the non-cheating candidate is an intrinsic **learning-progress** drive (consolidating ℓ makes ℓ+1 cheap; −d‖e‖/dt decaying on ℓ signals reallocation). This converges with [ideas/two_timescale_value_loop.md](../../../ideas/two_timescale_value_loop.md): compounding needs a *moving* frontier, and RHM's is internal to depth.
- **Consequence — cut-3 is aimed at the demoted question.** Cut-3 adds *domain-specific deep* structure (to make concentration beat breadth); the sample-efficiency/transfer question wants the **opposite** — *surface-distinct but deep-shared* domains, so depth learned on A transfers to a small B sample. And the *first* experiment likely needs no DGP change at all: the single tree already poses "climb ℓ+1 without a label."

**Crisp experiment**: break the ~d3.5 stall from a bounded data budget via an intrinsic learning-progress loop, *no* oracle deep target, against the standing anchors (floor root 0.08, oracle-aux ceiling 0.80). **Required control** (ratchet-arc capacity finding, [../ratchet/RHM_RL_GEN_DISTILL_EXTENDED_README.md](../ratchet/RHM_RL_GEN_DISTILL_EXTENDED_README.md)): run at an (L, m, capacity) where the model *demonstrably* reaches the target level under plain NTP, so the test is whether the loop *accelerates a reachable level* — not confounded with "this model could never compose it anyway." (Prior signal from that arc: gen-distill deepens L3 representation sub-threshold on stationary data but never crosses to behavior — the *final-layer* +15% η² originally cited was aliasing, but the *intermediate-layer* deepening is real, p<0.001 — exactly the gap a non-stationary value loop is meant to fill.)
...
- **Exp 1 and cut-2a are different axes — do not merge their verdicts (2026-07-29).** Exp 1 restricts the **corpus** (plain NTP, matched tokens) and is inert, Δ ≤ 0.013. Cut-2a restricts **where a direct target is applied** on a corpus that stays broad in every arm, and is *not* inert: −0.140 at d4. A downstream doc merged them under one "inert" cell; see [`ideas/meta_learning_under_metered_data.md`](../../../ideas/meta_learning_under_metered_data.md) §5's attached note. Relatedly, cut-2a's `aux_*` is a **precomputed external oracle label, not a drive**, so it licenses nothing about endogenous/intrinsic-drive settings — a claim that it shows corpus breadth and an intrinsic expansion drive to be *complements* was retracted on that basis ([`ideas/breadth_as_grader_heterogeneity.md`](../../../ideas/breadth_as_grader_heterogeneity.md) §6).
- **Exp 1's null is measured at the floor.** Both FULL and SUBTREE stall at ~d3.5 with root at chance even on S, so the inert result says breadth doesn't convert to depth *under plain NTP*; it cannot speak to whether breadth supplies expansion in a setting where anything expands. The floor/ceiling pair (root 0.08 → 0.80) is the reusable asset here.
