...
- **The practice arc on RHM**: [`practice/`](practice/README.md) — practice as a control loop over the *conditions and units* of learning, ported from [`mjc/practice/`](../mjc/practice/README.md). Children: [`practice/crystallize/`](practice/crystallize/README.md) (certificate-gated compilation under priced feedback), [`practice/ratchet/`](practice/ratchet/README.md) (earning a level-indexed vocabulary over a depth ladder — distinct from the meta-learning [`ratchet/`](ratchet/README.md) arc), [`practice/ear/`](practice/ear/README.md) (climbing the evaluation layer; provisional commitment rescues the frontier), [`practice/recital/`](practice/recital/README.md) (self-paced eras; no internal signal prices time-at-the-bottom, 3 worlds), [`practice/tall/`](practice/tall/README.md) (the depth-6 port attempt, closed with a diagnosed apparatus negative), and the crossed pair [`practice/transpose/`](practice/transpose/FILES.md) × [`practice/setlist/`](practice/setlist/FILES.md) — truth-news vs demand-news, jointly written up in [`practice/typed_gaps/`](practice/typed_gaps/README.md) (the conditioning gap is typed: one organ per currency of change).
...
- **The conditioning gap / belief revision**: [`endogenous_teacher/`](endogenous_teacher/README.md) (is the model's own surprise separable from the text's, and can it teach?) → [`conditional_revision/`](conditional_revision/README.md) (the exact-BP oracle for belief revision, and whether the model's state tracks it) → [`question_model/`](question_model/README.md) (the revision identity extended by a third, exactly-computable term — question-news, what the world is currently asking — under calibrated OU demand drift: NTP tracks it only above an evidence-density threshold, the one place an explicit question-tracker pays is warm-starting the fast component at context entry, and the demand-keyed self-knowledge readout is a vacuous null on an unmetered generalist).
...
Round 6 (2026-08-16), the crossed pair [`practice/transpose/`](practice/transpose/FILES.md) × [`practice/setlist/`](practice/setlist/FILES.md), jointly written up in [`practice/typed_gaps/`](practice/typed_gaps/README.md): drifting *what is true* (grammar rule cells, demand fixed) degrades the dense learner and leaves the recert channel silent (1/62 — a stale mined table even beats the current truth), while drifting *what is asked* (OU on the derivation distribution, precision-vs-truth pinned at 1.000 by gate) leaves the plant flat and fires recert decisively (9/62 swaps, all coverage-improving; ablating it costs Δ0.271 deep, ≈8× noise) — a crossed double dissociation. Committed chunks store demand-concentration rather than truth, skill maintenance is demand-tracking and evaluative rather than entropic, and the conditioning gap is **typed**: one organ per currency of change (dense learning ↔ truth, repair/metering ↔ interface, evaluative re-selection ↔ demand, the teacher ↔ level). Ranks are the claim.

### RL dimensionality ablation: reward optimization never moved the pretrained basis boundary (2026-08-23)

**Full writeup**: [rl_dimensionality/README.md](rl_dimensionality/README.md) | **Design record**: [rl_dimensionality/DESIGN.md](rl_dimensionality/DESIGN.md)

Sweeps m ∈ {1,2,3,4,6} at fixed (v=8, s=2, L=6, invertible rules) — the knob that moves semantic degeneracy (m^L) with horizon, architecture, and rollout compute exactly fixed — and splits every result into **bounds** (exact BP ceilings/floors, algorithm-free) vs **attainment** (a three-mechanism ladder: vanilla REINFORCE → KL-anchored REINFORCE → expert iteration, the class limit of sample reweighting, all rollout-matched at 384K). Bounds: a canonical-answer (exact-match) verifier's prefix-conditional headroom provably vanishes with m (0.096→~0), while a validity (parse) verifier's reward *densifies* on the same knob — "RL is bad on complex domains" is here a verifier×domain fact before it is an algorithm fact. Attainment: vanilla REINFORCE self-destructs at every m (9–14 nats NTP excess; at m=1 it drags a solved task from the Bayes ceiling 0.849→0.388); the KL anchor preserves the basis and reveals near-zero gains; EI is far stronger (recovers m=1 fully, nearly doubles root-level parse validity at m=4/6 with the STaR flywheel visibly spinning) — **and across all 3 mechanisms × 5 dimensionalities × 2 verifiers, reward optimization never created structure the basis lacked**: at m=6, root validity climbs 0.142→0.292 round-by-round while the unlearned deep levels stay pinned at uniform (L3 excess +0.97→+0.99) and eta² grows nowhere — pass rates up, rules unlearned, measurable only because the DGP is known. The reward/representation trade also steepens with m (EI's NTP damage per unit reward: 0.11→0.89 nats across m=2→6). Large, monotone-in-m effects triangulated across mechanisms and verifiers.
