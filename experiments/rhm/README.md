...
**Goal**: port E3's full inner/outer loop + forward model onto RHM sculpting. **Finding so far**: the naive port is a *confounded null by construction* — RHM rule usage is uniform (per-level feature-marginal entropy ≥ 0.93 everywhere; root-conditioning decays to that floor within two levels), so `visits` is flat and `lprog × visits` collapses to `lprog`. Three verified primitives fix that: **distractor channels** (structurally irrelevant by construction, exactly irreducible noise at the information-theoretic floor), **support-fixed rule drift** (an OU walk on synonym mixtures — rule tables never move, so the DP `d*` cannot either, giving immunity to the task-switch forgetting that made the wholesale rule-seed swap measure nothing; magnitude is closed form and the per-level calibration is load-bearing at 11.9× spread), and a **repair-cost instrument** whose attributable gap recovers the analytically known drift KL to 88–95%. Two traps found and documented: raw cross-entropy is *exactly* blind to support-fixed drift (a null that reads as a finding), and the gap that replaces it is itself confounded by ordinary continued training (at zero drift the "gap" exceeded the real one). **No learning result yet** — the loop is unbuilt. Full writeup: [`directed_sculpting/README.md`](directed_sculpting/README.md).
