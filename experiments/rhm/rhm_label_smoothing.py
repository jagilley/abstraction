"""Label smoothing experiment: does softening NTP shift learning from m to L?

The NTP objective's combination of geometric position weighting (level 0 gets
~50% of gradient for s=2) and cross-entropy's sharpness reward creates a strong
incentive for m-learning (sharpening predictions at already-learned levels) over
L-learning (extending compositional depth). Label smoothing caps the reward for
sharpening, potentially freeing capacity for deeper composition.

Claims to test:
1. Per-level loss: smoothed models learn deeper into the hierarchy
   (lower loss at levels 2-5) at the cost of worse level-0 sharpness
2. Val loss: smoothed models have higher overall val loss
3. FM residuals: smoothed models produce more predictable computation
   (higher cosine, lower residual norm, lower effective rank)

Settings: L=6/m=4, 6L/6H/192D (~2.7M params), 20M tokens
Sweep: ε ∈ {0.0, 0.05, 0.1, 0.2, 0.4}

Reproduction:
  cd experiments/
  modal run --detach rhm/rhm_label_smoothing.py::label_smoothing_sweep
"""
...
app = modal.App("rhm-label-smoothing", image=image)
...
# ---------------------------------------------------------------------------
# Phase 1: Train model with label smoothing + per-level trajectory
# ---------------------------------------------------------------------------
...
def train_and_eval(
    ...
    """Train model with label smoothing, evaluate per-level loss at checkpoints."""
    import torch
    import torch.nn.functional as F
    import numpy as np
    ...
    print(f"=== Training: {key}, {n_layer}L/{n_head}H/{n_embd}D, ε={label_smoothing} ===")
    ...
    for step in range(n_steps + 1):
        if step > 0:
            ...
            # Compute logits without internal loss, then apply label smoothing
            logits, _ = model(x)
        ...
        if step in checkpoint_steps:
            model.eval()
            # Val loss: always standard CE for comparability
            with torch.no_grad():
    ...
    # Save model for FM analysis
    save_dir = f"{DATA_DIR}/rhm_label_smoothing"
    os.makedirs(save_dir, exist_ok=True)
    model_path = os.path.join(save_dir, f"model_eps{label_smoothing:.2f}.pt")
    ...
    with open(os.path.join(save_dir, f"trajectory_eps{label_smoothing:.2f}.json"), "w") as f:
...
def fm_eval(
    ...
    print(f"=== FM eval: ε={label_smoothing}, FM {fwd_n_params/1e3:.0f}K params "
    ...
    print(f"  ε={label_smoothing}: cos={mean_cosine:.4f} res_norm={mean_res_norm:.4f} "
    ...
    save_dir = f"{DATA_DIR}/rhm_label_smoothing"
    os.makedirs(save_dir, exist_ok=True)
    with open(os.path.join(save_dir, f"fm_eval_eps{label_smoothing:.2f}.json"), "w") as f:
        json.dump(result, f, indent=2, cls=NumpyEncoder)
    volume.commit()
...
def label_smoothing_sweep(
    epsilons: str = "0.0,0.05,0.1,0.2,0.4",
    v: int = 8, s: int = 2, depth: int = 6, m: int = 4,
    n_tokens: int = 20_000_000,
    n_layer: int = 6, n_head: int = 6, n_embd: int = 192,
):
    """Sweep label smoothing values, measuring per-level loss and FM residuals."""
    import numpy as np

    eps_values = [float(x) for x in epsilons.split(",")]
    L = depth
    key = setting_key(v, s, L, m)

    print(f"\n{'='*100}")
    print(f"LABEL SMOOTHING SWEEP: {key}, {n_layer}L/{n_head}H/{n_embd}D")
    print(f"  ε values: {eps_values}")
    print(f"  n_tokens: {n_tokens:,}")
    print(f"{'='*100}")

    # --- Phase 1: Train all models in parallel ---
    print("\n--- Phase 1: Training models with label smoothing ---")
    train_handles = []
    for eps in eps_values:
        ...
        train_handles.append((eps, h))

    train_results = {}
    for eps, h in train_handles:
        result = h.get()
        train_results[eps] = result
        final = result["trajectory"][-1]
        print(f"  ε={eps:.2f}: val={final['val_loss']:.4f} "
    ...
    for eps in eps_values:
        h = fm_eval.spawn(
            model_path=train_results[eps]["model_path"],
            v=v, s=s, depth=depth, m=m, n_tokens=n_tokens,
            n_layer=n_layer, n_head=n_head, n_embd=n_embd,
            label_smoothing=eps,
        )
        fm_handles.append((eps, h))

    fm_results = {}
    for eps, h in fm_handles:
        fm_results[eps] = h.get()
    ...
    print("-" * len(header))
    for eps in eps_values:
        final = train_results[eps]["trajectory"][-1]
        pl = final["per_level"]
        row = (f"{eps:>5.2f}  {final['val_loss']:>7.4f}  "
    ...
    header2 = (f"{'ε':>5}  {'cos':>7}  {'1-cos':>7}  {'res_norm':>9}  "
               f"{'rank%':>6}  {'top1%':>6}")
    for ell in range(L):
    ...
    for eps in eps_values:
        ...
        row = (f"{eps:>5.2f}  {fm['fm_cosine']:>7.4f}  {1-fm['fm_cosine']:>7.4f}  "
    ...
    # Table 3: Per-level loss CHANGE relative to baseline (ε=0)
    baseline_pl = train_results[eps_values[0]]["trajectory"][-1]["per_level"]
    print("\n--- Per-level loss change vs baseline (ε=0) ---")
    header3 = f"{'ε':>5}  {'Δval':>7}" + "".join(
    ...
    for eps in eps_values:
        final = train_results[eps]["trajectory"][-1]
        pl = final["per_level"]
        delta_val = final["val_loss"] - train_results[eps_values[0]]["trajectory"][-1]["val_loss"]
        row = f"{eps:>5.2f}  {delta_val:>+7.4f}"
        for lvl in range(max_level + 1):
    ...
    save_dir = f"{DATA_DIR}/rhm_label_smoothing"
    combined = {
        "experiment": "label_smoothing_sweep",
        "setting": key, "v": v, "s": s, "L": L, "m": m,
        "model": f"{n_layer}L/{n_head}H/{n_embd}D",
        "n_tokens": n_tokens,
        "uniform_baseline": uniform,
        "epsilons": eps_values,
        "train_results": {str(eps): train_results[eps] for eps in eps_values},
        "fm_results": {str(eps): fm_results[eps] for eps in eps_values},
    }
