...
2. **A level-ordered allocation space**, so §12's satiety-gated *"recruit ℓ+1"* has an upward direction to
   ...
   8.2×). The *allocation* half — a budget spent over (channel, level) cells — still needs a
   forward model over spans, which is the standing blocker for testing §12 at all.
