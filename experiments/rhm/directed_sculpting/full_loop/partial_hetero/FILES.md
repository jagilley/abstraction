...
`share_rules_bottom` splices the bottom *k* (see [`shared_surface/`](shared_surface/README.md)), and
`make_layout` accepts `share_top` / `share_bottom` / `share_from` on any grammar channel. The environment and the
...
| File | Purpose |
|---|---|
| `geometry_check.py` | **G1–G3** certifying the partially-heterogeneous DGP before the loop runs on it. G1 re-asserts P1 structural irrelevance at every sharing depth (sharing rule tables is exactly the kind of change that could break it). G2 shows the surface stays distinct — legal-leaf-tuple overlap with the tree, plus `level_sibling_stats`, an exact forward-sampled per-level TV that reads the sharing depth straight out of the data and is what makes a G3 null interpretable **under `share_mode=top` only** — for the
bottom splice it reads no sharing at all, which is why `level_cond_sibling_stats` (G2c) exists: the
*conditional* sibling law at level ell is a functional of `rules[ell]` alone, so in closed form it is
exactly 0.000 on precisely the shared levels whichever end was spliced. G3 is the **transfer curve**: fresh block FMs trained under fixed channel allocations at matched budget and matched state distribution, in a **coverage-matched** design (`tree_structA` vs `tree_half` holds tree-block transitions exactly fixed) because the FM's per-block `action_embedding` makes the naive `structA_only` arm confound grammar transfer with parameter coverage. Static — no drift anywhere. `--share-mode` selects the splice direction; `top` is the default and reproduces every published run bit-identically |
| `aggregate.py` | Reads the mirrored `figures/` JSONs and prints the two curves: G3's added-value/exchange-rate table with the `+structA − +structB` contrast that cancels the generic extra-optimizer-steps effect, and E1-PH's per-policy tree FM error with the uniform→oracle, uniform→visits_only and oracle→oracle_shared prizes. Per-seed least-squares slopes against sharing depth with t-stats, so monotonicity is tested rather than eyeballed. Results are keyed on `share_mode` as well as `fm_arch` so the two DGPs are never pooled, and `cross_mode_check` reads the full-sharing rung — where the two splices are the same DGP — as a **measured run-to-run noise floor** |
...
| `figures/geometry_sb_s{1,2,3}/results.json` | **The shared-SURFACE sweep** (`--share-mode bottom`) on the published block FM — written by this folder's script to this folder's `figures/`, but read up in [`shared_surface/README.md`](shared_surface/README.md). Carries the G4 table where cross-channel feature decoding goes chance → 0.563 centered at one shared table, the G2c closed-form certification, and the transfer curve whose cells lie inside the measured ±0.0117 floor |
...
| Child | What |
|---|---|
| [`shared_surface/`](shared_surface/README.md) | The mirror splice — shared surface alphabet, independent deep composition. Dissolves this node's barrier (a), quantifies barrier (b) as an encoder ceiling of ~0.69 centered, and measures the transfer instrument's own run-to-run floor at ±0.0117. Adds no scripts of its own; see its [FILES.md](shared_surface/FILES.md) for what it added to which shared file |
