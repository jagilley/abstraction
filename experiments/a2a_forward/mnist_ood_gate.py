"""MNIST OOD meta-learning experiment: gate response to distribution shift.

Tests whether the bilevel learning gate produces different weights for known
vs novel inputs when the training distribution shifts — the key test of
whether the gate does genuine meta-learning (selective under non-stationarity)
vs just regularization (open uniformly on stationary data).
...
Three conditions, same total main-model gradient steps:
  1. WS_LG_shift: Gated ratchet with phase 1 → phase 2 digit shift
  2. WS_LG_full: Gated ratchet on all digits throughout (stationary control)
  3. OL_shift: Open-loop with same digit schedule (mechanism control)

Key predictions:
  - On stationary data (WS_LG_full), the gate opens uniformly (replicates
    the 4-cycle result: mean 0.31 → 0.77).
  - After the shift (WS_LG_shift cycles 3-4), the gate should produce
    different weights for known (0-6) vs novel (7-9) inputs. If the bilevel
    optimization is doing meta-learning, it should open for novel digits
    (where learning is needed) and partially close for known digits (where
    existing knowledge should be preserved from over-compression).
  - The alternative: the gate opens uniformly regardless, meaning the
    input-dependent gating doesn't discriminate known from novel.

Novel measurements:
  - Per-digit-group gate weights (known 0-6 vs novel 7-9)
  - Per-digit accuracy trajectory (forgetting on 0-6, learning on 7-9)
  - Gate weight correlation with digit novelty
"""
...
def a2a_mnist_ood_gate(
    ...
    print(f"MNIST OOD GATE EXPERIMENT on {device}")
    ...
    all_train_indices = torch.arange(len(train_images))
    print(f"  Known-digit train set: {len(known_train_indices)} "
          f"(digits {known_digits})")
    print(f"  Full train set: {len(all_train_indices)} (digits 0-9)")

    # -----------------------------------------------------------------
    # Pre-generate batch indices
    # -----------------------------------------------------------------
    # Shift conditions: phase 1 samples from known digits, phase 2 from all
    ...
    # Full condition: all digits throughout
    ...
    # Shared eval, probe, repoint indices (always from full test/train set)
    ...
    def eval_by_digit_group(model):
        """Full test set evaluation with per-digit-group breakdown."""
        model.eval()
    ...
    def get_gate_stats_by_digit(lgate, model, fwd_model, label):
        ...
        # Per-dimension mean for known vs novel
    ...
    # =================================================================
    # WS_LG TRAINING LOOP (shared between shift and full conditions)
    # =================================================================
    def run_wslg(condition_label, batch_indices, repoint_indices_list):
        """Run WS_LG for n_cycles with given batch/repoint indices."""
        print(f"\n{'='*60}")
        ...
        for cycle in range(n_cycles):
            ...
            # Gate stats at end of wake (per-digit breakdown)
            gs = get_gate_stats_by_digit(lgate, model, fm,
            ...
            gs_post = get_gate_stats_by_digit(
                lgate, model, fm,
    ...
    # =================================================================
    # CONDITION 1: WS_LG_shift (phase 1 on 0-6, phase 2 on 0-9)
    # =================================================================
    wslg_shift_model, wslg_shift_fm, wslg_shift_lgate, \
        wslg_shift_ckpts, wslg_shift_hist = run_wslg(
            "WS_LG_shift", shift_batch_indices, shift_repoint_indices)

    # =================================================================
    # CONDITION 2: WS_LG_full (all digits throughout)
    # =================================================================
    wslg_full_model, wslg_full_fm, wslg_full_lgate, \
        wslg_full_ckpts, wslg_full_hist = run_wslg(
            "WS_LG_full", full_batch_indices, full_repoint_indices)
    ...
    save_root = (f"{DATA_DIR}/a2a_forward/mnist_ood_gate"
                 f"/{model_tag}/{gap_tag}")
    ...
    print(f"  {'Cycle':>6s} | {'WS_LG_shift':>16s} | {'WS_LG_full':>16s} | "
          f"{'OL_shift':>16s}")
...
def main(
    ...
    result = a2a_mnist_ood_gate.remote(
        n_cycles=n_cycles,
        phase1_cycles=phase1_cycles,
        n_known_digits=n_known_digits,
        wake_steps=wake_steps,
