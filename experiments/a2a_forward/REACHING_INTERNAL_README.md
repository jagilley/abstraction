...
2. **Multi-step lookahead planner (the live next step).** Swap the greedy one-step `argmax` for an N-step FM rollout and sweep the horizon, with a "composed-trajectory veridicality" readout (`FMⁿ(s)` vs the true n-step state). Because the planner's base forecast is already transferable and veridical-on-what-matters, composition has a real chance (outcome A/C, not the doomed B). Outcomes: **(A)** composition stays faithful → a genuine runnable simulator on the task-relevant subspace; **(C)** it composes in a task/value-consistent space without full-state veridicality (the division-of-labor reframe; connects to a2a gauge symmetry); **(B)** it degrades with horizon → the one-step self-model is not composable. Only meaningful on a **lookahead-requiring** task (plain reaching is greedily solvable, so build obstacles/maze structure so greedy provably fails and composition is forced).
