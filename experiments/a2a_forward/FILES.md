...
| `ACTIVE_VISION_README.md` | [Active-vision looped ViT — the "missing u" / efference-copy test](ACTIVE_VISION_README.md) (self-model-needs-a-loop) — gives the loop a real command (glimpse location) and asks whether command-conditional dynamics require an arity-2 `f(s,u)` forward model that no capacity of arity-1 `f(s)` can match. Observational half: no-`u` control collapses the effect (`cmd_rel_spread` 0.24→0.00); with `u`, smallest command-aware FM beats largest command-blind FM (arity beats resolution), command-conditional cos 0.54/0.67 vs structural 0, scales with loop-necessity (Fashion ~2× MNIST). Causal/behavioral half pending |
