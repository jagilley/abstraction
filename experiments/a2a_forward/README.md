...
**Full writeup**: [LOOPED_README.md](LOOPED_README.md) | **Status**: in progress
...
> **Active-control (reaching) arc.** This section and the two below are the reaching / active-control arc. All of its code and full writeups now live in **[reaching/](reaching/README.md)** — see `reaching/README.md` for the pan-arc summary and reproduction commands.

**Full writeup**: [reaching/ACTIVE_VISION_README.md](reaching/ACTIVE_VISION_README.md) | **Status**: observational half done
...
**Full writeup**: [reaching/REACHING_LOOKAHEAD_README.md](reaching/REACHING_LOOKAHEAD_README.md) | **Status**: done, positive-with-a-dissociation ({obstacles, maze} × MNIST; perfect-sim control done)
...
## Efference-copy cancellation: subtract the forecast instead of adding it (2026-07-13)

**Full writeup**: [CANCELLATION_README.md](CANCELLATION_README.md) | **Status**: done (Fashion), closes LOOPED next-step #3

Tests [ideas/efference_copy_cancellation.md](../../ideas/efference_copy_cancellation.md): the whole arc *adds* the FM forecast (`G(s+p)` — summation); the doc argues it should *subtract* it and propagate the residual (`G(s−p)+p` — efference-copy / predictive coding). Implemented as a **one-flag strict generalization** on the looped ViT (`inject_mode=cancel`, zero-init gate → identical at init); the `add`-mode re-run reproduces the LOOPED headline exactly. **Payoff 3 (restoring force) confirmed three ways**: the perturbation `error_correction` **flips −0.18 → +0.05** (summation amplifies a perturbation, cancellation corrects it); the **mechanism** is a directional cancellation of the forecast (`cos(Δ,inj)` **0.89 → 0.095**), shown to be **topological, not learned** (every operator responds to a raw `+inj` identically at cos ≈ 0.89 — only the subtract-then-re-add topology cancels); and on **OOD inputs** the summation injection turns harmful while cancellation's stays helpful. **Payoff 4 mechanism confirmed** (graded forecast-corruption): cancellation's benefit falls monotonically and **flips negative** as the forecast degrades (+0.136 → −0.148) while summation stays positive (+0.176 → +0.037) — the loss-landscape asymmetry that would drive gate self-closing. **Task accuracy is unchanged** (0.850 vs 0.852). **Payoff 1 (modular dependency) is substrate-limited** — the fresh-FM swap is inconclusive because the low-rank loop's FM is near-unique. Untested: the gate *dynamics* (input-conditioned gate) and a gauge-free substrate (RHM / language) for Payoff 1.

## Confabulation test: does a self-report track the implementation or a self-theory? (2026-07-23)

**Full writeup**: [confabulation/README.md](confabulation/README.md) | **Status**: done — positive on the core dissociation, **null on loop-necessity, and that null disagrees with the RHM sibling**

Tests [ideas/confabulation_test.md](../../ideas/confabulation_test.md) on language (sibling: [rhm/rhm_confabulation.py](../rhm/confabulation/rhm_confabulation.py)). `a_j = FM(a_i) + r` makes the Nisbett & Wilson objection operational: `FM(a_i)` is by construction everything a *self-theory* could produce, so **confabulation lives in the range of the self-model and whatever cannot be confabulated lives in its complement**. A signal is introspective iff it is a function of M's internal state, **not cheaply recoverable from M's I/O map**, and causally used — (2) is the discriminator that kills calibration/entropy while `r` survives.

**The dissociation is strong and capacity-robust.** Implementation targets (residual-direction cluster + continuous direction) beat the best third-party observer by **+0.21 to +0.37** at every instrument capacity; all three controls are at zero or *negative* — **`ENT` is −0.287**, i.e. an observer handed M's output distribution predicts M's own entropy (0.961) far better than M reports it (0.674), which is exactly why calibration-style evidence cannot support an introspection claim. Channel ablation dissociates cleanly (`shuffle_r` collapses the report to ~0.08–0.11, `shuffle_p` leaves 0.35–0.61; residual-only is the *best* variant). Matched-KL steering is positive but modest (1.4–2.1×). The junk-residual trap is disarmed on both discriminators — the advantage moves *with* `ens_cos` (0.78–0.91), opposite to what noise would give, and the residual reproduces the published Cohen's *d* table (sentence_start −0.84 vs −0.85, before_closer +0.82 vs +0.84).

**But closing the loop adds nothing** (CL−OL ≤0.004 at every capacity, on every measure) — the pre-registered falsifier for the strong claim, so on language this is a property of the FM decomposition itself, not of training the model to use it. **The leading confound is ours**: this CL arm is net *worse* than OL (5.352 injected vs 5.320), and its co-trained FM sits at 42% of the predicted blocks — the saturation point — so there was no pressure to develop complementary self-knowledge. **Unresolved**: RHM found CL > OL on every secondary measure (confab margin +0.33 vs +0.15, steering 2.1 vs 1.4). Cheapest decisive test: a 6-layer main model restoring the canonical 3-layer gap with a sub-saturation FM, then re-run the arm comparison.

## Forward self-models, part 3: does the privilege extend past the current residual? (2026-09-13/14)

**Full writeup**: [fsm_part3/README.md](fsm_part3/README.md) | **Status**: done — one epistemic update over three parallel nodes (authorship; dispositional targets on language and on the grammar)

Paper 2's report head reads "what am I computing right now" with a first-person advantage. Pointed at three other self-facts on the same harnesses: **authorship** is public and *more* legible from outside than inside (an observer holding M's likelihoods reads it at 0.76, M reading its own stream at 0.53, on the same positions where the residual reads +0.27; the small self-signal lives entirely in the self-theory span). **Behavioral dynamics** ("will my answer here improve, when did it") are public in every cut on both substrates. **The residual's past** ("how long ago did my computation here settle") is private where the computation is deep and behaviorally silent: on the open-loop grammar the deep hierarchy levels carry +0.10 to +0.29 over every observer including a historian and an activation-access observer, on the class readout while the shallow levels, half of all positions, carry nothing there, so the pooled class number hides it (on the rank readout every level is positive, rising with depth to +0.34); on language the retrospective advantage is +0.026 (rank +0.071) pooled and concentrates in the top residual-magnitude quartile (+0.067). A forward-looking advantage survives instrument correction on the grammar and not on language. The closed loop shrinks the dispositional rows.
...
3. ~~**Looped transformer**~~: *In progress* — confound-free bounded-gate injection is causally necessary and scales with loop-necessity; baseline battery + imprint probes done; runnable-simulator probe negative on MNIST. ~~chase the cancellation signature~~ *Done — cancellation (subtract-the-forecast) built in and confirmed; see [CANCELLATION_README.md](CANCELLATION_README.md).* **Pending**: domain port (RHM / looped language) for the simulator question, cross-decodability matrix, input-conditioned gate for Payoff-4 dynamics. See [LOOPED_README.md](LOOPED_README.md).
