...
*Confidence: moderate — single seed on one world, but with an accidental frozen-reader control, a checkpoint-level mechanism, and retrospective consistency with every gauge that has worked across the arc*
...
- Scope: single seed on one substrate at depth 6; the stream-displaced twins bound stream-position noise, not seed noise, and era-5 magnitudes are consistent with but not separable from the measured floor — [census+assay §Caveats](../../experiments/rhm/practice/census/README.md)
