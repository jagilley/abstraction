...
- Once operation A is compressed to a primitive, it serves as input to higher-order operation B, which then gets compressed in turn — this is how you get hierarchical abstraction without anyone designing the hierarchy — same source; now run live in silico: the practice arc earns level 2, consolidates it into the learner's own planner and executor, and earns level 3 natively *over the routed policy*, twice, at roughly constant cycles per turn — [spiral](../../experiments/rhm/practice/spiral/README.md)
