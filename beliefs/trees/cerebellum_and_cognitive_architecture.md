...
- The sharpest version of that split, with the schedule confound removed by construction: re-keying the miner on a supplied class moves the coverage clock by 34–37× (live L5/L6 at-support gauges at 37/34 where the flat comparator's are identically 0, and an L5 commit the flat key never reaches) and the value clock **not at all** — against flat arms yoked cycle-for-cycle to the same advances and commits the era-4/5 gap is ≤0.053 in every cell and changes sign, where the *un-yoked* comparison read −0.40. The arc's largest single change to the vocabulary is its smallest change to value, and an un-yoked read would have credited the representation with the clock's effect — [en_s0](../../experiments/rhm/practice/enharmonic/figures/en_s0_reduction.txt), [en_s1 clock yokes](../../experiments/rhm/practice/enharmonic/figures/en_s1_reduction.txt)
...
- Individual bad entries are cheap, and the offline audit of the substrate now says the main reason is that **most of them were never bad**: on this world's rule draw, 68–95% of the committed rows a feature-space oracle marks junk are legal programs relabelled by a last-writer-wins inverse map — they render to a true key's leaf string and repair instances — so the arc's 0.15–0.50 committed-table precisions understate truth badly, and "a 6.2%-true table yields 71–76% true executions" is mostly a labelling artifact rather than an executor-DP filter. The genuinely off-grammar minority is **inert rather than filtered**: 0 of 101 token-junk rows repair a single instance at any level in any arm, and they sit alone. The channel itself is real — by observation *mass* at the mining node the relabelled share is a minority (~36–47%) — but selection removes the diverse real misreads before commit, which is why the committed rows are unrepresentative of it — [enharmonic sizing §6](../../experiments/rhm/practice/enharmonic/sizing/SIZING.md), [census+assay](../../experiments/rhm/practice/census/README.md)
- The selection that guards a chunk's life is an **argmax, and it latches**: `macro_features`' max-sum DP serves each call from one row, so where two rows write identical tokens it starves the loser below 1% of the winner's use mass in 68% of such pairs — the same shape as π starving a bogus address 6× under native routing. This is why junk dripping in mid-flight de-funds a level via π's mass while junk arriving at commit time costs little; and it is the mechanism's price, because the pairs the use record separates *most* are the ones that are literally the same program. No use-record statistic recovers which entries are aliases (three similarity statistics × two label sets × L2–L4 against 2,000-permutation nulls: nothing clears in the direction a merge criterion needs, and the cells that clear do so with the opposite sign) — [enharmonic sizing §4](../../experiments/rhm/practice/enharmonic/sizing/SIZING.md), [native](../../experiments/rhm/practice/native/README.md)
...
- The **fourth** turn is blocked by the learner's index, not by the grammar. Keyed on the flat lower row, level 5 needs ~1.3e5 mining observations against a run's own ~1,600 and never arrives — its L5/L6 at-support gauges are identically 0, so the commit thermostat's comparator fails *mechanically* rather than at a tuned floor. Re-keyed on the **token class** (closed under composition, so the level restates as `C[l+1] ⊆ C[l] × C[l]` and L5 shrinks from 205,824 flat keys to 73 legal class pairs) arrival is solved — 37/34 keys at support where flat's are 0, and one arm commits L5. What binds next is what the **freeze** captured: the committed L4 book holds 3–4 of the 13 L4 classes while the same arm's live miner reaches 5–9, and the L5 build only ever sees the frozen ones. So a ratchet's next rung is gated first by the coordinate the lower level is indexed in and then by the resolution the freeze locked in — [enharmonic sizing](../../experiments/rhm/practice/enharmonic/sizing/SIZING.md), [en_s0](../../experiments/rhm/practice/enharmonic/figures/en_s0_reduction.txt)
